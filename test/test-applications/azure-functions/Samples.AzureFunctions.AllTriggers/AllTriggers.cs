...
namespace Samples.AzureFunctions.AllTriggers
{
	...
	{
		private const string EveryTenSeconds = "*/1000 * * * * *";
        private static string DatadogEnv = Environment.GetEnvironmentVariable("DD_ENV") ?? "unset";
		...
		// {
		// 	log.LogInformation($"[env:{DatadogEnv}] C# Timer trigger function executed at: {DateTime.Now}");
		...
		public async Task<IActionResult> SimpleHttpTrigger(
			...
			ILogger log)
		{
			log.LogInformation($"[env:{DatadogEnv}] C# HTTP trigger function processed a request.");
		...
		private async Task Attempt(Func<Task> action, ILogger log)
		{
			...
			{
				log.LogError(ex, $"[env:{DatadogEnv}] Trigger attempt failure");
			}
		}
	}
}
