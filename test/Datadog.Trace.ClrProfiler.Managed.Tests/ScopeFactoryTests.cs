...
namespace Datadog.Trace.ClrProfiler.Managed.Tests
{
    public class ScopeFactoryTests
    {
        ...
        public void CleanUri_ResourceName(string uri, string method, string expected)
        {
            ...
            using (var automaticScope = ScopeFactory.CreateOutboundHttpScope(tracer, method, new Uri(uri), (int)IntegrationIds.HttpMessageHandler, out _))
            {
        ...
        public void CleanUri_HttpUrlTag(string uri, string expected)
        {
            ...
            using (var automaticScope = ScopeFactory.CreateOutboundHttpScope(tracer, method, new Uri(uri), (int)IntegrationIds.HttpMessageHandler, out var tags))
            {
        ...
        public void CreateOutboundHttpScope_AlwaysCreatesOneAutomaticInstrumentationScope(int integration1, int integration2)
        {
            ...
            using (var manualScope = tracer.StartActive("http.request"))
            {
                ...
                using (var automaticScope1 = ScopeFactory.CreateOutboundHttpScope(tracer, method, new Uri(url), integration1, out _))
                {
                    using (var automaticScope2 = ScopeFactory.CreateOutboundHttpScope(tracer, method, new Uri(url), integration2, out _))
                    {
