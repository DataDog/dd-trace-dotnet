...
namespace Datadog.InstrumentedAssemblyVerification
{
    ...
    public class VerificationsRunner
    {
        ...
        public VerificationsRunner(string instrumentedModulePath, string originalModulePath, List<string> methods, IList<Verification> verifications = null)
        {
            _instrumentedModulePath = instrumentedModulePath;
            _originalModulePath = originalModulePath;
            _methods = methods;
            _verifications = verifications ?? new List<Verification> { Verification.ILSpy, Verification.PrepareMethod, Verification.ILVerify, Verification.PEVerify };
        }
                ...
                try
                {
                    switch (verification)
                    {
                        ...
                        case Verification.ILVerify:
                            ...
                            errors.AddRange(VerifyOriginalAndInstrumentedAndReturnDiff(
                                                new ILVerifyVerification(_originalModulePath, logger),
                                                new ILVerifyVerification(_instrumentedModulePath, logger)));
                            break;
                        default:
