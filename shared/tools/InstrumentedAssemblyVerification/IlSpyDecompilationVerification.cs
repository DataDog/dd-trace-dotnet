...
namespace Datadog.InstrumentedAssemblyVerification
{
    ...
    internal class IlSpyDecompilationVerification : IVerification
    {
        ...
        {
            _logger.Info($"Start verify {_peFile} with {nameof(IlSpyDecompilationVerification)}");
            ...
            foreach (string method in _methods)
            {
                try
                {
                    ...
                    if (typeInfo == null)
                    {
                        string error = $"Type {typeName} not found in the compilation of {decompiler.TypeSystem.MainModule.AssemblyName}";
                        _logger.Error(error);
                        continue;
                    }
                    ...
                    {
                        string error = $"Method {methodAndParametersName} not found in type {typeInfo.FullTypeName}";
                        _logger.Error(error);
                        continue;
                    }
                    ...
                    if (decompiled == null)
                    {
                        string error = $"Compilation of {tokenOfFirstMethod.FullName} failed";
                        errors.Add(error);
                        continue;
                    }
            ...
            if (errors.Count == 0)
            {
                _logger.Info($"{nameof(IlSpyDecompilationVerification)} finish without errors");
            }
            else
            {
        ...
        private List<string> GetDecompilationIlErrors(SyntaxTree decompiledMethod, string methodAndParametersName, IMethod method)
        {
            ...
            if (ilFunction?.Warnings?.Any() == true)
            {
                errors.AddRange(ilFunction.Warnings.
                                           Select(what => $"Error: {what} when decompiling {where}")
            ...
            foreach (var errorNode in decompiledMethod.DescendantNodes().OfType<ErrorExpression>())
            {
                ...
                string error = $"Error: {comment} when decompiling {where}";
