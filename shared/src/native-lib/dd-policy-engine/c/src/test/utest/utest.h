...
utest_mul_div(const utest_int64_t value, const utest_int64_t numer, const utest_int64_t denom) {
...
#define UTEST(SET, NAME)                                                                                               \
  ...
  UTEST_INITIALIZER(utest_register_##SET##_##NAME) {                                                                   \
    ...
    utest_state.tests = UTEST_PTR_CAST(                                                                                \
        struct utest_test_state_s *,                                                                                   \
        utest_realloc(                                                                                                 \
            UTEST_PTR_CAST(void *, utest_state.tests), sizeof(struct utest_test_state_s) * utest_state.tests_length    \
        )                                                                                                              \
    );                                                                                                                 \
...
int utest_main(int argc, const char *const argv[]) {
  ...
  if (utest_state.output) {
    ...
    fprintf(
        utest_state.output, "<testsuites tests=\"%" UTEST_PRIu64 "\" name=\"All\">\n",
        UTEST_CAST(utest_uint64_t, ran_tests)
    );
    fprintf(
        utest_state.output, "<testsuite name=\"Tests\" tests=\"%" UTEST_PRIu64 "\">\n",
        UTEST_CAST(utest_uint64_t, ran_tests)
    );
  }
  ...
  for (index = 0; index < utest_state.tests_length; index++) {
...

    {
      ...
      if (UTEST_TEST_FAILURE == result) {
        printf(
            "%s[  FAILED  ]%s %s (%" UTEST_PRId64 "%s)\n", colours[RED], colours[RESET], utest_state.tests[index].name,
