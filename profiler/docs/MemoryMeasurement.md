...
- **Capacity planning** - Predict memory requirements
...
Tracks memory used by thread management structures.

#### API

**Get total memory size:**
```cpp
size_t totalBytes = managedThreadList->GetMemorySize();
Log::Info("Memory usage: ", totalBytes, " bytes");
```

**Log detailed breakdown:**
```cpp
managedThreadList->LogMemoryBreakdown();
```
...
Tracks memory used by frame resolution and type caching.

#### API

**Get total memory size:**
```cpp
size_t totalBytes = frameStore->GetMemorySize();
Log::Info("Memory usage: ", totalBytes, " bytes");
```
...
Tracks memory used by PDB debug information caching.

#### API

**Get total memory size:**
```cpp
size_t totalBytes = debugInfoStore->GetMemorySize();
```

**Log detailed breakdown:**
```cpp
debugInfoStore->LogMemoryBreakdown();
```
...
Tracks memory used by AppDomain name caching.

#### API

**Get total memory size:**
```cpp
size_t totalBytes = appDomainStore->GetMemorySize();
```

**Log detailed breakdown:**
```cpp
appDomainStore->LogMemoryBreakdown();
```
...
Tracks memory used by application metadata storage (service name, environment, version, git metadata).

#### API

**Get total memory size:**
```cpp
size_t totalBytes = applicationStore->GetMemorySize();
```

**Log detailed breakdown:**
```cpp
applicationStore->LogMemoryBreakdown();
```
...
Tracks memory used by runtime ID fallback cache.

#### API

**Get total memory size:**
```cpp
size_t totalBytes = runtimeIdStore->GetMemorySize();
```

**Log detailed breakdown:**
```cpp
runtimeIdStore->LogMemoryBreakdown();
```
...
**Note:** This cache is only used as a fallback when the native loader is not available, so it's often empty in production.
...
### 5. Add Tests
```cpp
TEST(MyComponentTest, CheckMemoryMeasurement) {
    MyComponent component;
    
    size_t initial = component.GetMemorySize();
    ASSERT_GT(initial, 0);
    
    // Add data
    component.AddData(/* ... */);
    
    size_t withData = component.GetMemorySize();
    ASSERT_GT(withData, initial);
    
    // Remove data
    component.Clear();
    
    size_t afterClear = component.GetMemorySize();
    ASSERT_LE(afterClear, withData);
}
```

### 7. Register Metrics and Add Shutdown Logging
...
Tracks memory used by CPU time tracking for threads.

#### API

**Get total memory size:**
```cpp
size_t totalBytes = threadsCpuManager->GetMemorySize();
Log::Info("Memory usage: ", totalBytes, " bytes");
```

**Log detailed breakdown:**
```cpp
threadsCpuManager->LogMemoryBreakdown();
```
...
Tracks memory used by exception profiling.

#### API

**Get total memory size:**
```cpp
size_t totalBytes = exceptionsProvider->GetMemorySize();
Log::Info("Memory usage: ", totalBytes, " bytes");
```

**Log detailed breakdown:**
```cpp
exceptionsProvider->LogMemoryBreakdown();
```
...
Tracks memory used by heap snapshot management.

#### API

**Get total memory size:**
```cpp
size_t totalBytes = heapSnapshotManager->GetMemorySize();
Log::Info("Memory usage: ", totalBytes, " bytes");
```

**Log detailed breakdown:**
```cpp
heapSnapshotManager->LogMemoryBreakdown();
```
...
Tracks memory used by HTTP/network profiling.

#### API

**Get total memory size:**
```cpp
size_t totalBytes = networkProvider->GetMemorySize();
Log::Info("Memory usage: ", totalBytes, " bytes");
```

**Log detailed breakdown:**
```cpp
networkProvider->LogMemoryBreakdown();
```
...
Metrics use `ProxyMetric` which calls `GetMemorySize()` on each component periodically:

```cpp
_metricsRegistry.GetOrRegister<ProxyMetric>("dotnet_memory_footprint_frame_store", [this]() {
    return _pFrameStore->GetMemorySize();
});
```

The `MetricsRegistry::Collect()` method is invoked by the metrics sender to gather all metric values for emission.
...
Memory breakdown logging occurs in `CorProfilerCallback::Shutdown()`, after all providers are stopped and before `DisposeInternal()`.
...
- **Profile/ProfileImpl** - Accumulated profile data between exports
