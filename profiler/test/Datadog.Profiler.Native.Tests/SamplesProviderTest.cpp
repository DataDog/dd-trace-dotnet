...
void OnWrite(TestSamplesProvider& provider)
{
...
void OnRead(TestSamplesProvider& provider, uint64_t* pReadSamples)
{

}

TEST(SamplesTimeProviderTest, MultiThreadCheck)
{
    TestSamplesProvider provider;

    uint64_t readSamples = 0;
    std::thread _writer = std::thread(&OnWrite, provider);
    std::thread _reader = std::thread(&OnRead, provider, &readSamples);
