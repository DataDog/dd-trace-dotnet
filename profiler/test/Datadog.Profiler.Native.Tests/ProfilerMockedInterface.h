...
std::shared_ptr<Sample> CreateSample(libdatadog::SymbolsStore* symbolsStore, std::string_view runtimeId, const std::vector<std::pair<ddog_prof_FunctionId, ddog_prof_MappingId>>& callstack, const std::vector<std::pair<ddog_prof_StringId, std::string>>& labels, std::int64_t value);

std::vector<std::pair<ddog_prof_FunctionId, ddog_prof_MappingId>> CreateCallstack(int depth, libdatadog::SymbolsStore* symbolsStore);
