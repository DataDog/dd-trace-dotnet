...
namespace Datadog.Profiler.IntegrationTests.CodeHotspot
{
    public class CodeHotspotTest
    {
        ...
        {
            var runner = new TestApplicationRunner(appName, framework, appAssembly, _output, enableTracer: true, commandLine: ScenarioCodeHotspot);
            runner.TestDurationInSeconds = 40;

            // The endpoint profiling feature requires the tracer to complete at least one
            // request to /Products/IndexSlow (1s+ per request due to SpinWait) AND propagate
            // the endpoint info to the native profiler before the next pprof export (every 3s).
            // On slow CI runners (especially Windows), 20s was insufficient — startup time +
            // the async tracer-to-profiler endpoint propagation could consume most of the window,
            // leaving no export cycle with endpoint labels attached.
