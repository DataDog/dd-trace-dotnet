    ...
    struct sigaction *_Nullable restrict oldact)
{
	...
	// Make sure our handler is called first and
	// it correctly forwards the call to the handler in `act`
	if ((act->sa_flags & SA_SIGINFO) == SA_SIGINFO)
	    ...
	    // 
            if (oldact != NULL &&
	        // How do we andle case where some register with SA_SIGINFO and another no ??
    ...
    // Unregister signal SIGSEGV
    // humm what if oldact is NULL, is it considered as unregistering the handler ?
