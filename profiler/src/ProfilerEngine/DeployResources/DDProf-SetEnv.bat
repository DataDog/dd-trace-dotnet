...
IF "%2"=="" (
    ...
    SET DD_MONITORING_HOME=%~dp0
) ELSE (
    SET DD_MONITORING_HOME=%2
...
SET COR_PROFILER_PATH_64=%DD_MONITORING_HOME%Datadog.AutoInstrumentation.NativeLoader.x64.dll
SET COR_PROFILER_PATH_32=%DD_MONITORING_HOME%Datadog.AutoInstrumentation.NativeLoader.x86.dll

SET CORECLR_ENABLE_PROFILING=1
SET CORECLR_PROFILER={846F5F1C-F9AE-4B07-969E-05C26BC060D8}
SET CORECLR_PROFILER_PATH_64=%DD_MONITORING_HOME%Datadog.AutoInstrumentation.NativeLoader.x64.dll
SET CORECLR_PROFILER_PATH_32=%DD_MONITORING_HOME%Datadog.AutoInstrumentation.NativeLoader.x86.dll

IF DEFINED DD_DOTNET_CONTINUOUS_PROFILER_BITNESS (
    SET COR_PROFILER_PATH=%DD_MONITORING_HOME%Datadog.AutoInstrumentation.NativeLoader.%DD_DOTNET_CONTINUOUS_PROFILER_BITNESS%.dll
    SET CORECLR_PROFILER_PATH=%DD_MONITORING_HOME%Datadog.AutoInstrumentation.NativeLoader.%DD_DOTNET_CONTINUOUS_PROFILER_BITNESS%.dll
) ELSE (
...
ECHO\
ECHO DD_DOTNET_PROFILER_HOME=%DD_MONITORING_HOME%/ContinuousProfiler
ECHO\
