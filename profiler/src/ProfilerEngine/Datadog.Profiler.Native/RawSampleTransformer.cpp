...
std::shared_ptr<Sample> RawSampleTransformer::Transform(const RawSample& rawSample, std::vector<SampleValueTypeProvider::Offset> const& offsets, libdatadog::SymbolsStore* pSymbolsStore)
{
    auto sample = std::make_shared<Sample>(rawSample.Timestamp, std::string_view(), rawSample.Stack.Size(), pSymbolsStore);
    Transform(rawSample, sample, offsets, pSymbolsStore);
    return sample;
}

void RawSampleTransformer::Transform(const RawSample& rawSample, std::shared_ptr<Sample>& sample, std::vector<SampleValueTypeProvider::Offset> const& offsets, libdatadog::SymbolsStore* pSymbolsStore)
{
    sample->Reset();
    ...
    {
        sample->AddLabel(NumericLabel{pSymbolsStore->GetLocalRootSpanId(), rawSample.LocalRootSpanId});
        sample->AddLabel(NumericLabel{pSymbolsStore->GetSpanId(), rawSample.SpanId});
    }
    ...
    rawSample.OnTransform(sample, offsets, pSymbolsStore);
}
