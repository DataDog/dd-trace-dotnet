...
std::shared_ptr<ManagedThreadInfo> ManagedThreadList::FindByProfilerId(uint32_t profilerThreadInfoId)
{
    std::lock_guard<std::recursive_mutex> lock(_mutex);
...
{
    auto pThreadInfo = FindByProfilerId(profilerThreadInfoId);
