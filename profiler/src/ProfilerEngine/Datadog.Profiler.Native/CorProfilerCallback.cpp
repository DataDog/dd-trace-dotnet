...
void CorProfilerCallback::InitializeServices()
{
    ...
    _pSymbolsStore = RegisterService<libdatadog::SymbolsStore>();
    ...
    _pFrameStore = std::make_unique<FrameStore>(_pCorProfilerInfo, _pConfiguration.get(), _pDebugInfoStore.get(), _managedCodeCache.get(), _pSymbolsStore);
    ...
    if (_pConfiguration->IsWallTimeProfilingEnabled())
    {
        ...
        _pWallTimeProvider = RegisterService<WallTimeProvider>(valueTypeProvider, _rawSampleTransformer.get(), pool, _pSymbolsStore);
    }
    ...
    if (_pConfiguration->IsCpuProfilingEnabled())
    {
        ...
        if (_pConfiguration->GetCpuProfilerType() == CpuProfilerType::ManualCpuTime)
        {
            _pCpuTimeProvider = RegisterService<CpuTimeProvider>(
                valueTypeProvider, _rawSampleTransformer.get(), MemoryResourceManager::GetDefault(), _pSymbolsStore);
        }
        else
        {
            ...
            _pCpuSampleProvider = RegisterService<CpuSampleProvider>(valueTypeProvider, _rawSampleTransformer.get(), _pCpuProfilerRb.get(), _metricsRegistry, _pSymbolsStore);
        }
#else // WINDOWS
        _pCpuTimeProvider = RegisterService<CpuTimeProvider>(
            valueTypeProvider, _rawSampleTransformer.get(), MemoryResourceManager::GetDefault(), _pSymbolsStore);
    ...
    {
        _gcThreadsCpuProvider = std::make_unique<GCThreadsCpuProvider>(valueTypeProvider, _rawSampleTransformer.get(), _metricsRegistry, _pSymbolsStore);
