...
libdatadog::profile_unique_ptr CreateProfile(std::vector<SampleValueType> const& valueTypes, std::string const& periodType, std::string const& periodUnit, libdatadog::SymbolsStore* pSymbolsStore);

std::unique_ptr<Profile> Profile::Create(IConfiguration* configuration, std::vector<SampleValueType> const& valueTypes, std::string const& periodType, std::string const& periodUnit, std::string applicationName, SymbolsStore* pSymbolsStore)
{
    auto impl = CreateProfile(valueTypes, periodType, periodUnit, pSymbolsStore);
    if (impl == nullptr)
    {
        return nullptr;
    }
...
libdatadog::profile_unique_ptr CreateProfile(std::vector<SampleValueType> const& valueTypes, std::string const& periodType, std::string const& periodUnit, libdatadog::SymbolsStore* pSymbolsStore)
{
    ...
    auto dictHandle = reinterpret_cast<ddog_prof_ProfilesDictionaryHandle>(pSymbolsStore->GetDictionary());
