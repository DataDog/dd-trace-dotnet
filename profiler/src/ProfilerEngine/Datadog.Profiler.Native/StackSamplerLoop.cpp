...
void StackSamplerLoop::WalltimeProfilingIteration()
{
    ...
    do
    {
        ...
        CollectOneThreadStackSample(_targetThread, thisSampleTimestampNanosecs, duration, RawWallTimeSample{});
        ...
        if (_targetThread != nullptr)
        {
            ...
            if (isRunning)
            {
                ...
                if (cpuForSample > 0)
                {
                    ...
                    CollectOneThreadStackSample(_targetThread, thisSampleTimestampNanosecs, cpuForSample, RawCpuSample{});
    ...
    TRawSample&& rawSample)
{
    ...
    time_t currentUnixTimestamp = {};
    {
        ...
        _pStackFramesCollector->PrepareForNextCollection(&rawSample);
        ...
        // block used to ensure that NotifyIterationFinished gets called
        {
            ...
            isStackSnapshotSuccessful = (countCollectedStackFrames > 0);
    ...
    {
        static_assert(std::is_same_v<TRawSample, RawCpuSample>);
        _pCpuTimeCollector->Add(std::move(rawSample));
    }
}