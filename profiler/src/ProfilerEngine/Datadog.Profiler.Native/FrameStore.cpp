...
// FrameInfo should be something like:
// - function id
// - mapping id
// - start line
std::pair<bool, MyFrameInfo> FrameStore::GetFrame(uintptr_t instructionPointer)
{
    ...
    if (instructionPointer <= MaxFakeIP)
    {
        ...
        {
            return { true, MyFrameInfo{_pSymbolsStore->GetFakeContentionFrameId(), _pSymbolsStore->GetFakeModuleId(), 0} };
        }
        else
        ...
        {
            return { true, MyFrameInfo{_pSymbolsStore->GetUnknownManagedFrameId(), _pSymbolsStore->GetFakeModuleId(), 0} };
        }
    }
    ...
    if (!result.has_value())
    {
        ...
        return {true, MyFrameInfo{_pSymbolsStore->GetNotResolvedFrameId(), _pSymbolsStore->GetNotResolvedModuleId(), 0}};
    }
    ...
    else
    {
        ...
        {
            return {false, MyFrameInfo{_pSymbolsStore->GetNotResolvedFrameId(), _pSymbolsStore->GetNotResolvedModuleId(), 0}};
        }

        auto [moduleName, frame] = GetNativeFrame(instructionPointer);
        return {true, MyFrameInfo{frame, moduleName, 0}};
    }
}
...
std::pair<ddog_prof_MappingId, ddog_prof_FunctionId> FrameStore::GetNativeFrame(uintptr_t instructionPointer)
{
...
MyFrameInfo FrameStore::GetManagedFrame(FunctionID functionId)
{
    {
    ...
    {
        return {_pSymbolsStore->GetUnknownManagedFrameId(), _pSymbolsStore->GetUnknownManagedAssemblyId(), 0};
    }
    ...
    if (!typeInCache)
    {
        ...
        if (!BuildTypeDesc(pMetadataImport.Get(), classId, moduleId, mdTokenType, typeDesc, false, nullptr))
        {
            ...
            std::stringstream builder;
            builder << "|lm:Unknown-Assembly |ns: |ct:Unknown-Type |cg: |fn:" << std::move(methodName) << " |fg:" << std::move(methodGenericParameters) << " |sg:" << std::move(signature);
            ...
            value = {functionId, _pSymbolsStore->GetUnknownManagedAssemblyId(), 0};
            return value;
        }
...

    {
        ...
        auto [it, _] = _methods.emplace(functionId, MyFrameInfo{*fnId, *mappingId, debugInfo.StartLine});
