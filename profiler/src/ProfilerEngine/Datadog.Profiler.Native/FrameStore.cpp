...
std::pair<ddog_prof_MappingId2, ddog_prof_FunctionId2> FrameStore::GetNativeFrame(uintptr_t instructionPointer)
{
...
FrameInfoView FrameStore::GetManagedFrame(FunctionID functionId)
{
    ...
    if (!typeInCache)
    {
        ...
        if (!BuildTypeDesc(pMetadataImport.Get(), classId, moduleId, mdTokenType, typeDesc, false, nullptr))
        {
            ...
            auto functionId = _pSymbolsStore->InternFunction(builder.str(), "").value_or(_pSymbolsStore->GetUnknownManagedFrameId());
            // TODO maybe we should log that InternFunction failed
