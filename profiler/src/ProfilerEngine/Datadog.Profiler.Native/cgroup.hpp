...
// Original file taken from https://github.com/dotnet/runtime/blob/b3a16a10978c1139c40cee4ac3ff1a5a4101058c/src/coreclr/pal/src/misc/cgroup.cpp
...
private:
    ...
    static char* FindCGroupPathForSubsystem(bool (*is_subsystem)(const char *))
    {
        ...
        while (!result && getline(&line, &lineLen, cgroupfile) != -1)
        {
            ...
            if (s_cgroup_version == 1)
            {
                ...
                char* context = nullptr;
                char* strTok = strtok(subsystem_list, ",");
    ...
    static bool GetCGroup2CpuLimit(double *val)
    {
        ...
        max_quota_string = strtok(line, " ");
        if (max_quota_string == nullptr)
        {
            goto done;
        }
        period_string = strtok(nullptr, " ");
