...
public:
    ...
    const char* addr()
    {
        return (const char*)strtoul(_addr, NULL, 16);
    }
    char type()
    {
        return _desc != NULL ? _desc[1] : 0;
    }
    const char* name()
    {
        return _desc + 3;
...
public:
    ...
    const char* end()
    {
        return (const char*)strtoul(_end, NULL, 16);
    }
    unsigned long offs()
    {
        return strtoul(_offs, NULL, 16);
    }
    unsigned long inode()
    {
        return strtoul(_inode, NULL, 10);
    }
...
bool ElfParser::loadSymbolsUsingBuildId()
{
    ...
    char path[PATH_MAX];
    char* p = path + sprintf(path, "/usr/lib/debug/.build-id/%02hhx/", (unsigned char)build_id[0]);
