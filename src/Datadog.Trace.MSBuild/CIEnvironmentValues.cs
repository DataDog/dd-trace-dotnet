...
namespace Datadog.Trace.MSBuild
{
    internal static class CIEnvironmentValues
    {
        ...
        public static void DecorateSpan(Span span)
        {
            ...
            span.SetTag(BuildTags.CIProvider, Provider);
            span.SetTag(BuildTags.CIBuildId, BuildId);
            span.SetTag(BuildTags.CIBuildNumber, BuildNumber);
            span.SetTag(BuildTags.CIBuildUrl, BuildUrl);

            span.SetTag(BuildTags.GitRepository, Repository);
            span.SetTag(BuildTags.GitCommit, Commit);
            span.SetTag(BuildTags.GitBranch, Branch);

            span.SetTag(BuildTags.BuildSourceRoot, SourceRoot);
        }
    }
}
