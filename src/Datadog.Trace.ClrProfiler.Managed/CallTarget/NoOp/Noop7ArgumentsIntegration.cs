...
namespace Datadog.Trace.ClrProfiler.CallTarget.NoOp
{
    ...
    public static class Noop7ArgumentsIntegration
    {
        ...
        {
            CallTargetReturn<TReturn> rValue = new CallTargetReturn<TReturn>(returnValue);
            string msg = $"{rValue} {nameof(Noop7ArgumentsIntegration)}.OnAsyncMethodEnd<{typeof(TTarget).FullName}, {typeof(TReturn).FullName}>({instance}, {returnValue}, {exception}, {originalTask}, {state})";
            Log.Information(msg);
            Console.WriteLine(msg);
            return rValue;
        }
