...
namespace Datadog.Trace.Agent.NamedPipes
{
    ...
    /// Originally named HttpMessageContent.
    /// </summary>
    internal class NamedPipeHttpMessageContent : HttpContent
    {
        ...
        public NamedPipeHttpMessageContent(HttpRequestMessage httpRequest)
        {
            if (httpRequest == null)
            {
        ...
        public NamedPipeHttpMessageContent(HttpResponseMessage httpResponse)
        {
        ...
        public async Task SerializeToPipeStream(PipeStream stream)
        {
            await base.SerializeToStreamAsync(stream, null);
        }
            ...
            if (contentType != null)
            {
                if (!contentType.MediaType.Equals(DefaultMediaType, StringComparison.OrdinalIgnoreCase))
                {
                    if (throwOnError)
                    {
                        ...
                        throw new ArgumentException("Invalid media type for content.");
                    }
                    else
                    {
                        return false;
        ...
        protected override bool TryComputeLength(out long length)
        {
            ...
            if (hasContent)
            {
                // START VENDOR CHANGES
                Stream readStream = null;
                var success = false;

                try
                {
                    readStream = _streamTask.Value.GetAwaiter().GetResult();
                    success = true;
                }
                catch
                {
