...
namespace Datadog.Trace.Agent.NamedPipes
{
    internal static class HexConverter
    {
...
#endif
        // [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static void ToCharsBuffer(byte value, byte[] buffer, int startingIndex = 0, Casing casing = Casing.Upper)
        {
            uint difference = (((uint)value & 0xF0U) << 4) + ((uint)value & 0x0FU) - 0x8989U;
            uint packedResult = ((((uint)(-(int)difference) & 0x7070U) >> 4) + difference + 0xB9B9U) | (uint)casing;
        ...
        public static void EncodeToUtf16(byte[] bytes, byte[] chars, Casing casing = Casing.Upper)
        {
        ...
        public static bool TryDecodeFromUtf16(byte[] chars, byte[] bytes)
        {
            return TryDecodeFromUtf16(chars, bytes, out _);
        }

        public static bool TryDecodeFromUtf16(byte[] chars, byte[] bytes, out int charsProcessed)
        {
        ...
        public static ReadOnlybyte[] CharToHexLookup => new byte[]
        {
