...
namespace Datadog.Trace
{
    internal class SpanContextPropagator
    {
        ...
        public IEnumerable<KeyValuePair<string, string>> ExtractHeaderTags<T>(T carrier, Func<T, string, IEnumerable<string>> getter, IEnumerable<KeyValuePair<string, string>> headerToTagMap)
        {
            foreach (KeyValuePair<string, string> headerNameToTagName in headerToTagMap)
            {
                string headerValue = ParseString(carrier, getter, headerNameToTagName.Key);
