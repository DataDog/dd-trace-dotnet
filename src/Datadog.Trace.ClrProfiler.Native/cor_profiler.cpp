...
CorProfiler::Initialize(IUnknown* cor_profiler_info_unknown) {
  ...
  if (tracing_enabled == WStr("0") || tracing_enabled == WStr("false")) {
    Info("DATADOG TRACER DIAGNOSTICS - Profiler disabled in ",
  ...
  if (!include_process_names.empty() &&
    ...
    Info("DATADOG TRACER DIAGNOSTICS - Profiler disabled: ", process_name,
         " not found in ", environment::include_process_names, ".");
    return E_FAIL;
  }
  ...
  if (Contains(exclude_process_names, process_name)) {
    Info("DATADOG TRACER DIAGNOSTICS - Profiler disabled: ", process_name,
         " found in ", environment::exclude_process_names, ".");
    return E_FAIL;
  }
  ...
  if (FAILED(hr)) {
    Warn(
        "DATADOG TRACER DIAGNOSTICS - Failed to attach profiler: interface "
        "ICorProfilerInfo4 not found.");
    return E_FAIL;
  }
  ...
  if (azure_app_services_value == WStr("1")) {
    ...
    if (app_pool_id_value.size() > 1 && app_pool_id_value.at(0) == '~') {
      Info("DATADOG TRACER DIAGNOSTICS - Profiler disabled: ",
           environment::azure_app_services_app_pool_id, " ", app_pool_id_value,
    ...
    if (cli_telemetry_profile_value == WStr("AzureKudu")) {
      Info(
          "DATADOG TRACER DIAGNOSTICS - Profiler disabled: ", app_pool_id_value,
  ...
  if (integrations_paths.empty()) {
    Warn("DATADOG TRACER DIAGNOSTICS - Profiler disabled: ",
         environment::integrations_path, " environment variable not set.");
    return E_FAIL;
  }
  ...
  if (is_calltarget_enabled) {
    rejit_handler = new RejitHandler(
        this->info_,
        [this](RejitHandlerModule* mod, RejitHandlerModuleMethod* method) {
  ...
  if (integration_methods_.empty()) {
    Warn(
        "DATADOG TRACER DIAGNOSTICS - Profiler disabled: no enabled "
        "integrations found.");
    return E_FAIL;
  } else {
  ...
  if (domain_neutral_instrumentation == WStr("1") ||
      domain_neutral_instrumentation == WStr("true")) {
  ...
  Info("Overriding event mask");

  event_mask = COR_PRF_MONITOR_JIT_COMPILATION | COR_PRF_MONITOR_MODULE_LOADS |
               COR_PRF_MONITOR_ASSEMBLY_LOADS | COR_PRF_ENABLE_REJIT;

  if (SUCCEEDED(hr)) {
    Debug("Interface ICorProfilerInfo6 found.");
    hr = info6->SetEventMask2(event_mask, COR_PRF_HIGH_MONITOR_NONE);

    if (instrument_domain_neutral_assemblies) {
      Info("Note: The ", environment::domain_neutral_instrumentation,
           " environment variable is not needed when running on .NET Framework "
           "4.5.2 or higher, and will be ignored.");
    }
  } else {
    hr = this->info_->SetEventMask(event_mask);

    if (instrument_domain_neutral_assemblies) {
      Info("Detected environment variable ",
           environment::domain_neutral_instrumentation, "=",
           domain_neutral_instrumentation);
      Info(
          "Enabling automatic instrumentation of methods called from "
          "domain-neutral assemblies. ",
          "Please ensure that there is only one AppDomain or, if applications "
          "are being hosted in IIS, ",
          "ensure that all Application Pools have at most one application "
          "each. ",
  ...
  if (FAILED(hr)) {
    Warn(
        "DATADOG TRACER DIAGNOSTICS - Failed to attach profiler: unable to set "
        "event mask.");
    return E_FAIL;
  }
...
CorProfiler::AssemblyLoadFinished(AssemblyID assembly_id, HRESULT hr_status) {
  ...
  if (assembly_info.name == WStr("Datadog.Trace.ClrProfiler.Managed")) {
    ...
    if (assembly_version == ToWSTRING(PROFILER_VERSION)) {
      Info("AssemblyLoadFinished: Datadog.Trace.ClrProfiler.Managed v",
           assembly_version, " matched profiler version v", PROFILER_VERSION);
      ...
      if (runtime_information_.is_desktop() && corlib_module_loaded) {
        ...
        if (assembly_info.app_domain_id == corlib_app_domain_id) {
          Info(
              "AssemblyLoadFinished: Datadog.Trace.ClrProfiler.Managed was "
              "loaded domain-neutral");
          managed_profiler_loaded_domain_neutral = true;
        } else {
          Info(
              "AssemblyLoadFinished: Datadog.Trace.ClrProfiler.Managed was not "
              "loaded domain-neutral");
        }
      }
    } else {
...
HRESULT STDMETHODCALLTYPE CorProfiler::ModuleLoadFinished(ModuleID module_id,
  ...
  // (which is domain-neutral) but the Datadog.Trace.ClrProfiler.Managed.Loader
  // assembly that the startup hook loads from a byte array will be loaded into
  // a non-shared AppDomain. In this case, do not insert another startup hook
  // into that non-shared AppDomain
  if (module_info.assembly.name ==
      WStr("Datadog.Trace.ClrProfiler.Managed.Loader")) {
    Info(
        "ModuleLoadFinished: Datadog.Trace.ClrProfiler.Managed.Loader loaded "
        "into AppDomain ",
...
HRESULT STDMETHODCALLTYPE CorProfiler::JITCompilationStarted(
  ...
  if (FAILED(hr)) {
    Warn(
        "JITCompilationStarted: Call to ICorProfilerInfo4.GetFunctionInfo() "
        "failed for ",
        function_id);
    return S_OK;
  }
  ...
  // System.Web.Compilation.BuildManager.InvokePreStartInitMethods. This will be
  // the first call-site considered for the startup hook injection, which
  // correctly loads Datadog.Trace.ClrProfiler.Managed.Loader into the
  // application's own AppDomain because at this point in the code path, the
  // ApplicationImpersonationContext has been started.
  //
  // Note: This check must only run on desktop because it is possible (and the
  // default) to host ASP.NET Core in-process, so a new .NET Core runtime is
  // instantiated and run in the same w3wp.exe process
  ...
  if (valid_startup_hook_callsite &&
    ...
    if (FAILED(hr)) {
      Warn("JITCompilationStarted: Call to RunILStartupHook() failed for ",
    ...
    if (is_desktop_iis) {
      ...
      if (FAILED(hr)) {
        Warn(
            "JITCompilationStarted: Call to AddIISPreStartInitFlags() failed "
            "for ",
            module_id, " ", function_token);
        return S_OK;
      }
  ...
  hr = ProcessInsertionCalls(module_metadata, function_id, module_id,
                             function_token, caller, method_replacements);

  if (FAILED(hr)) {
    Warn("JITCompilationStarted: Call to ProcessInsertionCalls() failed for ",
         function_id, " ", module_id, " ", function_token);
    return S_OK;
  }

  // Perform method replacement calls
  hr = ProcessReplacementCalls(module_metadata, function_id, module_id,
                               function_token, caller, method_replacements);

  if (FAILED(hr)) {
    Warn("JITCompilationStarted: Call to ProcessReplacementCalls() failed for ",
         function_id, " ", module_id, " ", function_token);
    return S_OK;
  }
...
HRESULT STDMETHODCALLTYPE CorProfiler::JITInlining(FunctionID callerId,
  ...
  *pfShouldInline = true;
  ...
  if (FAILED(hr)) {
    Warn("*** JITInlining: Failed to get the function info of the calleId: ",
         calleeId);
    return S_OK;
  }
...
HRESULT STDMETHODCALLTYPE CorProfiler::GetAssemblyReferences(
  ...
  if (in_azure_app_services) {
    Debug(
        "GetAssemblyReferences skipping entire callback because this is "
        "running in Azure App Services, which isn't yet supported for this "
        "feature. AssemblyPath=",
        wszAssemblyPath);
    return S_OK;
  }
  ...
  const AssemblyReference assemblyReference =
      trace::AssemblyReference(managed_profiler_full_assembly_version);
  ...
  Debug("GetAssemblyReferences extending assembly closure for ", assembly_name,
        " to include ", asmRefInfo.szName, ". Path=", wszAssemblyPath);
  instrument_domain_neutral_assemblies = true;
...
HRESULT CorProfiler::ProcessReplacementCalls(
  ...
  if (FAILED(hr)) {
    Warn("ProcessReplacementCalls: Call to ILRewriter.Import() failed for ",
         module_id, " ", function_token);
    return hr;
  }
  ...
  if (dump_il_rewrite_enabled) {
    original_code = GetILCodes("***   IL original code for caller: ", &rewriter,
  ...
  for (auto& method_replacement : method_replacements) {
    ...
    if (method_replacement.wrapper_method.action !=
        WStr("ReplaceTargetMethod")) {
      continue;
    }
    ...
    for (ILInstr* pInstr = rewriter.GetILList()->m_pNext;
      ...
      // it to define a MethodSpec Generate a method ref token for the wrapper
      // method
      ...
      //   2) The caller is domain-neutral AND we do not want to instrument
      //   domain-neutral assemblies 3) The target instruction is a constrained
      //   virtual method call (a constrained IL instruction followed by a
      //   callvirt IL instruction)
      ...
      if (!ProfilerAssemblyIsLoadedIntoAppDomain(
        ...
        Warn(
            ...
            module_metadata->app_domain_id, " function_id=", function_id,
            " token=", function_token, " caller_name=", caller.type.name, ".",
            caller.name, "()", " target_name=", target.type.name, ".",
            target.name, "()");
        continue;
      }
      ...
      if (caller_assembly_is_domain_neutral &&
        ...
        Warn("JITCompilationStarted skipping method: Method replacement",
             ...
             " has been loaded domain-neutral so its code is being shared "
             "across AppDomains,"
      ...
      //   3) The target instruction is a constrained virtual method call (a
      //   constrained IL instruction followed by a callvirt IL instruction)
      if (pInstr->m_opcode == CEE_CALLVIRT &&
          pInstr->m_pPrev->m_opcode == CEE_CONSTRAINED) {
        Warn(
            "JITCompilationStarted skipping method: Method replacement",
            " found but the target method call is a constrained virtual method "
            "call ",
            " (a 'constrained' IL instruction followed by a 'callvirt' IL "
            "instruction).",
      ...
      // IL Modification #1: Replace original method call with a NOP, so that
      // all original
      //                     jump targets resolve correctly and we correctly
      //                     populate the stack with additional arguments
      //
      // IMPORTANT: Conditional branches may jump to the original call
      // instruction which resulted in the InvalidProgramException seen in
      // https://github.com/DataDog/dd-trace-dotnet/pull/542. To avoid this,
      // we'll do the rest of our IL modifications AFTER this instruction.
      auto original_methodcall_opcode = pInstr->m_opcode;
      pInstr->m_opcode = CEE_NOP;
      pInstr = pInstr->m_pNext;
      rewriter_wrapper.SetILPosition(pInstr);

      // IL Modification #2: Conditionally box
      // System.Threading.CancellationToken or System.ReadOnlyMemory<T>
      //                     if it is the last argument in the target method.
      //
      // System.Threading.CancellationToken:
      // If the last argument in the method signature is of the type
      // System.Threading.CancellationToken (a struct) then box it before
      // calling our integration method. This resolves
      // https://github.com/DataDog/dd-trace-dotnet/issues/662, in which we did
      // not box the System.Threading.CancellationToken object, even though the
      // wrapper method expects an object. In that issue we observed some
      // strange CLR behavior when the target method was in System.Data and the
      // environment was 32-bit .NET Framework: the CLR swapped the values of
      // the CancellationToken argument and the opCode argument. For example,
      // the VIRTCALL opCode is '0x6F' and this value would be placed at the
      // memory location assigned to the CancellationToken variable. Since we
      // treat the CancellationToken variable as an object, this '0x6F' would be
      // dereference to access the underlying object, and an invalid memory read
      // would occur and crash the application.
      //
      // System.ReadOnlyMemory<T>:
      // If the last argument in the method signature is of the type
      // System.ReadOnlyMemory<T> (a generic valuetype) then box it before
      // calling our integration method. We need this modification for
      // RabbitMQ.Client 6.x.x instrumentation that uses
      // System.ReadOnlyMemory<byte> instead of byte[] for the message body
      // parameter.
      //
      ...
      //      3a) If the type begins with `ELEMENT_TYPE_VALUETYPE`, uncompress
      //      the compressed type token that follows 3b) If the type token
      //      represents System.Threading.CancellationToken, emit a 'box
      //      <type_token>' IL instruction before calling our wrapper method
      //   4) Check for System.ReadOnlyMemory<T>
      //      4a) If the type begins with `ELEMENT_TYPE_GENERICINST` and if the
      //      next byte is `ELEMENT_TYPE_VALUETYPE`, uncompress the compressed
      //      type token that follows 4b) If the type token represents
      //      System.ReadOnlyMemory<T>, emit a 'box <type_token>' IL instruction
      //      before calling our wrapper method. The type token
      //          will be a TypeSpec representing the specific generic
      //          instantiation of System.ReadOnlyMemory<T>
      ...
      PCCOR_SIGNATURE pSigCurrent = PCCOR_SIGNATURE(
          &target.signature.data[return_type_index]);  // index to the location
                                                       // of the return type
      bool signature_read_success = true;

      // iterate until the pointer is pointing at the last argument
      for (size_t signature_types_index = 0;
           signature_types_index < argument_count; signature_types_index++) {
      ...
      if (signature_read_success && *pSigCurrent == ELEMENT_TYPE_VALUETYPE) {
        ...
        // Currently, we only expect to see `System.Threading.CancellationToken`
        // as a valuetype in this position If we expand this to a general case,
        // we would always perform the boxing regardless of type
        if (GetTypeInfo(module_metadata->metadata_import, valuetype_type_token)
                .name == WStr("System.Threading.CancellationToken")) {
      ...
      if (signature_read_success && *pSigCurrent == ELEMENT_TYPE_GENERICINST) {
        ...
        if (*pSigCurrent == ELEMENT_TYPE_VALUETYPE) {
          ...
          if (GetTypeInfo(module_metadata->metadata_import,
                          valuetype_type_token)
                      .name == WStr("System.ReadOnlyMemory`1") &&
      ...
      // IL Modification #3: Insert a non-virtual call (CALL) to the
      // instrumentation wrapper.
      //                     Always use CALL because the wrapper methods are all
      //                     static.
      rewriter_wrapper.CallMember(wrapper_method_ref, false);
      rewriter_wrapper.SetILPosition(
          pInstr->m_pPrev);  // Set ILPosition to method call

      // IL Modification #4: Push the following additional arguments on the
      // evaluation stack in the
      //                     following order, which all integration wrapper
      //                     methods expect:
      //                       1) [int32] original CALL/CALLVIRT opCode
      //                       2) [int32] mdToken for original method call
      //                       target 3) [int64] pointer to MVID
      ...
      // IL Modification #5: Conditionally emit an unbox.any instruction on the
      // return value
      //                     of the wrapper method if we return an object but
      //                     the original method call returned a valuetype or a
      //                     generic type.
      //
      // This resolves https://github.com/DataDog/dd-trace-dotnet/pull/566,
      // which raised a System.EntryPointNotFoundException. This occurred
      // because the return type of the generic method was a generic type that
      // evaluated to a value type at runtime. As a result, this caller method
      // expected an unboxed representation of the return value, even though we
      // can only return values of type object. So if we detect that the
      // expected return type is a valuetype or a generic type, issue an
      // unbox.any instruction that will unbox it.
      mdToken typeToken;
      if (method_replacement.wrapper_method.method_signature
              .ReturnTypeIsObject() &&
          ReturnTypeIsValueTypeOrGeneric(
              module_metadata->metadata_import, module_metadata->metadata_emit,
              module_metadata->assembly_emit, target.id, target.signature,
              &typeToken)) {
        if (debug_logging_enabled) {
  ...
  if (modified) {
    ...
    if (FAILED(hr)) {
      Warn(
          "ProcessReplacementCalls: Call to ILRewriter.Export() failed for "
          "ModuleID=",
          module_id, " ", function_token);
      return hr;
    }
...
HRESULT CorProfiler::ProcessInsertionCalls(
  ...
  if (FAILED(hr)) {
    Warn("ProcessInsertionCalls: Call to ILRewriter.Import() failed for ",
         module_id, " ", function_token);
    return hr;
  }
  ...
  ILInstr* lastInstr =
      rewriter.GetILList()->m_pPrev;  // Should be a 'ret' instruction
  ...
  for (auto& method_replacement : method_replacements) {
    ...
    if (method_replacement.wrapper_method.action == WStr("InsertFirst")) {
      ...
      Info("*** JITCompilationStarted() : InsertFirst inserted call to ",
           method_replacement.wrapper_method.type_name, ".",
           method_replacement.wrapper_method.method_name, "() ",
           wrapper_method_ref, " to the beginning of method", caller.type.name,
           ".", caller.name, "()");
    }
  }
  ...
  if (modified) {
    ...
    if (FAILED(hr)) {
      Warn(
          "ProcessInsertionCalls: Call to ILRewriter.Export() failed for "
          "ModuleID=",
          module_id, " ", function_token);
      return hr;
    }
...
bool CorProfiler::GetWrapperMethodRef(
    ModuleMetadata* module_metadata, ModuleID module_id,
    const MethodReplacement& method_replacement,
    mdMemberRef& wrapper_method_ref, mdTypeRef& wrapper_type_ref) {
  const auto& wrapper_method_key =
      method_replacement.wrapper_method.get_method_cache_key();
  ...
  if (!module_metadata->TryGetWrapperMemberRef(wrapper_method_key,
    ...
    if (FAILED(hr)) {
      Warn(
          ...
          "assembly=",
          method_replacement.wrapper_method.assembly.name, ", Version=",
          method_replacement.wrapper_method.assembly.version.str(),
          ", Culture=", method_replacement.wrapper_method.assembly.locale,
          " PublicKeyToken=",
          method_replacement.wrapper_method.assembly.public_key.str());
      return false;
    }
...
std::string CorProfiler::GetILCodes(const std::string& title,
  ...
  if (localVarSig != mdTokenNil) {
    auto hr = module_metadata->metadata_import->GetSigFromToken(
  ...
  for (ILInstr* cInstr = rewriter->GetILList()->m_pNext;
    ...
    if (cInstr->m_pTarget != NULL) {
      ...
      if (cInstr->m_opcode == CEE_CALL || cInstr->m_opcode == CEE_CALLVIRT ||
          cInstr->m_opcode == CEE_NEWOBJ) {
        const auto memberInfo = GetFunctionInfo(
            module_metadata->metadata_import, (mdMemberRef)cInstr->m_Arg32);
        orig_sstream << "  | ";
      ...
      } else if (cInstr->m_opcode == CEE_CASTCLASS ||
                 cInstr->m_opcode == CEE_BOX ||
                 cInstr->m_opcode == CEE_UNBOX_ANY ||
                 cInstr->m_opcode == CEE_NEWARR ||
...
HRESULT CorProfiler::GenerateVoidILStartupMethod(
  ...
  hr = metadata_emit->DefineTypeRefByName(mscorlib_ref, WStr("System.Object"),
                                          &object_type_ref);
  if (FAILED(hr)) {
  ...
  hr = metadata_emit->DefineTypeDef(WStr("__DDVoidMethodType__"),
  ...
  hr = metadata_emit->DefineMethod(new_type_def, WStr("__DDVoidMethodCall__"),
                                   mdStatic, initialize_signature,
                                   sizeof(initialize_signature), 0, 0,
                                   ret_method_token);
  if (FAILED(hr)) {
  ...
  hr = metadata_emit->DefineTypeRefByName(mscorlib_ref,
                                          WStr("System.Threading.Interlocked"),
  ...
  hr = metadata_emit->DefineMethod(
      new_type_def, WStr("GetAssemblyAndSymbolsBytes"),
  ...
  WSTRING native_profiler_file =
      GetEnvironmentValue(WStr("CORECLR_PROFILER_PATH_64"));
  Debug(
...
#else   // BIT64
  WSTRING native_profiler_file =
      GetEnvironmentValue(WStr("CORECLR_PROFILER_PATH_32"));
  Debug(
  ...
  hr = metadata_emit->DefinePinvokeMap(
      pinvoke_method_def, 0, WStr("GetAssemblyAndSymbolsBytes"), profiler_ref);
  if (FAILED(hr)) {
  ...
  hr = metadata_emit->DefineTypeRefByName(mscorlib_ref, WStr("System.Byte"),
                                          &byte_type_ref);
  if (FAILED(hr)) {
    Warn("GenerateVoidILStartupMethod: DefineTypeRefByName failed");
    return hr;
  ...
  hr = metadata_emit->DefineTypeRefByName(
      mscorlib_ref, WStr("System.Runtime.InteropServices.Marshal"),
      &marshal_type_ref);
  if (FAILED(hr)) {
    Warn("GenerateVoidILStartupMethod: DefineTypeRefByName failed");
    return hr;
  ...
  const auto appdomain_get_current_domain_signature_length =
  ...
  const auto appdomain_load_signature_length =
  ...
  hr = metadata_emit->DefineMemberRef(
      system_appdomain_type_ref, WStr("Load"), appdomain_load_signature,
...
HRESULT CorProfiler::AddIISPreStartInitFlags(const ModuleID module_id,
                                             const mdToken function_token) {
  ComPtr<IUnknown> metadata_interfaces;
  auto hr = this->info_->GetModuleMetaData(module_id, ofRead | ofWrite,
                                           IID_IMetaDataImport2,
  ...
  mdToken boolToken;
  metadata_emit->DefineTypeRefByName(mscorlib_ref, SystemBoolean, &boolToken);
  ...
  hr = metadata_emit->DefineMemberRef(
      system_appdomain_type_ref, WStr("SetData"), appdomain_set_data_signature,
  ...
  pInstr = rewriter.GetILList()
               ->m_pPrev;  // The last instruction should be a 'ret' instruction
...
CorProfiler::GetReJITParameters(ModuleID moduleId, mdMethodDef methodId,
  ...
  return rejit_handler->NotifyReJITParameters(
...
/// <param name="filtered_integrations">Filtered vector of integrations to be
/// applied</param> <returns>Number of ReJIT requests made</returns>
size_t CorProfiler::CallTarget_RequestRejitForModule(
  ...
  for (const IntegrationMethod& integration : filtered_integrations) {
    ...
    if (integration.replacement.wrapper_method.action !=
        calltarget_modification_action) {
      continue;
    }
    ...
    if (FAILED(hr)) {
      ...
      // System.Data
      Debug("Can't load the TypeDef for: ",
    ...
    while (enumIterator != enumMethods.end()) {
      ...
      if (!caller.IsValid()) {
        Warn("The caller for the methoddef: ", TokenStr(&methodDef),
             " is not valid!");
      ...
      // functionInfo in the stack, to be used later in the ReJIT process
      ...
      if (numOfArgs !=
        ...
        Debug("The caller for the methoddef: ",
      ...
      const auto methodArguments =
          functionInfo->method_signature.GetMethodArguments();
      Debug("Comparing signature for method: ",
            integration.replacement.target_method.type_name, ".",
            integration.replacement.target_method.method_name);
      for (unsigned int i = 0; i < numOfArgs; i++) {
        const auto argumentTypeName =
            methodArguments[i].GetTypeTokName(metadata_import);
  ...
  if (!vtMethodDefs.empty()) {
    this->rejit_handler->EnqueueForRejit(vtMethodDefs.size(), vtModules.data(),
                                         vtMethodDefs.data());
  }
...
/// function is triggered by the ReJIT handler) Resulting code structure:
///
/// - Add locals for TReturn (if non-void method), CallTargetState,
/// CallTargetReturn/CallTargetReturn<TReturn>, Exception
/// - Initialize locals
///
/// try
/// {
...
///       method, the value on the stack is first stored in the TReturn local.
///   }
///   catch (Exception)
///   {
...
///     TReturn local (if non-void method), CallTargetState local, and Exception
///     local
...
HRESULT CorProfiler::CallTarget_RewriterCallback(
  ...
  std::vector<FunctionMethodArgument> methodArguments =
      caller->method_signature.GetMethodArguments();
  ...
  Debug("*** CallTarget_RewriterCallback() Start: ", caller->type.name, ".",
        caller->name, "() [IsVoid=", isVoid, ", IsStatic=", isStatic,
        ", IntegrationType=", method_replacement->wrapper_method.type_name,
        ", Arguments=", numArgs, "]");

  // *** Create rewriter
  ILRewriter rewriter(this->info_, methodHandler->GetFunctionControl(),
                      module_id, function_token);
  bool modified = false;
  auto hr = rewriter.Import();
  if (FAILED(hr)) {
  ...
  callTargetTokens->ModifyLocalSigAndInitialize(
      &reWriterWrapper, caller, &callTargetStateIndex, &exceptionIndex,
  ...
  if (isStatic) {
    if (caller->type.valueType) {
      ...
      //    ldloc.s [localIndex]
      Warn(
          "*** CallTarget_RewriterCallback(): Static methods in a ValueType "
    ...
    if (caller->type.valueType) {
      if (caller->type.type_spec != mdTypeSpecNil) {
        ...
        // IMetaDataImport::GetMemberRefProps returns The parent token as
        // mdTypeDef and not as a mdTypeSpec that's because the method
        // definition is stored in the mdTypeDef The problem is that we don't
        // have the exact Spec of that generic We can't emit LoadObj or Box
        // because that would result in an invalid IL. This problem doesn't
        // occur on a class type because we can always relay in the object type.
        return E_FAIL;
      }
    }
  }
  ...
  switch (numArgs) {
    ...
    case 2: {
      IfFailRet(callTargetTokens->WriteBeginMethodWithArguments(
          &reWriterWrapper, wrapper_type_ref, &caller->type,
          &methodArguments[0], &methodArguments[1], &beginCallInstruction));
      break;
    }
    case 3: {
      IfFailRet(callTargetTokens->WriteBeginMethodWithArguments(
  ...
  ILInstr* pStateLeaveToBeginOriginalMethodInstr =
      reWriterWrapper.CreateInstr(CEE_LEAVE_S);
  ...
  ILInstr* beginMethodCatchLeaveInstr =
      reWriterWrapper.CreateInstr(CEE_LEAVE_S);
  ...
  if (!isVoid) {
    ...
    callTargetTokens->WriteCallTargetReturnGetReturnValue(
        &reWriterWrapper, callTargetReturnToken,
  ...
  if (FAILED(hr)) {
    Warn(
        "*** CallTarget_RewriterCallback(): Call to ILRewriter.Export() failed "
        "for "
        "ModuleID=",
        module_id, " ", function_token);
    return hr;
