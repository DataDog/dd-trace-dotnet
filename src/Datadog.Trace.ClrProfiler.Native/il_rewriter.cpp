...
HRESULT ILRewriter::Import() {
  ...
  local_var_sig_size = decoder.cbLocalVarSig;
  local_var_sig_token = decoder.GetLocalVarSigTok();
  ...
  if (m_fGenerateTinyHeader) {
    ...
    *pCurrent = (BYTE)(CorILMethod_TinyFormat | (codeSize << 2));
    ...
    totalSize =
        ...
        (!eh.empty()
             ? (sizeof(IMAGE_COR_ILMETHOD_SECT_FAT) +
                sizeof(IMAGE_COR_ILMETHOD_SECT_EH_CLAUSE_FAT) * eh.size())
             : 0);
    ...
    pHeader->Flags = m_flags | (!eh.empty() ? CorILMethod_MoreSects : 0) |
    ...
    pHeader->LocalVarSigTok = local_var_sig_token;
    ...
    if (eh.size() != 0) {
      ...
      for (unsigned iEH = 0; iEH < eh.size(); iEH++) {
        EHClause* pSrc = &(eh[iEH]);
...
size_t ILRewriter::AddLocalVariable(const CorElementType element_type) {
  return 0;
}