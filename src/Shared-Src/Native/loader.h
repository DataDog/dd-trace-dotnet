...
struct RuntimeInfo
{
    ...
    RuntimeInfo() :
        RuntimeType((COR_PRF_RUNTIME_TYPE) 0x0), MajorVersion(0), MinorVersion(0), BuildVersion(0), QfeVersion(0)
    {
    }

    RuntimeInfo(COR_PRF_RUNTIME_TYPE runtimeType, USHORT majorVersion, USHORT minorVersion, USHORT buildVersion,
                USHORT qfeVersion) :
...
public:
    ...
    LoaderResourceMonikerIDs(const LoaderResourceMonikerIDs& ids) :
        ...
        NetCoreApp20_Datadog_AutoInstrumentation_ManagedLoader_dll(
            ids.NetCoreApp20_Datadog_AutoInstrumentation_ManagedLoader_dll),
        Net45_Datadog_AutoInstrumentation_ManagedLoader_pdb(ids.Net45_Datadog_AutoInstrumentation_ManagedLoader_pdb),
        NetCoreApp20_Datadog_AutoInstrumentation_ManagedLoader_pdb(
            ids.NetCoreApp20_Datadog_AutoInstrumentation_ManagedLoader_pdb)
    {
    }
...
private:
    ...
    static Loader* CreateNewLoaderInstance(ICorProfilerInfo4* pCorProfilerInfo,
    ...
    Loader(ICorProfilerInfo4* pCorProfilerInfo, const std::vector<WSTRING>& assemblyStringDefaultAppDomainVector,
    ...
    HRESULT EmitLoaderInModule(const ComPtr<IMetaDataImport2> metadataImport, const ComPtr<IMetaDataEmit2> metadataEmit,
                               ModuleID moduleId, AppDomainID appDomainId, WSTRING assemblyNameString);

    HRESULT EmitDDLoadInitializationAssembliesMethod(const ModuleID moduleId, mdTypeDef typeDef, WSTRING assemblyName,
                                                     mdMethodDef* pLoaderMethodDef,
                                                     mdMemberRef* pSecuritySafeCriticalCtorMemberRef);

    HRESULT GetGetAssemblyAndSymbolsBytesMethodDef(const ComPtr<IMetaDataEmit2> metadataEmit, mdTypeDef typeDef,
                                                   mdMethodDef* pGetAssemblyAndSymbolsBytesMethodDef);

    HRESULT WriteAssembliesStringArray(ILRewriterWrapper& rewriterWrapper, const ComPtr<IMetaDataEmit2> metadataEmit,
                                       const std::vector<WSTRING>& assemblyStringVector, mdTypeRef stringTypeRef);

    HRESULT EmitModuleCCtorMethod(const ModuleID moduleId, mdTypeDef typeDef, AppDomainID appDomainId,
    ...
    inline RuntimeInfo GetRuntimeInformation()
    {
        ...
        HRESULT hr = _pCorProfilerInfo->GetRuntimeInformation(nullptr, &runtimeType, &majorVersion, &minorVersion,
                                                              &buildVersion, &qfeVersion, 0, nullptr, nullptr);
        if (FAILED(hr))
        {
            return {};
        }
...
public:
    static const DWORD LoaderProfilerEventMask =
        COR_PRF_MONITOR_CACHE_SEARCHES | COR_PRF_DISABLE_TRANSPARENCY_CHECKS_UNDER_FULL_TRUST;
    ...
    bool GetAssemblyAndSymbolsBytes(void** ppAssemblyArray, int* pAssemblySize, void** ppSymbolsArray,
