...
static Enumerator<mdMethodDef> EnumMethodsWithName(const ComPtr<IMetaDataImport2>& metadata_import,
                                                   const mdToken& parent_token, LPCWSTR method_name)
{
    return Enumerator<mdMethodDef>(
        [metadata_import, parent_token, method_name](HCORENUM* ptr, mdMethodDef arr[], ULONG max, ULONG* cnt)
            -> HRESULT { return metadata_import->EnumMethodsWithName(ptr, parent_token, method_name, arr, max, cnt); },
...
{
    Loader* newSingeltonInstance = Loader::CreateNewLoaderInstance(
        pCorProfilerInfo, logDebugCallback, logInfoCallback, logErrorCallback, resourceMonikerIDs,
        pNativeProfilerLibraryFilename, nonIISAssemblyStringDefaultAppDomainVector,
        nonIISAssemblyStringNonDefaultAppDomainVector, iisAssemblyStringDefaultAppDomainVector,
...
Loader* Loader::CreateNewLoaderInstance(ICorProfilerInfo4* pCorProfilerInfo,
                                        std::function<void(const std::string& str)> logDebugCallback,
                                        std::function<void(const std::string& str)> logInfoCallback,
                                        std::function<void(const std::string& str)> logErrorCallback,
                                        const LoaderResourceMonikerIDs& resourceMonikerIDs,
                                        ...
                                        const std::vector<WSTRING>& iisAssemblyStringNonDefaultAppDomainVector)
{
    ...
    return new Loader(
        pCorProfilerInfo, isIIS ? iisAssemblyStringDefaultAppDomainVector : nonIISAssemblyStringDefaultAppDomainVector,
        isIIS ? iisAssemblyStringNonDefaultAppDomainVector : nonIISAssemblyStringNonDefaultAppDomainVector,
        logDebugCallback, logInfoCallback, logErrorCallback, resourceMonikerIDs, pNativeProfilerLibraryFilename);
}

Loader::Loader(ICorProfilerInfo4* pCorProfilerInfo, const std::vector<WSTRING>& assemblyStringDefaultAppDomainVector,
               const std::vector<WSTRING>& assemblyStringNonDefaultAppDomainVector,
               std::function<void(const std::string& str)> logDebugCallback,
               std::function<void(const std::string& str)> logInfoCallback,
               std::function<void(const std::string& str)> logErrorCallback,
               const LoaderResourceMonikerIDs& resourceMonikerIDs, const WCHAR* pNativeProfilerLibraryFilename)
{
...
HRESULT Loader::InjectLoaderToModuleInitializer(const ModuleID moduleId)
{
    ...
    hr = this->_pCorProfilerInfo->GetAssemblyInfo(assemblyId, STRINGMAXSIZE, &assemblyNameLength, assemblyName,
                                                  &appDomainId, NULL);
    if (FAILED(hr))
    {
    ...
    {
        Debug(
            "Loader::InjectLoaderToModuleInitializer: The loader was already loaded in the AppDomain.  [AppDomainID=" +
            appDomainIdHex + "]");
        return S_OK;
    }
    ...
    for (const auto asm_name : _assembliesExclusionList)
    {
        ...
        {
            Debug("Loader::InjectLoaderToModuleInitializer: Skipping " + ToString(assemblyNameString) +
    ...
    hr = this->_pCorProfilerInfo->GetModuleMetaData(moduleId, ofRead | ofWrite, IID_IMetaDataImport2,
                                                    metadataInterfaces.GetAddressOf());
    if (FAILED(hr))
    {
        Error("Loader::InjectLoaderToModuleInitializer: failed fetching metadata interfaces for ModuleID=" +
              moduleIdHex);
        return hr;
    }
    ...
    if (assemblyNameString == WStr("mscorlib") || assemblyNameString == WStr("System.Private.CoreLib"))
    {
        ...
        hr = assemblyImport->GetAssemblyProps(_corlibMetadata.Token, &_corlibMetadata.pPublicKey,
                                              &_corlibMetadata.PublicKeyLength, &_corlibMetadata.HashAlgId, name,
                                              STRINGMAXSIZE, &name_len, &_corlibMetadata.Metadata,
        ...
        Debug("Loader::InjectLoaderToModuleInitializer: [" + ToString(_corlibMetadata.Name) + ", " +
        ...
        if (enumIterator != enumMethods.end())
        {
            ...
            hr = metadataEmit->DefineTypeDef(WStr("DD_LoaderMethodsType"), tdAbstract | tdSealed, systemObjectTypeRef,
                                             NULL, &newTypeDef);
            if (FAILED(hr))
            {
                return hr;
            ...
            hr = EmitDDLoadInitializationAssembliesMethod(moduleId, newTypeDef, assemblyNameString, &loaderMethodDef,
            ...
            if (SUCCEEDED(hr))
            {
                Info("Loader::InjectLoaderToModuleInitializer: Loader injected successfully (in " +
                     ToString(_specificTypeToInject) + "." + ToString(_specificMethodToInject) +
    ...
    //      static extern bool GetAssemblyAndSymbolsBytes(out IntPtr assemblyPtr, out int assemblySize, out IntPtr
    //      symbolsPtr, out int symbolsSize, string moduleName);
    //
    //      static <Module>() {
    ...
    //      {
    //          if (GetAssemblyAndSymbolsBytes(out var assemblyPtr, out var assemblySize, out var symbolsPtr, out var
    //          symbolsSize, "[ModuleName]"))
    //          {
...
HRESULT Loader::HandleFunctionSearch(FunctionID functionId, BOOL* pbUseCachedFunction)
{
    ...
    ComPtr<IUnknown> metadataInterfaces;
    hr = _pCorProfilerInfo->GetModuleMetaData(moduleId, ofRead | ofWrite, IID_IMetaDataImport2,
                                              metadataInterfaces.GetAddressOf());
    if (FAILED(hr))
    {
        return S_FALSE;
...
HRESULT Loader::EmitLoaderCallInMethod(ModuleID moduleId, mdMethodDef methodDef, mdMethodDef loaderMethodDef)
{
    ...
    if (FAILED(hr))
    {
        Error("Loader::InjectLoaderToModuleInitializer: Call to ILRewriter.Import() failed for ModuleID=" +
              moduleIdHex + ", methodDef=0x" + HexStr(methodDef));
        return hr;
    }
    ...
    if (FAILED(hr))
    {
        Error("Loader::InjectLoaderToModuleInitializer: Call to ILRewriter.Export() failed for ModuleID=" +
              moduleIdHex + ", methodDef=0x" + HexStr(methodDef));
    }
    return hr;
}

HRESULT Loader::EmitLoaderInModule(const ComPtr<IMetaDataImport2> metadataImport,
                                   const ComPtr<IMetaDataEmit2> metadataEmit, ModuleID moduleId,
                                   AppDomainID appDomainId, WSTRING assemblyNameString)
{
    ...
    if (FAILED(hr))
    {
        Error("Loader::EmitLoaderInModule: failed fetching " + ToString(moduleTypeName) +
              " (module type) typedef for ModuleID=" + moduleIdHex);
        return hr;
    }
    ...
    mdMethodDef loaderMethodDef;
    mdMemberRef securitySafeCriticalCtorMemberRef;
    hr = EmitDDLoadInitializationAssembliesMethod(moduleId, moduleTypeDef, assemblyNameString, &loaderMethodDef,
                                                  &securitySafeCriticalCtorMemberRef);
    if (FAILED(hr))
    {
        return hr;
    ...
    hr = metadataEmit->DefineCustomAttribute(moduleTypeDef, securitySafeCriticalCtorMemberRef, nullptr, 0,
...
HRESULT Loader::EmitDDLoadInitializationAssembliesMethod(const ModuleID moduleId, mdTypeDef typeDef,
                                                         WSTRING assemblyName, mdMethodDef* pLoaderMethodDef,
                                                         mdMemberRef* pSecuritySafeCriticalCtorMemberRef)
{
    ...
    HRESULT hr = this->_pCorProfilerInfo->GetModuleMetaData(moduleId, ofRead | ofWrite, IID_IMetaDataImport2,
                                                            metadataInterface.GetAddressOf());
    if (FAILED(hr))
    {
        Error("Loader::EmitDDLoadInitializationAssemblies: failed fetching metadata interfaces for ModuleID=" +
              moduleIdHex);
        return hr;
    }
    ...
    const ComPtr<IMetaDataAssemblyEmit> assmeblyEmit =
        metadataInterface.As<IMetaDataAssemblyEmit>(IID_IMetaDataAssemblyEmit);
    ...
    if (FAILED(hr))
    {
        Info("Loader::EmitDDLoadInitializationAssemblies: Error loading the name for the type. ModuleID=" +
             moduleIdHex + ", TypeDef=" + ToString(HexStr(typeDef)));
        return hr;
    }
    ...
    //
    WSTRING loaderMethodName = WSTRING(WStr("DD_LoadInitializationAssemblies_")) +
                               ReplaceString(assemblyName, (WSTRING) WStr("."), (WSTRING) WStr(""));
    ...
    hr = metadataImport->FindMethod(typeDef, loaderMethodName.c_str(), loaderMethodSignature,
                                    sizeof(loaderMethodSignature), pLoaderMethodDef);
    if (SUCCEEDED(hr))
    {
        Info("Loader::EmitDDLoadInitializationAssemblies: Loader was already injected in ModuleID=" + moduleIdHex +
             ", LoaderMethodName=" + ToString(loaderMethodName));
        return hr;
    }
    ...
    hr = assmeblyEmit->DefineAssemblyRef(_corlibMetadata.pPublicKey, _corlibMetadata.PublicKeyLength,
                                         _corlibMetadata.Name.c_str(), &_corlibMetadata.Metadata, NULL, 0,
                                         _corlibMetadata.Flags, &corlibAssemblyRef);
    if (FAILED(hr))
    {
    ...
    hr = metadataEmit->DefineTypeRefByName(corlibAssemblyRef, WStr("System.Reflection.Assembly"),
                                           &systemReflectionAssemblyTypeRef);
    if (FAILED(hr))
    {
        Error("Loader::EmitDDLoadInitializationAssemblies: DefineTypeRefByName failed");
        return hr;
    ...
    hr = metadataEmit->DefineTypeRefByName(corlibAssemblyRef, WStr("System.Reflection.MethodInfo"),
                                           &systemReflectionMethodInfoTypeRef);
    if (FAILED(hr))
    {
        Error("Loader::EmitDDLoadInitializationAssemblies: DefineTypeRefByName failed");
        return hr;
    ...
    hr = metadataEmit->DefineTypeRefByName(corlibAssemblyRef, WStr("System.Reflection.MethodBase"),
                                           &systemReflectionMethodBaseTypeRef);
    if (FAILED(hr))
    {
        Error("Loader::EmitDDLoadInitializationAssemblies: DefineTypeRefByName failed");
        return hr;
    ...
    hr = metadataEmit->DefineTypeRefByName(corlibAssemblyRef, WStr("System.Runtime.InteropServices.Marshal"),
                                           &systemRuntimeMarshalTypeRef);
    if (FAILED(hr))
    {
        Error("Loader::EmitDDLoadInitializationAssemblies: DefineTypeRefByName failed");
        return hr;
    ...
    hr = metadataEmit->DefineTypeRefByName(corlibAssemblyRef, WStr("System.Security.SecuritySafeCriticalAttribute"),
                                           &securitySafeCriticalAttributeTypeRef);
    if (FAILED(hr))
    {
        Error("Loader::EmitDDLoadInitializationAssemblies: DefineTypeRefByName failed");
        return hr;
    ...
    COR_SIGNATURE marshalCopySignature[] = {IMAGE_CEE_CS_CALLCONV_DEFAULT, // Calling convention
    ...
    hr = metadataEmit->DefineMemberRef(systemRuntimeMarshalTypeRef, WStr("Copy"), marshalCopySignature,
                                       sizeof(marshalCopySignature), &marshalCopyMemberRef);
    if (FAILED(hr))
    {
    ...
    ULONG systemReflectionAssemblyTypeRefCompressedTokenLength =
        CorSigCompressToken(systemReflectionAssemblyTypeRef, systemReflectionAssemblyTypeRefCompressedToken);
    ...
    memcpy(&assemblyLoadSignature[offset], systemReflectionAssemblyTypeRefCompressedToken,
    ...
    hr = metadataEmit->DefineMemberRef(systemReflectionAssemblyTypeRef, WStr("Load"), assemblyLoadSignature, offset,
    ...
    ULONG systemTypeTypeRefCompressedTokenLength =
        CorSigCompressToken(systemTypeTypeRef, systemTypeTypeRefCompressedToken);
    ...
    hr = metadataEmit->DefineMemberRef(systemReflectionAssemblyTypeRef, WStr("GetType"), assemblyGetTypeSignature,
                                       offset, &assemblyGetTypeMemberRef);
    if (FAILED(hr))
    {
        Error("Loader::EmitDDLoadInitializationAssemblies: System.Reflection.Assembly.GetType(string name, bool "
              "throwOnError) DefineMemberRef failed");
        return hr;
    }
    ...
    ULONG systemReflectionMethodInfoTypeRefCompressedTokenLength =
        CorSigCompressToken(systemReflectionMethodInfoTypeRef, systemReflectionMethodInfoTypeRefCompressedToken);
    ...
    memcpy(&typeGetMethodSignature[offset], systemReflectionMethodInfoTypeRefCompressedToken,
           systemReflectionMethodInfoTypeRefCompressedTokenLength);
    ...
    hr = metadataEmit->DefineMemberRef(systemTypeTypeRef, WStr("GetMethod"), typeGetMethodSignature, offset,
                                       &typeGetMethodMemberRef);
    if (FAILED(hr))
    {
    ...
    COR_SIGNATURE methodBaseInvokeSignature[] = {
        IMAGE_CEE_CS_CALLCONV_HASTHIS, 2, ELEMENT_TYPE_OBJECT, ELEMENT_TYPE_OBJECT, ELEMENT_TYPE_SZARRAY,
        ELEMENT_TYPE_OBJECT,
    };
    mdMemberRef methodBaseInvokeMemberRef;
    hr = metadataEmit->DefineMemberRef(systemReflectionMethodBaseTypeRef, WStr("Invoke"), methodBaseInvokeSignature, 6,
                                       &methodBaseInvokeMemberRef);
    if (FAILED(hr))
    {
        Error("Loader::EmitDDLoadInitializationAssemblies: System.Reflection.MethodBase.Invoke(object instance, "
              "object[] args) DefineMemberRef failed");
        return hr;
    }
    ...
    hr = metadataEmit->DefineMemberRef(securitySafeCriticalAttributeTypeRef, WStr(".ctor"), ctorSignature,
                                       sizeof(ctorSignature), pSecuritySafeCriticalCtorMemberRef);
    if (FAILED(hr))
    {
        Error("Loader::EmitDDLoadInitializationAssemblies: System.Security.SecuritySafeCriticalAttribute..ctor() "
    ...
    //
    const LPCWSTR managedLoaderAssemblyLoaderTypeName =
        WStr("Datadog.AutoInstrumentation.ManagedLoader.AssemblyLoader");
    mdString managedLoaderAssemblyLoaderTypeNameStringToken;
    hr = metadataEmit->DefineUserString(managedLoaderAssemblyLoaderTypeName,
                                        (ULONG) WStrLen(managedLoaderAssemblyLoaderTypeName),
                                        &managedLoaderAssemblyLoaderTypeNameStringToken);
    if (FAILED(hr))
    {
        Error("Loader::EmitDDLoadInitializationAssemblies: 'Datadog.AutoInstrumentation.ManagedLoader.AssemblyLoader' "
              "DefineUserString failed");
        return hr;
    }
    ...
    Debug("Loader::EmitDDLoadInitializationAssemblies: Creating " + ToString(typeNameString) + "." +
          ToString(loaderMethodName) + "() in ModuleID = " + moduleIdHex);

    //
    // If the loader method cannot be found we create [Type].DD_LoadInitializationAssemblies() mdMethodDef
    //
    hr = metadataEmit->DefineMethod(typeDef, loaderMethodName.c_str(), mdStatic | mdPublic, loaderMethodSignature,
                                    sizeof(loaderMethodSignature), 0, 0, pLoaderMethodDef);
    if (FAILED(hr))
    {
    ...
    hr = metadataEmit->GetTokenFromSig(loaderLocalsSignature, sizeof(loaderLocalsSignature),
                                       &loaderLocalsSignatureToken);
    if (FAILED(hr))
    {
        Error("Loader::EmitDDLoadInitializationAssemblies: Unable to generate locals signature. ModuleID=0x" +
              moduleIdHex);
        return hr;
    }
    ...
    // Step 1) Call bool GetAssemblyAndSymbolsBytes(out IntPtr assemblyPtr, out int assemblySize, out IntPtr symbolsPtr,
    // out int symbolsSize, string moduleName)
    ...
    // call bool GetAssemblyAndSymbolsBytes(out IntPtr assemblyPtr, out int assemblySize, out IntPtr symbolsPtr, out int
    // symbolsSize, string moduleName)
    ...
    // Step 7) Call instance method System.Reflection.MethodBase.Invoke(null, new object[] { new string[] { "Assembly1"
    // }, new string[] { "Assembly2" } });
    ...
    // write string array
    hr = WriteAssembliesStringArray(rewriterWrapper, metadataEmit, _assemblyStringDefaultAppDomainVector,
                                    systemStringTypeRef);
    if (FAILED(hr))
    {
        return hr;
    ...
    // write string array
    hr = WriteAssembliesStringArray(rewriterWrapper, metadataEmit, _assemblyStringNonDefaultAppDomainVector,
                                    systemStringTypeRef);
    if (FAILED(hr))
    {
        return hr;
    ...
    if (FAILED(hr))
    {
        Error("Loader::EmitDDLoadInitializationAssemblies: Call to ILRewriter.Export() failed for ModuleID=" +
              moduleIdHex);
        return hr;
    }
...
HRESULT Loader::GetGetAssemblyAndSymbolsBytesMethodDef(const ComPtr<IMetaDataEmit2> metadataEmit, mdTypeDef typeDef,
                                                       mdMethodDef* pGetAssemblyAndSymbolsBytesMethodDef)
{
    ...
    // method:
    // C++: bool GetAssemblyAndSymbolsBytes(void** pAssemblyArray, int* assemblySize, void** pSymbolsArray, int*
    // symbolsSize, string moduleName) C#: static extern void GetAssemblyAndSymbolsBytes(out IntPtr assemblyPtr, out int
    // assemblySize, out IntPtr symbolsPtr, out int symbolsSize, string moduleName)
    const LPCWSTR getAssemblyAndSymbolsBytesName = WStr("GetAssemblyAndSymbolsBytes");
    COR_SIGNATURE getAssemblyAndSymbolsBytesSignature[] = {IMAGE_CEE_CS_CALLCONV_DEFAULT, // Calling convention
    ...
    HRESULT hr =
        metadataEmit->DefineMethod(typeDef, getAssemblyAndSymbolsBytesName, mdStatic | mdPinvokeImpl | mdHideBySig,
                                   getAssemblyAndSymbolsBytesSignature, sizeof(getAssemblyAndSymbolsBytesSignature), 0,
                                   miPreserveSig, pGetAssemblyAndSymbolsBytesMethodDef);
    if (FAILED(hr))
    {
    ...
    if (FAILED(hr))
    {
        Error("Loader::GetGetAssemblyAndSymbolsBytesMethodDef: Failed! DefineModuleRef for " +
              ToString(nativeProfilerLibraryFilename));
        return hr;
    }

    hr = metadataEmit->DefinePinvokeMap(*pGetAssemblyAndSymbolsBytesMethodDef, pmCharSetUnicode,
                                        getAssemblyAndSymbolsBytesName, nativeProfilerModuleRef);
    if (FAILED(hr))
    {
...
HRESULT Loader::WriteAssembliesStringArray(ILRewriterWrapper& rewriterWrapper,
                                           const ComPtr<IMetaDataEmit2> metadataEmit,
                                           const std::vector<WSTRING>& assemblyStringVector, mdTypeRef stringTypeRef)
{
    ...
    for (ULONG i = 0; i < assemblyStringVector.size(); i++)
    {
        ...
        auto hr = metadataEmit->DefineUserString(assemblyStringVector[i].c_str(),
                                                 (ULONG) assemblyStringVector[i].size(), &stringToken);
        if (FAILED(hr))
        {
...
HRESULT Loader::EmitModuleCCtorMethod(const ModuleID moduleId, mdTypeDef typeDef, AppDomainID appDomainId,
                                      mdMethodDef loaderMethodDef)
{
    std::string moduleIdHex = "0x" + HexStr(moduleId);
    std::string appDomainIdHex = "0x" + HexStr(appDomainId);
    ...
    if (FAILED(hr))
    {
        ...
        //
        hr = metadataEmit->DefineMethod(typeDef, cctorName, mdPublic | mdStatic | mdRTSpecialName | mdSpecialName,
                                        cctorSignature, sizeof(cctorSignature), 0, 0, &cctorMethodDef);

        if (FAILED(hr))
        {
            Error("Loader::EmitModuleCCtor: Error creating .cctor for <Module> [ModuleID=" + moduleIdHex +
                  ", AppDomainID=" + appDomainIdHex + "]");
            return hr;
        }
...
bool Loader::GetAssemblyAndSymbolsBytes(void** ppAssemblyArray, int* pAssemblySize, void** ppSymbolsArray,
                                        int* pSymbolsSize, WCHAR* pModuleName)
{
    //
    // global lock
    //
    ...
    std::string trait =
        "[AppDomainId=" + appDomainIdHex + ", ThreadId=" + threadIdHex + ", ModuleName=" + ToString(moduleName) + "]";
    ...
    *ppSymbolsArray = (LPBYTE) LockResource(hResSymbols);
    ...
    Debug("Loader::GetAssemblyAndSymbolsBytes: Loaded resouces for " + trait +
          " (platform=_WIN32)."
          " *assemblySize=" +
          ToString(*pAssemblySize) +
          ","
          " *pAssemblyArray=" +
          HexStr(reinterpret_cast<std::uint64_t>(*ppAssemblyArray)) +
          ","
          " *symbolsSize=" +
          ToString(*pSymbolsSize) +
          ","
          " *pSymbolsArray=" +
          HexStr(reinterpret_cast<std::uint64_t>(*ppSymbolsArray)) + ".");

    Debug("Loader::GetAssemblyAndSymbolsBytes: resourceMonikerIDs_: "
          "_.Net45_Datadog_AutoInstrumentation_ManagedLoader_dll=" +
          ToString(_resourceMonikerIDs.Net45_Datadog_AutoInstrumentation_ManagedLoader_dll) +
          ","
          " _.Net45_Datadog_AutoInstrumentation_ManagedLoader_pdb=" +
          ToString(_resourceMonikerIDs.Net45_Datadog_AutoInstrumentation_ManagedLoader_pdb) +
          ","
          " _.NetCoreApp20_Datadog_AutoInstrumentation_ManagedLoader_dll=" +
          ToString(_resourceMonikerIDs.NetCoreApp20_Datadog_AutoInstrumentation_ManagedLoader_dll) +
          ","
          " _.NetCoreApp20_Datadog_AutoInstrumentation_ManagedLoader_pdb=" +
          ToString(_resourceMonikerIDs.NetCoreApp20_Datadog_AutoInstrumentation_ManagedLoader_pdb) + ".");
