...
Linux x64 managed unit tests now use the same generated child-pipeline architecture as Windows. The existing matrix-generator job invokes both focused NUKE targets and publishes two small configuration artifacts. `GenerateGitlabLinuxUnitTestsPipeline` reads `GetTestingFrameworks(PlatformFamily.Linux)` and emits paired glibc and musl jobs for every selected TFM: six jobs (`netcoreapp3.1`, `net9.0`, and `net10.0` on both libc variants) for a normal merge request and sixteen jobs for a thorough run. Existing `unit-tests-linux-x64:*` names remain the Debian/glibc cells; new `unit-tests-linux-musl-x64:*` cells use Alpine.

The `unit-tests-linux-x64` bridge waits for both Linux producers. Each glibc child downloads only `build-linux-tracer-x64`, while each musl child downloads only `build-linux-tracer-x64-musl`, then builds the matching Debian or Alpine `tester` image and runs `BuildManagedUnitTests RunManagedUnitTests` for one framework. Child jobs set `IncludeAllTestFrameworks=true` because the generated framework is already authoritative; without it, a thorough-only framework such as `net6.0` would be filtered out again inside NUKE and could produce a false-green job with no tests. The same guard is applied to Windows child jobs. Building the tester image in every consumer is intentionally temporary; a content-addressed prebuilt test image should replace it after the job behavior is validated. The first Alpine producer and default generated musl matrix are green; a thorough sixteen-job Linux run is in progress. ARM64 remains pending.

The first generated Linux unit-test pipeline was green, and its framework artifacts contained the expected ordered TRX results and tracer logs. Glibc managed tests execute in the Debian `tester` container; the glibc producer's downloaded artifact also contains native outputs previously built and tested on CentOS 7, but CentOS is not a second managed-test environment. Musl managed tests execute separately in Alpine using the Alpine producer artifact. Both Windows and Linux managed-test containers now explicitly enable the Datadog test logger and receive the GitLab pipeline, job, commit, runner, and merge-request variables consumed by `GitlabEnvironmentValues`. `CI_PROJECT_DIR` is translated to the mounted in-container checkout path, and logger diagnostics are retained under `artifacts/build_data/infra_logs`. The first telemetry-enabled Linux run proved that the logger loaded because every test assembly produced a `Datadog_TestResult_*.txt` file, but it produced no diagnostic log: the pinned `DatadogTestLogger` 0.0.55 embeds an older tracer that supports `DD_TRACE_LOG_PATH`, not `DD_TRACE_LOG_DIRECTORY`. The child jobs now create the diagnostic directory and set the compatible file-path option. Validate the next run by confirming `CIEnvironmentValues: Gitlab CI detected` in `ci-visibility.log` and that the test session, module, suite, and individual pass/fail events appear in CI Visibility with links to the correct GitLab child job.
...
The first consumer is `unit-tests-macos-amd64:net10.0`. It downloads the producer artifact and runs `BuildManagedUnitTests RunManagedUnitTests` for only `net10.0`; the NUKE-generated macOS matrix is deliberately deferred until this path is green. Both jobs install the pinned 10.0.100 SDK under the job checkout's `.dotnet` directory and use checkout-local CLI home and NuGet package directories, so the persistent shared runner is not modified globally. The unit-test job retains TRX results, tracer/CI Visibility logs, and dumps, and requests the same short-lived dd-sts API key used by the Windows and Linux unit jobs. CI still needs to validate runner prerequisites, universal native compilation, the selective artifact handoff, and dd-sts trust on macOS.
...
| `unit-tests-macos-amd64:net10.0` | Initial PoC uses only `net10.0`; expand with NUKE after validation | `macos:sonoma-amd64` | parent `build-macos-tracer-amd64` | `BuildManagedUnitTests RunManagedUnitTests --framework net10.0` |
