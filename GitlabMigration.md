...
The next migration slice adds a `build-linux-x64-glibc` producer on the `docker-in-docker:amd64` runner. It mirrors Azure's three-part build: `Clean CompileManagedLoader` in the Debian .NET 10 builder, `BuildNativeTracerHome CompileTracerNativeTests RunTracerNativeTests` in the CentOS 7 .NET 7 builder, and `BuildManagedTracerHome ExtractDebugInfoLinux ValidateNativeTracerGlibcCompatibility` back in Debian. The job publishes a selective artifact for the future managed-unit-test consumer instead of Azure's entire working directory: monitoring home, managed binaries and reference assemblies, native symbols, and test diagnostics. Alpine/musl, ARM64, profiler, packaging, and managed Linux unit-test jobs remain outside this initial producer.
