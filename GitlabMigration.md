...
The first generated Linux unit-test pipeline was green, and its framework artifacts contained the expected ordered TRX results and tracer logs. Managed tests execute in the Debian `tester` container only; the downloaded producer artifact also contains native outputs previously built and tested on CentOS 7, but CentOS is not a second managed-test environment. Both Windows and Linux managed-test containers now explicitly enable the Datadog test logger and receive the GitLab pipeline, job, commit, runner, and merge-request variables consumed by `GitlabEnvironmentValues`. `CI_PROJECT_DIR` is translated to the mounted in-container checkout path, and logger diagnostics are retained under `artifacts/build_data/infra_logs`. Validate the next run by confirming `CIEnvironmentValues: Gitlab CI detected` in those diagnostics and that the test session, module, suite, and individual pass/fail events appear in CI Visibility with links to the correct GitLab child job.
