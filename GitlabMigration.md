...
The jobs retain test results, tracer and CI Visibility logs, dumps, and snapshot output; obtain a short-lived API key through dd-sts; and run `CheckBuildLogsForErrors` after the tests. The Debian/glibc framework matrix is green; the immediate validation target is the equivalent Alpine/musl matrix. ASM remains the next matrix dimension before introducing the Docker dependency groups.

The initial Windows integration-test slice follows the same generated-child-pipeline pattern. `GenerateGitlabWindowsIntegrationTestsPipeline` uses the Windows framework selection from NUKE and emits one x64 Tracer job per selected TFM: four jobs normally (`net48`, `netcoreapp3.1`, `net9.0`, and `net10.0`) or nine during thorough testing. Each child combines the parent `build` and `build-samples-standalone` artifacts, installs an end-of-life runtime on demand when required, and runs `CompileTrimmingSamples`, `BuildIntegrationTests`, and `RunIntegrationTests` inside the content-addressed Windows build image. The first slice explicitly excludes tests requiring Docker as well as regression, IIS, Azure Functions, ASM, and x86 coverage; those remain separate follow-up dimensions. Results, CI Visibility logs, tracer logs, dumps, and snapshots are retained, and build logs are validated even when the test invocation fails.
