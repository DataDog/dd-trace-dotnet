...
Each macOS child installs the pinned 10.0.100 SDK under the checkout's `.dotnet` directory and, for older TFMs, installs only the requested x64 runtime into that same directory. Checkout-local CLI home and NuGet package directories avoid modifying the persistent shared runner globally. Every child retains TRX results, tracer/CI Visibility logs, and dumps, and requests the same short-lived dd-sts API key used by the Windows and Linux unit jobs. The generated matrix still needs CI validation, especially for legacy runtimes on Sonoma; the already-green net10 cell remains the baseline.
...
- 4 producer jobs: Windows, Linux glibc, Linux musl, and macOS amd64. Native tests run inside the relevant producers.
- 1 matrix-generator job and 3 child-pipeline bridge jobs.
- 4 Windows managed-unit-test jobs normally; 9 for thorough runs.
- 6 Linux managed-unit-test jobs normally; 16 for thorough runs.
- 3 macOS managed-unit-test jobs normally; 8 for thorough runs.
- 21 jobs normally; 41 for thorough runs. ARM64 build and test cells are not included yet.
