...
- Windows IIS runs in separate `net48` x64/x86 Tracer and ASM cells. LocalDB has a dedicated `net48` x64 cell and MSMQ runs in eligible regular Tracer cells. Chrome runs in one non-blocking `net10.0` x64 host cell with checkout-local tools because Server Core lacks its runtime dependencies.
...
- Windows Docker-dependent and regression coverage remain pending.
