...
| `perform_comprehensive_testing` | Includes minor package versions in sample, integration, debugger, and profiler test builds. Default `false`. | Implemented as a prefilled `false`/`true` variable and forwarded to the multi-version sample producer and the Linux x64 non-Docker integration jobs. Debugger and profiler integration consumers have not migrated. | Validate the expanded sample and Linux integration paths, then forward the same value when the remaining test families migrate. |
