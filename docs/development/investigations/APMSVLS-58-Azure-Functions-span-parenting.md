...
### Phase 3: HttpContext.Items Bridge ✅ COMPLETED

**Status**: ✅ Fully implemented and verified

**Final Changes Made**:

1. **Skip stale header extraction in ASP.NET Core mode** ✅
   - File: `tracer/src/Datadog.Trace/ClrProfiler/AutoInstrumentation/Azure/Functions/AzureFunctionsCommon.cs:242-252`
   - Detects ASP.NET Core integration by checking for `"HttpRequestContext"` key in `FunctionContext.Items`
   - Skips extracting trace context from gRPC message headers (which contain stale host root span context)
   - Only extracts headers in non-ASP.NET Core mode

2. **Store scope in HttpContext.Items** ✅
   - File: `tracer/src/Datadog.Trace/PlatformHelpers/AspNetCoreHttpRequestHandler.cs:159-171`
   - Added: `httpContext.Items[HttpContextActiveScopeKey] = scope;` (when in Azure Functions)
   - Stores the AspNetCore scope immediately after creation
   - Uses constant `HttpContextActiveScopeKey` = `"__Datadog.AspNetCoreHttpRequestHandler.ActiveScope"`

3. **Add Items property to IFunctionContext** ✅
   - File: `tracer/src/Datadog.Trace/ClrProfiler/AutoInstrumentation/Azure/Functions/Isolated/IFunctionContext.cs:21`
   - Added: `IDictionary<object, object>? Items { get; }`
   - Allows duck-typed access to FunctionContext.Items

4. **Retrieve scope from HttpContext.Items** ✅
   - File: `tracer/src/Datadog.Trace/ClrProfiler/AutoInstrumentation/Azure/Functions/AzureFunctionsCommon.cs:286-375`
   - New helper method: `GetAspNetCoreScope()` retrieves parent scope from HttpContext.Items bridge
   - Logic:
     1. Check if `tracer.InternalActiveScope == null` (AsyncLocal didn't flow)
     2. Call `GetAspNetCoreScope()` to try HttpContext.Items bridge
     3. Get HttpContext from `context.Items["HttpRequestContext"]` (set by `FunctionsHttpProxyingMiddleware`)
     4. Get scope from `httpContext.Items[HttpContextActiveScopeKey]`
     5. Use retrieved scope as parent if found, otherwise fall back to extracted context or create root
   - Only uses HttpContext.Items when AsyncLocal fails (maintains backward compatibility)

5. **Added comprehensive debug logging** ✅
   - Logs when skipping header extraction in ASP.NET Core mode
   - Logs when storing/retrieving scope via HttpContext.Items bridge
   - Logs parent span IDs and trace IDs for troubleshooting

**Verification**: Completed on Nov 12, 2025 with trace `14656220060439490006` showing correct span hierarchy
