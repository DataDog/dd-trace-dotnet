...
az functionapp log download --name $APP_NAME --resource-group $RESOURCE_GROUP --log-path $BEFORE_LOG
...
az functionapp log download --name $APP_NAME --resource-group $RESOURCE_GROUP --log-path $AFTER_LOG
...
# Download
az functionapp log download --name $AppName --resource-group $ResourceGroup --log-path $LogFile
