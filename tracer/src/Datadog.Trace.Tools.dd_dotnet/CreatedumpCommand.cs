...
internal class CreatedumpCommand : Command
{
    ...
    internal static bool ParseArguments(string[] arguments, out int pid, out int? signal, out int? crashThread, out IntPtr? threadContextAddress)
    {
        pid = default;
        signal = default;
        crashThread = default;
    ...
    private void Execute(InvocationContext context)
    {
        ...
        try
        {
            ...
            {
                if (ParseArguments(allArguments, out var pid, out var signal, out var crashThread, out var threadContextAddress))
                {
                    GenerateCrashReport(pid, signal, crashThread, threadContextAddress);
                }
                else
                {
    ...
    private unsafe void GenerateCrashReport(int pid, int? signal, int? crashThread, IntPtr? threadContextAddress)
    {
