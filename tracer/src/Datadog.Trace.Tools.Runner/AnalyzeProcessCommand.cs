// <copyright file="AnalyzeProcessCommand.cs" company="Datadog">
...
{
    internal class AnalyzeProcessCommand : AsyncCommand<CheckProcessSettings>
    {
        ...
        private static async Task UploadToInstrumentationTelemetry(ProcessInfo process, string checkProcessReport)
        {
            var telemetryReporter = OneShotTelemetryReporter.CreateForProcess(process);
            bool success = await telemetryReporter.UploadAsLogMessage(checkProcessReport, GetApplicationData(process)).ConfigureAwait(false);
