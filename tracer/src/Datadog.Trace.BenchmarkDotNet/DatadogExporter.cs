                ...
                */
                foreach (var report in summary.Reports)
                {
                    ...
                    if (report?.BenchmarkCase?.Descriptor is { Type: { } type } descriptor && summary.HostEnvironmentInfo is { CpuInfo.Value: { } cpuInfo } hostEnvironmentInfo)
                    {
                        ...
                        {
                            testModuleWithDuration = new TestModuleWithDuration
                            {
                                Module = testSession.CreateModule(type.Assembly.GetName().Name, "BenchmarkDotNet", hostEnvironmentInfo.BenchmarkDotNetVersion, startTime),
                        ...
                        {
                            testSuiteWithDuration = new TestSuiteWithDuration
                            {
                                Suite = testModuleWithDuration.Module.GetOrCreateSuite(type.FullName ?? "Suite", startTime),
                                Duration = TimeSpan.Zero,
                            };
                        ...
                        Console.WriteLine(testMethod.StartTime.ToUnixTimeNanoseconds());
                        ...
                        {
                            foreach (var keyValue in metrics)
                            {
                                if (keyValue.Value?.Descriptor is null)
                                {
                                    continue;
                                }

                                testMethod.SetTag($"benchmark.metrics.{keyValue.Key}.displayName", keyValue.Value.Descriptor.DisplayName);
                                testMethod.SetTag($"benchmark.metrics.{keyValue.Key}.legend", keyValue.Value.Descriptor.Legend);
                                testMethod.SetTag($"benchmark.metrics.{keyValue.Key}.unit", keyValue.Value.Descriptor.Unit);
                                testMethod.SetTag($"benchmark.metrics.{keyValue.Key}.value", keyValue.Value.Value);
                            }
                        }

                        if (report.BenchmarkCase.Config?.HasMemoryDiagnoser() == true)
                        {
                            testMethod.SetTag("benchmark.memory.gen0Collections", report.GcStats.Gen0Collections);
                            testMethod.SetTag("benchmark.memory.gen1Collections", report.GcStats.Gen1Collections);
                            testMethod.SetTag("benchmark.memory.gen2Collections", report.GcStats.Gen2Collections);
                            testMethod.SetTag("benchmark.memory.total_operations", report.GcStats.TotalOperations);
                            testMethod.SetTag("benchmark.memory.mean_bytes_allocations", report.GcStats.BytesAllocatedPerOperation);
                            testMethod.SetTag("benchmark.memory.total_bytes_allocations", report.GcStats.GetTotalAllocatedBytes(false));
                        }
