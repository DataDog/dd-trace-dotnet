...
internal sealed class ArrayBuilder<T> : IReadOnlyList<T>, IDisposable
{
...
#pragma warning restore IDE0056
    ...
    /// </summary>
    public ImmutableArray<TU> ToDowncastedImmutable<TU>()
        where TU : T
    {
        if (Count == 0)
        {
            return ImmutableArray<TU>.Empty;
        }

        var tmp = ArrayBuilder<TU>.GetInstance(Count);
    ...
    public void AddRange<TU>(ArrayBuilder<TU> items)
        where TU : T
    {
        _builder.AddRange(items._builder);
    }
    ...
    public void AddRange<TS>(ImmutableArray<TS> items)
        where TS : class, T
    {
    ...
    public ImmutableArray<TS> SelectDistinct<TS>(Func<T, TS> selector)
    {
        using var result = ArrayBuilder<TS>.GetInstance(Count);
        using var set = PooledHashSet<TS>.GetInstance();
    ...
    internal Dictionary<TK, ImmutableArray<T>> ToDictionary<TK>(Func<T, TK> keySelector, IEqualityComparer<TK>? comparer = null)
        where TK : notnull
    {
        if (this.Count == 1)
        {
            var dictionary1 = new Dictionary<TK, ImmutableArray<T>>(1, comparer);
            T value = this[0];
        ...
        if (this.Count == 0)
        {
            return new Dictionary<TK, ImmutableArray<T>>(comparer);
        }

        // bucketize
        // prevent reallocation. it may not have 'count' entries, but it won't have more.
        var accumulator = new Dictionary<TK, ArrayBuilder<T>>(Count, comparer);
        for (int i = 0; i < Count; i++)
        {
            var item = this[i];
        ...
        var dictionary = new Dictionary<TK, ImmutableArray<T>>(accumulator.Count, comparer);
