...
namespace Datadog.InstrumentedAssemblyGenerator
{
    ...
    internal class ProfilerMetadataImporter
    {
        ...
        private readonly ModuleDefMD _originalModule;

        public ProfilerMetadataImporter(InstrumentedAssemblyGeneratorContext instrumentedAssemblyGeneratorContext, ModuleDefMD originalModule)
        {
            _instrumentedAssemblyGeneratorContext = instrumentedAssemblyGeneratorContext;
            _originalModule = originalModule;
        }

        internal ModuleDefMD Import()
        {
        ...
        {
            if (!_instrumentedAssemblyGeneratorContext.InstrumentedModulesTypesTokens.TryGetValue((_originalModule.FullName, _originalModule.Mvid), out var module))
            {
            ...
            foreach (var tokensAndName in module.TokensAndNames)
            {
                switch (tokensAndName.Key.Table)
                {
                    case MetadataTable.TypeDef:
                    {
                        ...
                        _originalModule.AddAsNonNestedType(newTypeDef);
                        break;
                    }
                    ...
                    case MetadataTable.ModuleRef:
                    {
                        try
                        {
                            _originalModule.Assembly.Modules.Add(ModuleDefMD.Load(tokensAndName.Value.FullName));
                        }
                    ...
                    {
                        var typeToAddField = _originalModule.Types.SingleOrDefault(t =>
                        ...
                        typeToAddField?.Fields.Add(
                            new FieldDefUser(tokensAndName.Value.MethodOrField,
                                             new FieldSig(tokensAndName.Value.ReturnTypeSig.GetTypeSig(_originalModule, _instrumentedAssemblyGeneratorContext, _importedTypes)),
                    ...
                    case MetadataTable.Method:
                    {
                        ...
                        var instrumentedMethods =
                            _instrumentedAssemblyGeneratorContext.InstrumentedMethodsByModule[(_originalModule.Name, _originalModule.Mvid)];

                        InstrumentedMethod instrumentedMethodInfo =
                            instrumentedMethods.SingleOrDefault(rm =>
                                                rm.ModuleName.Equals(_originalModule.Name, StringComparison.InvariantCultureIgnoreCase) &&
                        ...
                        var parameters = tokensAndName.Value.Parameters.Select(p => p.GetTypeSig(_originalModule, _instrumentedAssemblyGeneratorContext, _importedTypes)).ToArray();
                            ...
                            method = new MethodDefUser(tokensAndName.Value.MethodOrField,
                                                           instrumentedMethodInfo.IsStatic
                                                               ? MethodSig.CreateStatic(
                                                                   instrumentedMethodInfo.ReturnType.GetTypeSig(_originalModule, _instrumentedAssemblyGeneratorContext, _importedTypes), parameters)
                        ...
                        else
                        {
                            ...
                            method = new MethodDefUser(tokensAndName.Value.MethodOrField,
                                                       MethodSig.CreateStatic(
                                                           tokensAndName.Value.ReturnTypeSig?.GetTypeSig(_originalModule, _instrumentedAssemblyGeneratorContext, _importedTypes) ?? _originalModule.CorLibTypes.Void, parameters),
                                                       MethodAttributes.PinvokeImpl);
                        }

                        _originalModule.Types.SingleOrDefault(t =>
                    ...
                    case MetadataTable.MemberRef:
                    {
                        ...
                        if (existingMethod != null)
                        {
                            ...
                            var newMemberRef = _originalModule.Import(memberRef);
                    ...
                    case MetadataTable.MethodSpec:
                    {
                        ...
                        if (existingMethod != null)
                        {
                            var genericInstMethodSig = tokensAndName.Value.MethodGenericParameters.Select(p => p.GetTypeSig(_originalModule, _instrumentedAssemblyGeneratorContext, _importedTypes)).ToList();
                            var methodSpec = new MethodSpecUser(existingMethod, new GenericInstMethodSig(genericInstMethodSig));
                            var newMethodSpec = _originalModule.Import(methodSpec);
        ...
        private void ImportAssemblyReferences()
        {
            foreach (string assemblyName in _assembliesToAddReference)
            {
                foreach (KeyValuePair<string, ITypeDefOrRef> importedType in _importedTypes)
                {
                    ...
                    {
                        var assemblyRef = _originalModule.UpdateRowId(new AssemblyRefUser(importedType.Value.DefinitionAssembly));
        ...
        {
            return tokensAndName.Value.Parameters.Select(p => p.GetTypeSig(_originalModule, _instrumentedAssemblyGeneratorContext, _importedTypes)).ToList();
        }

        private TypeSpec ImportTypeSpec(KeyValuePair<Token, MetadataMember> tokensAndName)
        {
            var spec = new TypeSpecUser(tokensAndName.Value.TypeSig.GetTypeSig(_originalModule, _instrumentedAssemblyGeneratorContext, _importedTypes));
            TypeSpec newTypeSpec = _originalModule.Import(spec);
            return newTypeSpec;
        }
        ...
        private ITypeDefOrRef ImportTypeRef(string typeName)
        {
            ...
            if (moduleToImportFrom.Any())
            {
                ...
                {
                    return _originalModule.Import(typeDefToImport);
                }
            }

            var corLibType = _originalModule.CorLibTypes.GetAllTypes()
                                           .SingleOrDefault(t => typeName.EndsWith(t.name, StringComparison.InvariantCultureIgnoreCase));
            if (corLibType.sig != null)
            {
                return _originalModule.Import(corLibType.sig).ToTypeDefOrRef();
            }
            ...
            if (typeToImport != null)
            {
                return _originalModule.Import(typeToImport);
            }
        ...
        public void EnforceUsage()
        {
            ...
            IMethod getTypeFromHandleRef = _originalModule.Import(_instrumentedAssemblyGeneratorContext.ResolveType(ImportTypeRef(typeof(Type).FullName)).type.Methods.FirstOrDefault(m => m.Name == "GetTypeFromHandle"));
            IMethod getMethodFromHandleRef = _originalModule.Import(_instrumentedAssemblyGeneratorContext.ResolveType(ImportTypeRef(typeof(MethodBase).FullName)).type.Methods.FirstOrDefault(m => m.Name == "GetMethodFromHandle" && m.Parameters.Count == 1 && m.Parameters.First().Type.TypeName == "RuntimeMethodHandle"));
            ...
            {
                var instrumentedMethods = _instrumentedAssemblyGeneratorContext.InstrumentedMethodsByModule[(_originalModule.Name, _originalModule.Mvid)];
                ...
                var allMethods = _originalModule.Types.SelectMany(t => t.Methods).ToList();
