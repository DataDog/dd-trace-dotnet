...
namespace Datadog.InstrumentedAssemblyGenerator
{
    internal class InstrumentedAssemblyGenerator
    {
        ...
        private ModuleDefMD[] GetOriginalModulesMetadata(ILookup<(string ModuleName, Guid? Mvid), InstrumentedMethod> instrumentedMethodsByModule, List<string> originalModulesPaths)
        {
            ...
            {
                throw new InvalidOperationException("Can't find all the original module to modify. " +
                                                    "Verify that it is from same compilation as it exported in '.instrlog' file");
            }
        ...
        internal InstrumentedAssemblyGenerationResult ModifyMethods()
        {
            ...
            InstrumentedAssemblyGenerationResult result = InstrumentedAssemblyGenerationResult.Succeeded;
            foreach (IGrouping<(string ModuleName, Guid? Mvid), InstrumentedMethod> methods in ModuleRewriteContext.InstrumentedMethodsByModule)
            {
                try
                {
                    ...
                    string moduleFullName = Path.ChangeExtension(module.FullName, ".dll");
                    string exportPath = Path.Combine(_args.InstrumentedAssembliesFolder, moduleFullName);
        ...
        {
            var metadataImporter = new ProfilerMetadataImporter(ModuleRewriteContext, originalModule);
            return metadataImporter.Import();
        }
