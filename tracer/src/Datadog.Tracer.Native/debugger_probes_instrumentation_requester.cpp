...
void DebuggerProbesInstrumentationRequester::ModuleLoadFinished_AddMetadataToModule(const ModuleID moduleId)
{
    ...
    Logger::Info("[DebuggerProbesInstrumentationRequester:TOREMOVE] Requesting Rejit for Module: ", moduleInfo.assembly.name);

    ComPtr<IUnknown> metadataInterfaces;

    Logger::Info("[DebuggerProbesInstrumentationRequester:TOREMOVE]  Loading Assembly Metadata...");
    auto hr = corProfilerInfo->GetModuleMetaData(moduleInfo.id, ofRead | ofWrite, IID_IMetaDataImport2,
                                                 metadataInterfaces.GetAddressOf());
    if (hr != S_OK)
    {
        Logger::Warn(
            "[DebuggerProbesInstrumentationRequester:TOREMOVE]DebuggerProbesInstrumentationRequester::sAddMetadataToModule failed to get metadata interface for ",
    ...
    Logger::Info("[DebuggerProbesInstrumentationRequester:TOREMOVE]  Assembly Metadata loaded for: ", assemblyMetadata->name, "(", assemblyMetadata->version.str(),
                  ").");
    ...
    for (; typeDefIterator != typeDefEnum.end(); typeDefIterator = ++typeDefIterator)
    {
        ...
        if (typeInfo.name == WStr("<Main>d__0") && moduleInfo.assembly.name == WStr("Samples.TraceAnnotations.VersionMismatch.AfterFeature"))
        {
            Logger::Info("skipping IsFirstEntry for main. [ModuleId=",
                         moduleInfo.id, ", Assembly=", moduleInfo.assembly.name, ", Type=", typeInfo.name,
                         ", IsValueType=", typeInfo.valueType, "]");
            continue;
        }
        ...
        Logger::Info("[DebuggerProbesInstrumentationRequester:TOREMOVE] Added IsFirstEntry field [ModuleId=", moduleInfo.id, ", Assembly=", moduleInfo.assembly.name,
