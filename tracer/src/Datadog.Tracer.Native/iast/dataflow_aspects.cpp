...
{
    if (subject == WStr("AspectCtorReplace"))
    {
        return AspectBehavior::CtorReplace;
    }
    if (subject == WStr("AspectMethodReplace"))
    {
        return AspectBehavior::MethodReplace;
    }
    if (subject == WStr("AspectMethodInsertAfter"))
    {
        return AspectBehavior::InsertAfter;
    }
    if (subject == WStr("AspectMethodInsertBefore"))
    {
        return AspectBehavior::InsertBefore;
    }
...
{
    if (filter == WStr("StringOptimization"))
    {
        return DataflowAspectFilterValue::StringOptimization;
    }
    if (filter == WStr("StringLiterals"))
    {
        return DataflowAspectFilterValue::StringLiterals;
    }
    if (filter == WStr("StringLiterals_Any"))
    {
        return DataflowAspectFilterValue::StringLiterals_Any;
    }
    if (filter == WStr("StringLiteral_0"))
    {
        return DataflowAspectFilterValue::StringLiteral_0;
    }
    if (filter == WStr("StringLiteral_1"))
    {
        return DataflowAspectFilterValue::StringLiteral_1;
    }
...
DataflowAspectClass::DataflowAspectClass(Dataflow* dataflow, const WSTRING& aspectsAssembly, const WSTRING& line)
{
    ...
    //[AspectClassAttribute("mscorlib,netstandard,System.Private.CoreLib",PROPAGATION,"")]
    //Hdiv.AST.Aspects.Aspects.System_StringAspect
...
{
    //[AspectMethodReplaceAttribute("System.String::Concat(System.String,System.String)","",[0,1],[False,True],DEFAULT,"")]
    //100663375|100663444
    ...
    if ((int) parts.size() > ++part) // Param shift
    {
        ...
        if (_paramShift.size() == 0)
        {
            _paramShift.push_back(0);
        }
    }
...
{
    if (Contains(_targetMethodAssemblies, module->_name))
    {
        return true;
    }
    if (_aspectClass)
    {
        return _aspectClass->IsTargetModule(module);
    }
    return false;
}
...
{
    if (_vulnerabilityTypes.size() > 0)
    {
        return _vulnerabilityTypes;
    }
...
DataflowAspectReference::DataflowAspectReference(ModuleAspects* moduleAspects, DataflowAspect* aspect,
                                                 mdMemberRef method, mdTypeRef type,
                                                 const std::vector<mdTypeRef>& paramType)
{
    ...
    if (this->_targetParamTypeToken.size() == 0)
    {
        this->_targetParamTypeToken.push_back(0);
    }
...
void DataflowAspectReference::ResolveAspect()
{
    if (_aspectMemberRef == 0)
    {
        // Import aspect
        _aspectMemberRef =
            _module->DefineMemberRef(_aspect->_aspectClass->_aspectsAssembly, _aspect->_aspectClass->_aspectTypeName,
...
{
    static bool aspectFilterEnabled =
        true; // HdivConfig::Instance.GetEnabled("hdiv.net.ast.profiler.aspect.filter.enabled"_W, true);
...
InstrumentResult DataflowAspectReference::Apply(ILRewriter* processor, ILInstr* instruction)
{
    ...
    if (IsTargetMethod(operand))
    {
        ...
        else if (_aspect->IsVirtual())
        {
            ...
            process = false;
            auto thisInstructions =
                processor->StackAnalysis()->LocateCallParamInstructions(instruction, 0); // Get first param (this)
            ...
            for (unsigned int x = 0; x < _aspect->_paramShift.size(); x++)
            {
                ...
                else if (!IsReplace(_aspect->_behavior) && _aspect->_paramShift[x] > 0)
                {
                    ...
                    for (auto iInfo : processor->StackAnalysis()->LocateCallParamInstructions(
                             instruction, paramCount - _aspect->_paramShift[x] - 1)) // Locate param load instruction
                    {
                        instructionsToProcess.push_back(InstructionProcessInfo(
                            iInfo->_instruction, x,
                            AspectBehavior::InsertAfter)); // Insert after the target param load always
                    }
        ...
        for (auto instructionToProcess : instructionsToProcess)
        {
            ...
            else if (instructionToProcess.behavior == AspectBehavior::InsertAfter)
            {
                ...
                if (_aspect->_boxParam[instructionToProcess.paramIndex])
                {
                    ...
                    inserted =
                        processor->InsertAfter(aspectInstruction, processor->NewILInstr(CEE_UNBOX_ANY, paramType));
                }
