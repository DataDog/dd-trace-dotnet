...
internal sealed class ManagedTraceSampler : ITraceSampler
{
    ...
    public ManagedTraceSampler(TracerSettings settings)
    {
        ...
        settings.Manager.SubscribeToChanges(changes =>
        {
              ...
              || AreDifferent(updated.GlobalSamplingRate, changes.PreviousMutable.GlobalSamplingRate)))
            {
                ...
                var rates = Volatile.Read(ref _defaultSampleRates);
                if (rates is not null)
                {
                    newSampler.SetDefaultSampleRates(rates);
                }

                Interlocked.Exchange(ref _current, newSampler);

                // There's a small edge case here where the agent rates are being updated at the same time as we're
                // updating the sampler, so check and just update them again if necessary
                if (Volatile.Read(ref _defaultSampleRates) is { } rates2 && !ReferenceEquals(rates, rates2))
                {
                    newSampler.SetDefaultSampleRates(rates2);
                }
            }
        });
    ...
    public void SetDefaultSampleRates(IReadOnlyDictionary<string, float> sampleRates)
    {
        ...
        Interlocked.Exchange(ref _defaultSampleRates, sampleRates);
        Volatile.Read(ref _current).SetDefaultSampleRates(sampleRates);
    }
