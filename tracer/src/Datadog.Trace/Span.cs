...
namespace Datadog.Trace
{
    ...
    internal partial class Span : ISpan, ISpanContext
    {
        ...
        internal ISpan SetTag(string key, string value)
        {
            ...
            switch (key)
            {
                case Trace.Tags.Env:
                    ...
                    TraceContext.Environment = value;
                    break;
                case Trace.Tags.Version:
                    ...
                    TraceContext.ServiceVersion = value;
                    break;
                ...
                case Trace.Tags.SamplingPriority:
                    ...
                    {
                        TraceContext.SetSamplingPriority(samplingPriorityInt32, SamplingMechanism.Manual);
                    }
                    else if (Enum.TryParse<SamplingPriority>(value, out var samplingPriorityEnum))
                    {
                        TraceContext.SetSamplingPriority((int?)samplingPriorityEnum, SamplingMechanism.Manual);
                    }
        ...
        internal string GetTag(string key)
        {
            ...
            {
                case Trace.Tags.SamplingPriority:
                    return TraceContext?.SamplingPriority?.ToString();
                case Trace.Tags.Env:
                    return TraceContext?.Environment;
                case Trace.Tags.Version:
                    return TraceContext?.ServiceVersion;
                case Trace.Tags.Origin:
