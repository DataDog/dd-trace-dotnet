            ...
            for (int i = 0; i < evaluationResult.Decorations.Length; i++)
            {
                ...
                switch (ProbeInfo.TargetSpan)
                {
                    case TargetSpan.Root:
                        TracerInternal.Instance.ScopeManager.Active.Root.Span.SetTag(decoration.TagName, decoration.Value);
                        TracerInternal.Instance.ScopeManager.Active.Root.Span.SetTag(probeIdTag, ProbeInfo.ProbeId);
                        if (decoration.Errors?.Length > 0)
                        {
                            TracerInternal.Instance.ScopeManager.Active.Root.Span.SetTag(evaluationErrorTag, string.Join(";", decoration.Errors));
                        }
                        else if (TracerInternal.Instance.ScopeManager.Active.Span.GetTag(evaluationErrorTag) != null)
                        {
                            TracerInternal.Instance.ScopeManager.Active.Root.Span.SetTag(evaluationErrorTag, null);
                        }
                    ...
                    case TargetSpan.Active:
                        TracerInternal.Instance.ScopeManager.Active.Span.SetTag(decoration.TagName, decoration.Value);
                        TracerInternal.Instance.ScopeManager.Active.Span.SetTag(probeIdTag, ProbeInfo.ProbeId);
                        if (decoration.Errors?.Length > 0)
                        {
                            TracerInternal.Instance.ScopeManager.Active.Span.SetTag(evaluationErrorTag, string.Join(";", decoration.Errors));
                        }
                        else if (TracerInternal.Instance.ScopeManager.Active.Span.GetTag(evaluationErrorTag) != null)
                        {
                            TracerInternal.Instance.ScopeManager.Active.Span.SetTag(evaluationErrorTag, null);
                        }
