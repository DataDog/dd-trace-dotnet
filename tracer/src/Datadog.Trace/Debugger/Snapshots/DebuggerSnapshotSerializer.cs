...
namespace Datadog.Trace.Debugger.Snapshots
{
    internal static partial class DebuggerSnapshotSerializer
    {
            ...
            MaxInfo maxInfo)
        {
            ...
            SerializeInternal(source, type, jsonWriter, cts, currentDepth: 0, name, fieldsOnly: false, maxInfo);
        }

        public static void SerializeStaticFields(Type declaringType, JsonTextWriter jsonWriter, MaxInfo maxInfo)
        {
            using var cts = CreateCancellationTimeout();
            WriteFields(null, declaringType, jsonWriter, cts, currentDepth: 0, writeStaticFields: true, maxInfo);
        }
            ...
            MaxInfo maxInfo)
        {
            try
            {
                                                         ...
                                                         Redaction.IsSupportedDictionary(source)))
                {
                    ...
                    jsonWriter.WriteStartObject();
                    SerializeEnumerable(source, type, jsonWriter, enumerable, currentDepth, cts, maxInfo);
                    jsonWriter.WriteEndObject();
            ...
            MaxInfo maxInfo)
        {
            if (!fieldsOnly)
            {
                WriteTypeAndValue(source, type, jsonWriter, variableName, maxInfo);
            }

            try
            {
                SerializeInstanceFieldsInternal(source, type, jsonWriter, cts, currentDepth, maxInfo);
                if (!fieldsOnly)
                {
                    jsonWriter.WriteEndObject();
                }
            ...
            MaxInfo maxInfo)
        {
            ...
            else if (Redaction.IsSafeToCallToString(type))
            {
                ...
                var stringValueTruncated = stringValue?.Length < maxInfo.MaxLength ? stringValue : stringValue?.Substring(0, maxInfo.MaxLength);
            ...
            MaxInfo maxInfo)
        {
            ...
            if (currentDepth >= maxInfo.MaxReferenceDepth)
            {
                WriteNotCapturedReason(jsonWriter, NotCapturedReason.depth);
                return;
            }

            WriteFields(source, type, jsonWriter, cts, currentDepth, writeStaticFields: false, maxInfo);
        }

        private static void WriteFields(object source, Type type, JsonWriter jsonWriter, CancellationTokenSource cts, int currentDepth, bool writeStaticFields, MaxInfo maxInfo)
        {
            ...
            WriteFieldsInternal(source, jsonWriter, cts, currentDepth, fields.Where(f => IsStatic(f) == writeStaticFields), writeStaticFields ? "staticFields" : "fields", maxInfo);
        }
        ...
        private static void WriteFieldsInternal(object source, JsonWriter jsonWriter, CancellationTokenSource cts, int currentDepth, IEnumerable<MemberInfo> fields, string fieldsObjectName, MaxInfo maxInfo)
        {
            int index = 0;
            ...
            foreach (var field in fields)
            {
                ...
                if (index >= maxInfo.MaxFieldCount)
                {
           ...
           MaxInfo maxInfo)
        {
            try
            {
                ...
                if (source is ICollection collection)
                {
                    ...
                    while (itemIndex < maxInfo.MaxCollectionSize && enumerator.MoveNext())
                    {
                        ...
                        if (isDictionary)
                        {
                            serialized = SerializeKeyValuePair(enumerator.Current, jsonWriter, cts, currentDepth, maxInfo);
                        }
                        else
                        {
            ...
            MaxInfo maxInfo)
        {
            ...
            bool serializedKey = SerializeInternal(reflectionObject.GetValue(current, "Key"), reflectionObject.GetType("Key"), jsonWriter, cts, currentDepth, variableName: null, fieldsOnly: false, maxInfo);
            bool serializedValue = SerializeInternal(reflectionObject.GetValue(current, "Value"), reflectionObject.GetType("Value"), jsonWriter, cts, currentDepth, variableName: null, fieldsOnly: false, maxInfo);
