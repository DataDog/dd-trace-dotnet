    ...
    public record TracerSettings
    {
        ...
        internal TracerSettings(IConfigurationSource? source, IConfigurationTelemetry telemetry, OverrideErrorLog errorLog, LibDatadogAvailableResult isLibDatadogAvailable)
        {
            ...
            ExperimentalFeaturesEnabled = config
                    .WithKeys<ConfigKeyDdTraceExperimentalFeaturesEnabled>()
            ...
            IsRunningInCiVisibility = new ConfigurationBuilder(source, NullConfigurationTelemetry.Instance)
                                     .WithKeys<ConfigKeyDdInternalIsRunningInCivisibility>()
                                     .AsBool(false);
            ...
            GitMetadataEnabled = config
                                .WithKeys<ConfigKeyDdTraceGitMetadataEnabled>()
                                .AsBool(defaultValue: true);

            ApmTracingEnabled = config
                                      .WithKeys<ConfigKeyDdApmTracingEnabled>()
                                      .AsBool(defaultValue: true);

            var otelActivityListenerEnabled = config
                                             .WithKeys<ConfigKeyOtelSdkDisabled>()
                                             .AsBoolResult(
            ...
            IsActivityListenerEnabled = config
                                       .WithKeys<ConfigKeyDdTraceOtelEnabled>()
                                       .AsBoolResult()
            ...
            PeerServiceTagsEnabled = config
               .WithKeys<ConfigKeyDdTracePeerServiceDefaultsEnabled>()
               .AsBool(defaultValue: false);
            RemoveClientServiceNamesEnabled = config
               .WithKeys<ConfigKeyDdTraceRemoveIntegrationServiceNamesEnabled>()
               .AsBool(defaultValue: false);
            SpanPointersEnabled = config
               .WithKeys<ConfigKeyDdTraceAwsAddSpanPointers>()
               .AsBool(defaultValue: true);

            PeerServiceNameMappings = InitializeServiceNameMappings<ConfigKeyDdTracePeerServiceMapping>(config);

            MetadataSchemaVersion = config
                                   .WithKeys<ConfigKeyDdTraceSpanAttributeSchema>()
                                   .GetAs(
            ...
            StatsComputationInterval = config.WithKeys<ConfigKeyDdTraceStatsComputationInterval>().AsInt32(defaultValue: 10);

            var otelMetricsExporter = config
               .WithKeys<ConfigKeyOtelMetricsExporter>();
            ...
            var runtimeMetricsEnabledResult = config
                                             .WithKeys<ConfigKeyDdRuntimeMetricsEnabled>()
                                             .AsBoolResult();
            ...
            OtelMetricExportIntervalMs = config
                            .WithKeys<ConfigKeyOtelMetricExportInterval>()
                            .AsInt32(defaultValue: 10_000);

            OtelMetricExportTimeoutMs = config
                            .WithKeys<ConfigKeyOtelMetricExportTimeout>()
                            .AsInt32(defaultValue: 7_500);

            OtlpMetricsProtocol = config
                                 .WithKeys<ConfigKeyOtelExporterOtlpMetricsProtocol>()
                                 .GetAs(
            ...
            OtlpEndpoint = config
                .WithKeys<ConfigKeyOtelExporterOtlpEndpoint>()
                .GetAs(
            ...
            OtlpMetricsEndpoint = config
                .WithKeys<ConfigKeyOtelExporterOtlpMetricsEndpoint>()
                .GetAs(
            ...
            OtlpMetricsHeaders = config
                            .WithKeys<ConfigKeyOtelExporterOtlpMetricsHeaders>()
            ...
            OtlpMetricsTimeoutMs = config
                            .WithKeys<ConfigKeyOtelExporterOtlpMetricsTimeout>()
                            .AsInt32(defaultValue: 10_000);

            OtlpMetricsTemporalityPreference = config
                            .WithKeys<ConfigKeyOtelExporterOtlpMetricsTemporalityPreference>()
                            .GetAs(
            ...
            DataPipelineEnabled = config
                                  .WithKeys<ConfigKeyDdTraceDataPipelineEnabled>()
            ...
            CustomSamplingRulesFormat = config.WithKeys<ConfigKeyDdTraceSamplingRulesFormat>()
                                              .GetAs(
            ...
            SpanSamplingRules = config.WithKeys<ConfigKeyDdSpanSamplingRules>().AsString();

            TraceBufferSize = config
                             .WithKeys<ConfigKeyDdTraceBufferSize>()
            ...
            TraceBatchInterval = config
                                .WithKeys<ConfigKeyDdTraceBatchInterval>()
                                .AsInt32(defaultTraceBatchInterval);

            RouteTemplateResourceNamesEnabled = config
                                               .WithKeys<ConfigKeyDdTraceRouteTemplateResourceNamesEnabled>()
                                               .AsBool(defaultValue: true);

            ExpandRouteTemplatesEnabled = config
                                         .WithKeys<ConfigKeyDdTraceExpandRouteTemplatesEnabled>()
            ...
            AzureServiceBusBatchLinksEnabled = config
                                             .WithKeys<ConfigKeyDdTraceAzureServicebusBatchLinksEnabled>()
                                             .AsBool(defaultValue: true);

            DelayWcfInstrumentationEnabled = config
                                            .WithKeys<ConfigKeyDdTraceDelayWcfInstrumentationEnabled>()
                                            .AsBool(defaultValue: true);

            WcfWebHttpResourceNamesEnabled = config
                                            .WithKeys<ConfigKeyDdTraceWcfWebHttpResourceNamesEnabled>()
                                            .AsBool(defaultValue: true);

            WcfObfuscationEnabled = config
                                   .WithKeys<ConfigKeyDdTraceWcfResourceObfuscationEnabled>()
                                   .AsBool(defaultValue: true);

            InferredProxySpansEnabled = config
                                      .WithKeys<ConfigKeyDdTraceInferredProxyServicesEnabled>()
                                      .AsBool(defaultValue: false);

            ObfuscationQueryStringRegex = config
                                         .WithKeys<ConfigKeyDdTraceObfuscationQueryStringRegexp>()
                                         .AsString(defaultValue: TracerSettingsConstants.DefaultObfuscationQueryStringRegex);

            QueryStringReportingEnabled = config
                                         .WithKeys<ConfigKeyDdHttpServerTagQueryString>()
                                         .AsBool(defaultValue: true);

            QueryStringReportingSize = config
                                      .WithKeys<ConfigKeyDdHttpServerTagQueryStringSize>()
                                      .AsInt32(defaultValue: 5000); // 5000 being the tag value length limit

            ObfuscationQueryStringRegexTimeout = config
                                                .WithKeys<ConfigKeyDdTraceObfuscationQueryStringRegexpTimeout>()
            ...
            var otelPropagation = config
                            .WithKeys<ConfigKeyOtelPropagators>()
                            .GetAsClassResult(
            ...
            PropagationStyleInject = config
               .WithKeys<ConfigKeyDdTracePropagationStyleInject>()
               .GetAsClassResult(
                    validator: injectionValidator, // invalid individual values are rejected later
            ...
            PropagationStyleExtract = config
                                     .WithKeys<ConfigKeyDdTracePropagationStyleExtract>()
                                     .GetAsClassResult(
                                          validator: injectionValidator, // invalid individual values are rejected later
                                          converter: style => TrimSplitString(style, commaSeparator))
                                     .OverrideWith(in otelPropagation, ErrorLog, getDefaultPropagationHeaders);
            ...
            PropagationExtractFirstOnly = config
                                         .WithKeys<ConfigKeyDdTracePropagationExtractFirst>()
                                         .AsBool(false);

            PropagationBehaviorExtract = config
                                         .WithKeys<ConfigKeyDdTracePropagationBehaviorExtract>()
                                         .GetAs(
            ...
            TraceMethods = config
                          .WithKeys<ConfigKeyDdTraceMethods>()
                          .AsString(string.Empty);

            OutgoingTagPropagationHeaderMaxLength = config
                                                   .WithKeys<ConfigKeyDdTraceXDatadogTagsMaxLength>()
                                                   .AsInt32(
            ...
            IsDataStreamsLegacyHeadersEnabled = config
                                               .WithKeys<ConfigKeyDdDataStreamsLegacyHeaders>()
                                               .AsBool(!IsDataStreamsMonitoringInDefaultState);

            IsRareSamplerEnabled = config
                                  .WithKeys<ConfigKeyDdApmEnableRareSampler>()
                                  .AsBool(false);

            StatsComputationEnabled = config
                                     .WithKeys<ConfigKeyDdTraceStatsComputationEnabled>()
            ...
            var urlSubstringSkips = config
                                   .WithKeys<ConfigKeyDdTraceHttpClientExcludedUrlSubstrings>()
            ...
            DbmPropagationMode = config
                                .WithKeys<ConfigKeyDdDbmPropagationMode>()
                                .GetAs(
            ...
            RemoteConfigurationEnabled = config.WithKeys<ConfigKeyDdRemoteConfigurationEnabled>().AsBool(true);

            TraceId128BitGenerationEnabled = config
                                            .WithKeys<ConfigKeyDdTrace128BitTraceidGenerationEnabled>()
                                            .AsBool(true);
            TraceId128BitLoggingEnabled = config
                                         .WithKeys<ConfigKeyDdTrace128BitTraceidLoggingEnabled>()
                                         .AsBool(TraceId128BitGenerationEnabled);

            CommandsCollectionEnabled = config
                                       .WithKeys<ConfigKeyDdTraceCommandsCollectionEnabled>()
                                       .AsBool(false);

            BypassHttpRequestUrlCachingEnabled = config.WithKeys<ConfigKeyDdTraceBypassHttpRequestUrlCachingEnabled>()
                                                       .AsBool(false);

            InjectContextIntoStoredProceduresEnabled = config.WithKeys<ConfigKeyDdTraceInjectContextIntoStoredProceduresEnabled>()
                                                       .AsBool(false);

            var defaultDisabledAdoNetCommandTypes = new string[] { "InterceptableDbCommand", "ProfiledDbCommand" };
            var userDisabledAdoNetCommandTypes = config.WithKeys<ConfigKeyDdTraceDisabledAdonetCommandTypes>().AsString();
            ...
            OpenTelemetryMetricsEnabled = config
                                    .WithKeys<ConfigKeyDdMetricsOtelEnabled>()
                                    .AsBool(defaultValue: false);

            var enabledMeters = config.WithKeys<ConfigKeyDdMetricsOtelMeterNames>().AsString();
            ...
            var disabledActivitySources = config.WithKeys<ConfigKeyDdTraceDisabledActivitySources>().AsString();
            ...
            PartialFlushEnabled = config.WithKeys<ConfigKeyDdTracePartialFlushEnabled>().AsBool(false);
            PartialFlushMinSpans = config
                                  .WithKeys<ConfigKeyDdTracePartialFlushMinSpans>()
                                  .AsInt32(500, value => value > 0).Value;

            GraphQLErrorExtensions = TrimSplitString(
                config.WithKeys<ConfigKeyDdTraceGraphqlErrorExtensions>().AsString(),
                commaSeparator);
        ...
        internal static ReadOnlyDictionary<string, string>? InitializeServiceNameMappings<TKey>(ConfigurationBuilder config)
