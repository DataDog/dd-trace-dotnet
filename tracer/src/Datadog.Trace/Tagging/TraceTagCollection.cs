...
namespace Datadog.Trace.Internal.Tagging
{
    internal class TraceTagCollection
    {
        ...
        public TraceTagCollection(List<KeyValuePair<string, string>>? tags, string? cachedPropagationHeader)
        {
            if (tags?.Count > 0)
            {
                lock (tags)
                {
                    ...
                    {
                        if (item.Key == Trace.Internal.Tags.Propagated.DecisionMaker)
                        {
        ...
        private bool SetTag(string name, string? value, bool replaceIfExists)
        {
            ...
            if (name == Trace.Internal.Tags.Propagated.DecisionMaker)
            {
        ...
        {
            var tagValue = GetTag(Trace.Internal.Tags.Propagated.TraceIdUpper);
            ...
            if (traceId.Upper > 0)
            {
                ...
                {
                    SetTag(Trace.Internal.Tags.Propagated.TraceIdUpper, HexString.ToHexString(traceId.Upper));
                }
            }
            else if (traceId.Upper == 0 && tagValue != null)
            {
                // remove tag "_dd.p.tid" if trace id is only 64 bits
                RemoveTag(Trace.Internal.Tags.Propagated.TraceIdUpper);
            }
        }
            ...
            where TTagEnumerator : struct, ITagEnumerator
        {
            ...
            {
                tagEnumerator.Next(new KeyValuePair<string, string>(Trace.Internal.Tags.Propagated.DecisionMaker, _samplingMechanismValue));
            }
