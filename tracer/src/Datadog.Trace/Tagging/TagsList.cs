...
internal class TagsList : ITags
{
    ...
    public virtual void SetTag(string key, string value)
    {
        ...
        if (tags == null)
        {
            ...
            tags = Interlocked.CompareExchange(ref _tags, newTags, comparand: null) ?? newTags;
        }
        ...
        where TProcessor : struct, IItemProcessor<string>
    {
        ...
        if (tags is not null)
        {
            lock (tags)
            {
                for (var i = 0; i < tags.Count; i++)
                {
                    processor.Process(new TagItem<string>(tags[i].Key, tags[i].Value, serializedKey: null));
                }
            }
        }
    }
    ...
    public virtual void SetMetric(string key, double? value)
    {
        ...
        if (metrics == null)
        {
            ...
            metrics = Interlocked.CompareExchange(ref _metrics, newMetrics, comparand: null) ?? newMetrics;
        }
        ...
        where TProcessor : struct, IItemProcessor<double>
    {
        ...
        lock (metrics)
        {
            ...
            {
                var item = metrics[i];
                processor.Process(new TagItem<double>(item.Key, item.Value, serializedKey: null));
            }
        }
    }
    ...
    public virtual void SetMetaStruct(string key, byte[] value)
    {
        if (!string.IsNullOrEmpty(key))
        {
            ...
            if (metastruct == null)
            {
                ...
                metastruct = Interlocked.CompareExchange(ref _metaStruct, newMetastruct, comparand: null) ?? newMetastruct;
            }
        ...
        where TProcessor : struct, IItemProcessor<byte[]>
    {
        ...
        lock (metastruct)
        {
            for (var i = 0; i < metastruct.Count; i++)
            {
                ...
                processor.Process(new TagItem<byte[]>(item.Key, item.Value, serializedKey: null));
            }
        }
    }
