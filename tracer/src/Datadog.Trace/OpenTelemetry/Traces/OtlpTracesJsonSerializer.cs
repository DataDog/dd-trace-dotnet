...
internal sealed class OtlpTracesJsonSerializer : ISpanBufferSerializer
{
    ...
    {
        return OtlpTracesJsonSerializer.SerializeToJson(ref bytes, 0, in traceChunk, maxSize: maxSize);
    }
    ...
    internal static int SerializeToJson(ref byte[] bytes, int offset, in TraceChunkModel traceChunk, int? maxSize)
    {
