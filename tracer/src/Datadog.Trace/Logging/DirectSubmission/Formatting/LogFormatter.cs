...
namespace Datadog.Trace.Logging.DirectSubmission.Formatting
{
    internal class LogFormatter
    {
        ...
        public LogFormatter(DirectLogSubmissionSettings settings)
        {
            ...
            Func<JsonTextWriter, T, LogPropertyRenderingDetails> renderProperties)
        {
            ...
            var renderingDetails = renderProperties(writer, state);
            ...
            if (_hostname is not null && !renderingDetails.HasRenderedSource)
            {
                writer.WritePropertyName(HostPropertyName, escape: false);
                writer.WriteValue(_hostname);
            }
