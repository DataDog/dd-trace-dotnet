...
{
    internal static Obj Encode(this List<RuleData> ruleData, WafLibraryInvoker wafLibraryInvoker)
    {
        var dictionary = new Dictionary<string, object> { { "rules_data", ruleData.Select(r => r.ToKeyValuePair()).ToArray() } };
        return Encoder.Encode(dictionary, wafLibraryInvoker, new List<Obj>(), false);
    }
