    ...
    internal sealed class AspNetCoreHttpRequestHandler
    {
        ...
        public void StopAspNetCorePipelineScope(Tracer tracer, Security security, Scope rootScope, HttpContext httpContext, Scope proxyScope)
        {
            if (rootScope != null)
            {
                ...
                CopyAspNetCoreActivityTagsIfRequired(span, tracer.Settings.OpenTelemetryTraceCompatibilityEnabled);
        ...
        public void CopyAspNetCoreActivityTagsIfRequired(Span span, bool openTelemetryTraceCompatibilityEnabled)
        {
            ...
            AddActivityTags(span, rawActivity, _log, openTelemetryTraceCompatibilityEnabled);

            // Extracted to method as not invoked in default config (only when otel enabled)
            static void AddActivityTags(Span span, object rawActivity, IDatadogLogger log, bool openTelemetryTraceCompatibilityEnabled)
            {
                     ...
                     && activity5.HasTagObjects())
                    {
                        ...
                        ActivityEnumerationHelper.EnumerateTagObjects(
                            ...
                            static (ref s, kvp) =>
                            {
                                ...
                                {
                                    OtlpHelpers.SetTagObject(s.Span, kvp.Key, kvp.Value, setKnownValues: false, remapOtelKeys: !s.OpenTelemetryTraceCompatibilityEnabled);
                                }
