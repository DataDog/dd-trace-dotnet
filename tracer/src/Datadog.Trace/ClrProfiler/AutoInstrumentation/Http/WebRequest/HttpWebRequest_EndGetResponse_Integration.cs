    ...
    public class HttpWebRequest_EndGetResponse_Integration
    {
            ...
            where TTarget : IHttpWebRequest, IDuckType
        {
            if (instance.Instance is HttpWebRequest request && WebRequestCommon.IsTracingEnabled(request))
            {
                ...
                InternalSpanContext existingSpanContext = null;
                ...
                bool setSamplingPriority = existingSpanContext?.SamplingPriority != null && InternalTracer.Instance.ActiveScope == null;
                ...
                try
                {
                    ...
                    if (scope is not null)
                    {
                        ...
                        {
                            scope.Span.SetHttpStatusCode((int)response.StatusCode, isServer: false, InternalTracer.Instance.Settings);
                            scope.Dispose();
                        }
                        else if (exception is WebException { Status: WebExceptionStatus.ProtocolError, Response: HttpWebResponse exceptionResponse })
                        {
                            ...
                            scope.Span.SetHttpStatusCode((int)exceptionResponse.StatusCode, isServer: false, InternalTracer.Instance.Settings);
                            scope.Dispose();
                        }
                        else
                        {
