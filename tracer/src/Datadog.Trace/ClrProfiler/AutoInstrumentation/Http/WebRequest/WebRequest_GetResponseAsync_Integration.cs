...
namespace Datadog.Trace.ClrProfiler.AutoInstrumentation.Http.WebRequest
{
    ...
    public sealed class WebRequest_GetResponseAsync_Integration
    {
        ...
        internal static TReturn OnAsyncMethodEnd<TTarget, TReturn>(TTarget instance, TReturn returnValue, Exception exception, in CallTargetState state)
        {
            if (state.Scope != null)
            {
                ...
                {
                    state.Scope.Span.SetHttpStatusCode((int)response.StatusCode, false, tracer.CurrentTraceSettings.Settings, tracer.Settings.OpenTelemetrySemanticsEnabled);
                    state.Scope.Dispose();
                }
                else if (exception is WebException { Status: WebExceptionStatus.ProtocolError, Response: HttpWebResponse exceptionResponse })
                {
                    ...
                    state.Scope.Span.SetHttpStatusCode((int)exceptionResponse.StatusCode, false, tracer.CurrentTraceSettings.Settings, tracer.Settings.OpenTelemetrySemanticsEnabled);
