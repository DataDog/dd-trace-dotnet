...
#pragma warning restore CS8605 // Unboxing a possibly null value.
            ...
            where T : IFunctionContext
        {
            ...
            try
            {
                ...
                // EventHub uses "Properties" for single events, similar to ServiceBus "UserProperties"
                if (triggerMetadata.TryGetValue("Properties", out var singlePropsObj) == true)
                {
                    Log.Debug(logPrefix + "Found Properties in trigger metadata");
                    if (TryParseJson<Dictionary<string, object>>(singlePropsObj) is { } singleProps)
                    {
                        if (ExtractSpanContextFromProperties(singleProps) is { } singleContext)
                        {
                            spanContexts.Add(singleContext);
                            Log.Debug(logPrefix + "Extracted context from single event: TraceId={0}", singleContext.TraceId128.ToString());
                        }
                    }
                    else
                    {
                        Log.Warning(logPrefix + "Failed to parse Properties as Dictionary");
                    }
                }

                // EventHub uses "PropertiesArray" for batch events
                if (triggerMetadata.TryGetValue("PropertiesArray", out var arrayPropsObj) == true)
                {
                    Log.Debug(logPrefix + "Found PropertiesArray in trigger metadata");
                    if (TryParseJson<Dictionary<string, object>[]>(arrayPropsObj) is { } propsArray)
                    {
                        foreach (var props in propsArray)
                        {
                            ...
                            {
                                spanContexts.Add(batchContext);
                                Log.Debug(logPrefix + "Extracted context from batch event: TraceId={0}", batchContext.TraceId128.ToString());
                            }
                        }
                    }
                    else
                    {
                        Log.Warning(logPrefix + "Failed to parse PropertiesArray as Dictionary array");
                    }
                }

                if (spanContexts.Count == 0)
                {
                    // Log available keys to help debug if structure is different
                    var availableKeys = string.Join(", ", triggerMetadata.Keys);
                    Log.Warning(logPrefix + "No trace context found in EventHub trigger metadata. Available keys: {Keys}", availableKeys);
                    return default;
                }
