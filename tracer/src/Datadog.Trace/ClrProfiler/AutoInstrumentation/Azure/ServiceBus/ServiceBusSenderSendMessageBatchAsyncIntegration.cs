...
namespace Datadog.Trace.ClrProfiler.AutoInstrumentation.Azure.ServiceBus
{
    ...
    public class ServiceBusSenderSendMessageBatchAsyncIntegration
    {
        ...
        private static readonly IDatadogLogger Log = DatadogLogging.GetLoggerFor(typeof(ServiceBusSenderSendMessageBatchAsyncIntegration));

        internal static CallTargetState OnMethodBegin<TTarget, TBatch>(TTarget instance, ref TBatch messageBatch, ref CancellationToken cancellationToken)
            ...
            where TBatch : IServiceBusMessageBatch, IDuckType
        {
            ...
            // Store sender information for individual message spans
            if (messageBatch?.Instance != null)
            {
                ServiceBusBatchSenderContext.StoreSenderContext(messageBatch.Instance, instance);
            }

            // Get span links from batch message contexts
            IEnumerable<SpanLink>? spanLinks = null;
            if (messageBatch?.Instance != null)
            {
                var messageSpanContexts = ServiceBusBatchSpanContext.GetMessageSpanContexts(messageBatch.Instance, clear: true);
