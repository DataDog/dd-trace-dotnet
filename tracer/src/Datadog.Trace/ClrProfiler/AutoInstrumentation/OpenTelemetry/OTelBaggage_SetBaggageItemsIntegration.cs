...
namespace Datadog.Trace.ClrProfiler.AutoInstrumentation.OpenTelemetry
{
    ...
    public sealed class OTelBaggage_SetBaggageItemsIntegration
    {
        ...
        internal static CallTargetReturn<TReturn> OnMethodEnd<TTarget, TReturn>(TTarget instance, TReturn returnValue, Exception exception, in CallTargetState state)
        {
            ...
            {
                // Important: Before returning to the caller, the OpenTelemetry.Baggage API sets the to-be-returned baggage object as the current baggage using
                // private API's. To keep Datadog.Trace.Baggage.Current in-sync with OpenTelemetry.Baggage.Current, we must replace the contents of
                // Datadog.Trace.Baggage.Current with baggage items in the return value.
                //
                // Additional notes:
                ...
                // - The API can be invoked with an arbitrary OpenTelemetry.Baggage object, not just the current one, so we must replace all baggage items every time
