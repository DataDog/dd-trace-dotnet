    ...
    internal static class AwsSqsCommon
    {
        ...
        public static Scope? CreateScope(Tracer tracer, string operation, out AwsSqsTags? tags, ISpanContext? parentContext = null, string spanKind = SpanKinds.Client)
        {
            ...
            try
            {
                ...
                string? serviceNameSource = perTraceSettings.GetServiceNameSource(DatadogAwsSqsServiceName, serviceName);
        ...
        internal static string GetOperationName(Tracer tracer, string spanKind)
        {
            ...
            {
                SpanKinds.Consumer => tracer.CurrentTraceSettings.Schema.Messaging.GetInboundOperationName(MessagingSchema.OperationType.AwsSqs),
                SpanKinds.Producer => tracer.CurrentTraceSettings.Schema.Messaging.GetOutboundOperationName(MessagingSchema.OperationType.AwsSqs),
                _ => "aws.sqs.request"
            };
        }
    }
