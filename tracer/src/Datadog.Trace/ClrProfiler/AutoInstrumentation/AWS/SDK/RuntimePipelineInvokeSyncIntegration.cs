            ...
            where TResponseContext : IResponseContext, IDuckType
        {
            if (state.Scope?.Span.Tags is AwsSdkTags tags)
            {
                ...
                if (responseContext.Instance is not null && responseContext.Response is { } response)
                {
                    ...
                    state.Scope.Span.SetHttpStatusCode((int)response.HttpStatusCode, false, Tracer.Instance.CurrentTraceSettings.Settings, Tracer.Instance.Settings.OpenTelemetrySemanticsEnabled);
                }
            }
