...
namespace Datadog.Trace.ClrProfiler.AutoInstrumentation.Elasticsearch
{
    internal static class ElasticsearchNetCommon
    {
        ...
        public static Scope CreateScope<T>(Tracer tracer, IntegrationIds integrationId, RequestPipelineStruct pipeline, T requestData)
        ...
        public static Scope CreateScope(Tracer tracer, IntegrationIds integrationId, string path, string method, object requestParameters, out ElasticsearchTags tags)
        {
            if (!tracer.Settings.IsIntegrationEnabled(integrationId))
            {
                // integration disabled, don't create a scope, skip this trace
