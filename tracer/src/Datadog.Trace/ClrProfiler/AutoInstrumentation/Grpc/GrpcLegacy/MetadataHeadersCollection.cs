...
namespace Datadog.Trace.ClrProfiler.AutoInstrumentation.Grpc.GrpcLegacy
{
    internal readonly struct MetadataHeadersCollection<T> : IHeadersCollection
        where T : IMetadata
    {
        private readonly T _headers;

        public MetadataHeadersCollection(T? headers)
        {
            if (headers is null)
            {
