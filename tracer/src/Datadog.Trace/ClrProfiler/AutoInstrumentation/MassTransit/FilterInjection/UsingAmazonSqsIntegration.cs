// <copyright file="UsingAmazonSqsIntegration.cs" company="Datadog">
...
/// <summary>
/// MassTransit IBusRegistrationConfigurator.UsingAmazonSqs calltarget instrumentation
...
public sealed class UsingAmazonSqsIntegration
{
    private static readonly IDatadogLogger Log = DatadogLogging.GetLoggerFor(typeof(UsingAmazonSqsIntegration));
    ...
    {
        Log.Debug("MassTransit UsingAmazonSqsIntegration.OnMethodBegin() - Wrapping configuration callback");

        if (configure == null)
        {
            Log.Debug("MassTransit UsingAmazonSqsIntegration - Configure delegate is null, skipping");
    ...
    private readonly struct ConfigureCallbacks : IBegin2Callbacks, IVoidReturnCallback
    {
        ...
        {
            Log.Debug("MassTransit UsingAmazonSqsIntegration.ConfigureCallbacks.OnDelegateBegin() - Injecting filter into configurator");
        ...
        public void OnDelegateEnd(object? sender, Exception? exception, object? state)
        {
            if (exception != null)
            {
                Log.Warning(exception, "MassTransit UsingAmazonSqsIntegration - Original configure callback threw an exception");
            }
        }

        public void OnException(object? sender, Exception ex)
        {
            Log.Warning(ex, "MassTransit UsingAmazonSqsIntegration - Exception in filter injection");
        }
    }
}
