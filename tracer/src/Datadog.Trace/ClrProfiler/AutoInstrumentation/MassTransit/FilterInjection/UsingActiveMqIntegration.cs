// <copyright file="UsingActiveMqIntegration.cs" company="Datadog">
...
/// <summary>
/// MassTransit IBusRegistrationConfigurator.UsingActiveMq calltarget instrumentation
/// This hooks into the UsingActiveMq configuration method to inject Datadog filters during configuration time.
/// </summary>
[InstrumentMethod(
    AssemblyName = "MassTransit.ActiveMQ",
    TypeName = "MassTransit.ActiveMqRegistrationExtensions",
    MethodName = "UsingActiveMq",
    ReturnTypeName = ClrNames.Void,
    ParameterTypeNames = ["MassTransit.IBusRegistrationConfigurator", "System.Action`2[MassTransit.IBusRegistrationContext,MassTransit.IActiveMqBusFactoryConfigurator]"],
    MinimumVersion = "7.0.0",
    MaximumVersion = "7.*.*",
    IntegrationName = MassTransitConstants.IntegrationName)]
[Browsable(false)]
[EditorBrowsable(EditorBrowsableState.Never)]
public sealed class UsingActiveMqIntegration
{
    private static readonly IDatadogLogger Log = DatadogLogging.GetLoggerFor(typeof(UsingActiveMqIntegration));
    ...
    {
        Log.Debug("MassTransit UsingActiveMqIntegration.OnMethodBegin() - Wrapping configuration callback");

        if (configure == null)
        {
            Log.Debug("MassTransit UsingActiveMqIntegration - Configure delegate is null, skipping");
    ...
    private readonly struct ConfigureCallbacks : IBegin2Callbacks, IVoidReturnCallback
    {
        ...
        {
            Log.Debug("MassTransit UsingActiveMqIntegration.ConfigureCallbacks.OnDelegateBegin() - Injecting filter into configurator");
        ...
        public void OnDelegateEnd(object? sender, Exception? exception, object? state)
        {
            if (exception != null)
            {
                Log.Warning(exception, "MassTransit UsingActiveMqIntegration - Original configure callback threw an exception");
            }
        }

        public void OnException(object? sender, Exception ex)
        {
            Log.Warning(ex, "MassTransit UsingActiveMqIntegration - Exception in filter injection");
        }
    }
}
