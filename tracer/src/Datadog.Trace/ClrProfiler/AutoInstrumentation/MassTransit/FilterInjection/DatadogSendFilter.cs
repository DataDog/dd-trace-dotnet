// <copyright file="DatadogSendFilter.cs" company="Datadog">
...
/// <summary>
/// A MassTransit filter that instruments send operations for Datadog tracing.
/// This implements GreenPipes.IFilter{SendContext} for MassTransit 7.x.
///
/// The send filter creates a producer span that encompasses the entire send operation,
/// including the actual transport call (RabbitMQ, SQS, etc.). This ensures that
/// transport-level spans become children of the MassTransit send span.
/// </summary>
internal sealed class DatadogSendFilter
{
    private static readonly IDatadogLogger Log = DatadogLogging.GetLoggerFor(typeof(DatadogSendFilter));
    ...
    public void Probe(object context)
    {
        try
        {
            ...
            createScopeMethod?.Invoke(context, new object[] { "datadog-send" });
        }
        catch (Exception ex)
        {
            Log.Debug(ex, "DatadogSendFilter: Failed to create probe scope");
        }
    }

    /// <summary>
    /// Processes the send context through the filter pipeline.
    /// Creates a Datadog "send" span that encompasses the entire send operation,
    ...
    [DuckReverseMethod(ParameterTypeNames = ["MassTransit.SendContext, MassTransit", "GreenPipes.IPipe`1[MassTransit.SendContext], GreenPipes"])]
    public async Task Send(object? context, object next)
    {
        Log.Debug<string?>("DatadogSendFilter.Send() - Processing send context: {ContextType}", context?.GetType().Name);
        ...
        try
        {
            // Extract message info from the send context
            ExtractMessageInfo(context, out var messageType, out var destinationName, out var messagingSystem);

            // Create the producer/send span
            sendScope = MassTransitIntegration.CreateProducerScope(
                tracer,
            ...
            if (sendScope != null && context != null)
            {
                // Set send-specific tags from the send context
                SetSendContextTags(sendScope, context);

                // Inject trace context into headers for propagation
                InjectTraceContext(context, sendScope.Span.Context, tracer);
            }
        }
        catch (Exception ex)
        {
            Log.Debug(ex, "DatadogSendFilter: Failed to create send scope");
        }

        try
        {
            // Call next.Send(context) to continue the pipeline
            // This is where the actual transport send happens
            await InvokeNextPipe(next, context!).ConfigureAwait(false);
        }
        catch (Exception ex)
        {
            sendScope?.Span?.SetException(ex);
            throw;
        }
        finally
        {
    ...
    private static void ExtractMessageInfo(object? context, out string? messageType, out string? destinationName, out string? messagingSystem)
    {
        ...
        {
            Log.Debug(ex, "DatadogSendFilter: Failed to extract message info");
        }
    }
    ...
    private static void SetSendContextTags(Scope scope, object context)
    {
        ...
        try
        {
            if (context.TryDuckCast<ISendContext>(out var duckContext))
            {
                // Set send context tags similar to publish context tags
        ...
        {
            Log.Debug(ex, "DatadogSendFilter: Failed to set send context tags");
        }
    }
    ...
    private static void InjectTraceContext(object context, SpanContext spanContext, Tracer tracer)
    {
        try
        {
            if (context.TryDuckCast<ISendContext>(out var duckContext))
            {
                ...
                if (headersObj != null)
                {
                    ...
                    // MassTransit's DictionarySendHeaders uses explicit interface implementation for SendHeaders,
                    // so we need to get the method from the interface, not the concrete class.
                    ...
                    if (setMethod != null)
                    {
                        ...
                        Log.Debug("DatadogSendFilter: Successfully injected trace context into headers");
                    }
                    else
                    {
                        Log.Warning("DatadogSendFilter: Could not find Set method on headers type: {HeadersType}", headersType.FullName);
                    }
                }
            }
        }
        ...
        {
            Log.Debug(ex, "DatadogSendFilter: Failed to inject trace context");
        }
    }
    ...
    private static async Task InvokeNextPipe(object next, object context)
    {
        ...
        if (sendMethod == null)
        {
            foreach (var method in nextType.GetMethods(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance))
            {
                if (method.Name == "Send" || method.Name.EndsWith(".Send"))
                {
                    ...
                    {
                        sendMethod = method;
                        Log.Debug<string>("DatadogSendFilter: Found Send method: {MethodName}", method.Name);
                        break;
                    }
                }
            }
                ...
                if (iface.IsGenericType && iface.Name.StartsWith("IPipe"))
                {
                    ...
                    for (int i = 0; i < map.InterfaceMethods.Length; i++)
                    {
                        if (map.InterfaceMethods[i].Name == "Send")
                        {
                            ...
                            Log.Debug<string>("DatadogSendFilter: Found Send method via interface map: {MethodName}", sendMethod.Name);
                            break;
                        }
                    }
        ...
        if (sendMethod == null)
        {
            Log.Warning<string>("DatadogSendFilter: Could not find Send method on next pipe. Type: {NextType}", nextType.FullName ?? "unknown");
            return;
        }
    ...
    private readonly struct HeaderInjectionCarrier
    {
        ...
        private static void SetHeader(HeaderInjectionCarrier carrier, string key, string value)
        {
            ...
            {
                Log.Debug(ex, "DatadogSendFilter: Failed to set header {Key}", key);
            }
        }
    }
}
