    ...
    public sealed class AsyncControllerActionInvoker_EndInvokeAction_Integration
    {
            ...
            if (scope != null)
            {
                ...
                else
                {
                    ...
                    if (proxyScope?.Span != null)
                    {
                        ...
                        proxyScope.Span.SetHttpStatusCode(httpContext.Response.StatusCode, isServer: true, Tracer.Instance.CurrentTraceSettings.Settings, Tracer.Instance.Settings.OpenTelemetrySemanticsEnabled);
                    }
        ...
        private static void OnRequestCompletedAfterException(HttpContext httpContext, Scope scope, Scope? proxyScope, DateTimeOffset finishTime)
        {
            try
            {
                ...
                {
                    HttpContextHelper.AddHeaderTagsFromHttpResponse(httpContext, proxyScope);
                    proxyScope.Span.SetHttpStatusCode(statusCode, isServer: true, Tracer.Instance.CurrentTraceSettings.Settings, Tracer.Instance.Settings.OpenTelemetrySemanticsEnabled);
                    proxyScope.Span.Finish(finishTime);
                }

                HttpContextHelper.AddHeaderTagsFromHttpResponse(httpContext, scope);
                scope.Span.SetHttpStatusCode(statusCode, isServer: true, Tracer.Instance.CurrentTraceSettings.Settings, Tracer.Instance.Settings.OpenTelemetrySemanticsEnabled);
