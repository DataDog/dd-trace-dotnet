// <copyright file="ActivityAddTagStringIntegration.cs" company="Datadog">
...
{
    /// <summary>
    /// CallTarget instrumentation for <c>System.Diagnostics.Activity.AddTag(string, string?)</c>.
    ...
    public sealed class ActivityAddTagStringIntegration
    {
        ...
        internal static CallTargetState OnMethodBegin<TTarget, TKey, TValue>(TTarget instance, TKey key, TValue value)
        {
            ...
            if (scope is not null)
            {
                ...
                var valueStr = (string?)(object?)value;
                OtlpHelpers.AgentSetOtlpTag(scope.Span, keyStr, valueStr);
        ...
        /// to preserve chaining semantics (<c>activity.AddTag(...).AddTag(...)</c>).
        /// </summary>
        internal static CallTargetReturn<TReturn> OnMethodEnd<TTarget, TReturn>(TTarget instance, TReturn returnValue, Exception? exception, in CallTargetState state)
        {
            if (state.GetSkipMethodBody())
            {
                // returnValue is null/default because the original method body was skipped.
                // Return the instance so that method chaining continues to work.
