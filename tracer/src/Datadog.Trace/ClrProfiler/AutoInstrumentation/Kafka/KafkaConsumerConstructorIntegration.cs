...
public class KafkaConsumerConstructorIntegration
{
    ...
    {
        if (TracerInternal.Instance.Settings.IsIntegrationEnabled(KafkaConstants.IntegrationId))
        {
            string groupId = null;
            ...
            if (TracerInternal.Instance.TracerManager.DataStreamsManager.IsEnabled)
            {
