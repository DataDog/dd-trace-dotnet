...
namespace Datadog.Trace.ClrProfiler.AutoInstrumentation.Logging.ILogger
{
    internal class DatadogLoggingScope : IReadOnlyList<KeyValuePair<string, object>>
    {
        ...
        public KeyValuePair<string, object> this[int index]
        {
            get
            {
                return index switch
                {
                    ...
                    3 => new KeyValuePair<string, object>("dd_trace_id", (_tracer.InternalActiveScope?.Span.TraceId ?? 0).ToString()),
                    4 => new KeyValuePair<string, object>("dd_span_id", (_tracer.InternalActiveScope?.Span.SpanId ?? 0).ToString()),
        ...
        {
            var span = _tracer.InternalActiveScope?.Span;
            if (span is null)
            {
