    ...
    internal class TaskContinuationGenerator<TIntegration, TTarget, TReturn, TResult> : ContinuationGenerator<TTarget, TReturn, TResult>
    {
        ...
        private class SyncCallbackHandler : CallbackHandler
        {
            ...
            public override unsafe TReturn ExecuteCallback(TTarget instance, TReturn returnValue, Exception exception, in CallTargetState state)
            {
                ...
                {
                    ((delegate*<TTarget, object, Exception, in CallTargetState, TResult>)_continuation)(instance, default, exception, in state);
                    return returnValue;
                }
                ...
                {
                    var result = ((delegate*<TTarget, object, Exception, in CallTargetState, TResult>)_continuation)(instance, previousTask.Result, default, in state);
            ...
            private async Task<TResult> ContinuationAction(Task<TResult> previousTask, TTarget target, CallTargetState state)
            {
                ...
                try
                {
                    ...
                    unsafe
                    {
                        continuationResult = ((delegate*<TTarget, object, Exception, in CallTargetState, TResult>)_continuation)(target, taskResult, exception, in state);
                    }
                }
                catch (Exception ex)
                {
        ...
        private class AsyncCallbackHandler : CallbackHandler
        {
            ...
            private async Task<TResult> ContinuationAction(Task<TResult> previousTask, TTarget target, CallTargetState state, Exception exception)
            {
                ...
                try
                {
                    ...
                    unsafe
                    {
                        task = ((delegate*<TTarget, object, Exception, in CallTargetState, Task<TResult>>)_asyncContinuation)(target, taskResult, exception, in state);
                    }

                    continuationResult = await task.ConfigureAwait(_preserveContext);
                }
                catch (Exception ex)
                {
                    IntegrationOptions<TIntegration, TTarget>.LogException(ex);
