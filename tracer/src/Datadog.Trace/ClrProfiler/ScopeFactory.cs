    ...
    internal static class ScopeFactory
    {
        ...
        internal static Span CreateInactiveOutboundHttpSpan(Tracer tracer, string httpMethod, Uri requestUri, IntegrationId integrationId, out HttpTags tags, ulong? traceId, ulong? spanId, DateTimeOffset? startTime, bool addToTraceContext)
        {
            ...
            try
            {
                ...
                span = tracer.StartSpanInternal(OperationName, tags, serviceName: serviceName, traceId: traceId, spanId: spanId, startTime: startTime, addToTraceContext: addToTraceContext);
                ...
                if (!addToTraceContext && span.InternalContext.TraceContext.SamplingPriority == null)
                {
                    // If we don't add the span to the trace context, then we need to manually call the sampler
                    span.InternalContext.TraceContext.SamplingPriority = tracer.TracerManager.Sampler?.GetSamplingPriority(span);
                }
            }
            catch (Exception ex)
            {
