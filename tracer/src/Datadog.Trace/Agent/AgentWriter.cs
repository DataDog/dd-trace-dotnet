...
namespace Datadog.Trace.Agent
{
    internal sealed class AgentWriter : IAgentWriter
    {
        ...
        public AgentWriter(IApi api, IStatsAggregator? statsAggregator, IStatsdManager statsd, bool automaticFlush = true, int maxBufferSize = 1024 * 1024 * 10, int batchInterval = 100, bool apmTracingEnabled = true, bool initialTracerMetricsEnabled = false, bool openTelemetryTraceCompatibilityEnabled = false)
        : this(api, statsAggregator, statsd, MovingAverageKeepRateCalculator.CreateDefaultKeepRateCalculator(), automaticFlush, maxBufferSize, batchInterval, apmTracingEnabled, initialTracerMetricsEnabled, openTelemetryTraceCompatibilityEnabled)
        {
        }

        internal AgentWriter(IApi api, IStatsAggregator? statsAggregator, IStatsdManager statsd, IKeepRateCalculator traceKeepRateCalculator, bool automaticFlush, int maxBufferSize, int batchInterval, bool apmTracingEnabled, bool initialTracerMetricsEnabled, bool openTelemetryTraceCompatibilityEnabled)
        {
            ...
            {
                TracesEncoding.OtlpJson => new OtlpTracesJsonSerializer(openTelemetryTraceCompatibilityEnabled),
                TracesEncoding.OtlpProtobuf => new OtlpTracesProtobufSerializer(openTelemetryTraceCompatibilityEnabled),
