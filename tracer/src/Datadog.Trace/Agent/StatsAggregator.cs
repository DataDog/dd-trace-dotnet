...
namespace Datadog.Trace.Agent
{
    internal sealed class StatsAggregator : IStatsAggregator
    {
        ...
        {
            _api = api;
            _isOtlp = settings.Manager.InitialExporterSettings.TracesExporter == TracesExporterType.Otlp; // TODO: Do better
        ...
        internal static StatsAggregationKey BuildKey(Span span, bool isOtlp)
        {
            ...
            return new StatsAggregationKey(
                span.ResourceName,
                span.ServiceName, // For OTLP, maybe we don't need this.
                span.OperationName,
                span.Type,
                httpStatusCode,
        ...
        {
            if (!_isOtlp // If we are using OTLP, we don't need to check for non-top-level spans
