...
namespace Datadog.Trace.Propagators
{
    internal class DistributedContextExtractor : IContextExtractor
    {
            ...
            where TCarrierGetter : struct, ICarrierGetter<TCarrier>
        {
            ...
            var traceIdLower = ParseUtility.ParseUInt64(carrier, carrierGetter, InternalSpanContext.Keys.TraceId);
            ...
            var rawTraceId = ParseUtility.ParseString(carrier, carrierGetter, InternalSpanContext.Keys.RawTraceId);
            ...
            var parentId = ParseUtility.ParseUInt64(carrier, carrierGetter, InternalSpanContext.Keys.ParentId) ?? 0;
            var samplingPriority = ParseUtility.ParseInt32(carrier, carrierGetter, InternalSpanContext.Keys.SamplingPriority);
            var origin = ParseUtility.ParseString(carrier, carrierGetter, InternalSpanContext.Keys.Origin);
            var rawSpanId = ParseUtility.ParseString(carrier, carrierGetter, InternalSpanContext.Keys.RawSpanId);
            var propagatedTraceTags = ParseUtility.ParseString(carrier, carrierGetter, InternalSpanContext.Keys.PropagatedTags);
            var w3CTraceState = ParseUtility.ParseString(carrier, carrierGetter, InternalSpanContext.Keys.AdditionalW3CTraceState);
            ...
            spanContext = new InternalSpanContext(traceId, parentId, samplingPriority, serviceName: null, origin, rawTraceId, rawSpanId, isRemote: false)
                          {
