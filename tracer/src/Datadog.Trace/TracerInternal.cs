// <copyright file="TracerInternal.cs" company="Datadog">
...
namespace Datadog.Trace
{
    ...
    /// </summary>
    public class TracerInternal : ITracer, IDatadogTracer, IDatadogOpenTracingTracer
    {
        ...
        private static TracerInternal _instance;
        private static volatile bool _globalInstanceInitialized;

        private readonly TracerManager _tracerManager;

        /// <summary>
        /// Initializes a new instance of the <see cref="TracerInternal"/> class with default settings. Replaces the
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="TracerInternal"/>
        /// class using the specified <see cref="IConfigurationSource"/>. This constructor updates the global settings
        /// for all <see cref="TracerInternal"/> instances in the application.
        /// </summary>
        /// <param name="settings">
        /// A <see cref="TracerSettingsInternal"/> instance with the desired settings,
        ...
        [Obsolete("This API is deprecated, as it replaces the global settings for all Tracer instances in the application. " +
                ...
                + nameof(TracerInternal) + "." + nameof(Configure) + "() to replace the global Tracer settings for the application")]
        [PublicApi]
        public TracerInternal(TracerSettingsInternal settings)
        {
            TelemetryFactory.Metrics.Record(PublicApiUsage.Tracer_Ctor_Settings);
            // Don't call Configure because it will call Start on the TracerManager
            // before this new instance of Tracer is assigned to Tracer.Instance
            TracerManager.ReplaceGlobalManager(settings is null ? null : new ImmutableTracerSettingsInternal(settings, true), TracerManagerFactory.Instance);
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="TracerInternal"/> class.
        /// For testing only.
        ...
        /// </summary>
        internal TracerInternal(TracerSettingsInternal settings, IAgentWriter agentWriter, ITraceSampler sampler, IScopeManager scopeManager, IDogStatsd statsd, ITelemetryController telemetry = null, IDiscoveryService discoveryService = null)
            : this(TracerManagerFactory.Instance.CreateTracerManager(settings is null ? null : new ImmutableTracerSettingsInternal(settings, true), agentWriter, sampler, scopeManager, statsd, runtimeMetrics: null, logSubmissionManager: null, telemetry: telemetry ?? NullTelemetryController.Instance, discoveryService ?? NullDiscoveryService.Instance, dataStreamsManager: null, remoteConfigurationManager: null, dynamicConfigurationManager: null, tracerFlareManager: null))
        {
        }
        ...
        /// </summary>
        private protected TracerInternal(TracerManager tracerManager)
        {
        ...
        /// <summary>
        /// Finalizes an instance of the <see cref="TracerInternal"/> class.
        /// </summary>
        ~TracerInternal()
        {
            // update the count of Tracer instances
        ...
        public static TracerInternal Instance
        {
            get
            {
                ...
                lock (GlobalInstanceLock)
                {
                    ...
                    instance = new TracerInternal(tracerManager: null); // don't replace settings, use existing
                    _instance = instance;
            ...
            [Obsolete("Use " + nameof(TracerInternal) + "." + nameof(Configure) + " to configure the global Tracer" +
                      " instance in code.")]
            [PublicApi]
            set
            {
        ...
        /// </summary>
        public ImmutableTracerSettingsInternal Settings => TracerManager.Settings;
        ...
        /// Gets this tracer's settings.
        /// </summary>
        ImmutableTracerSettingsInternal ITracer.Settings => Settings;
        ...
        /// <summary>
        /// Replaces the global Tracer settings used by all <see cref="TracerInternal"/> instances,
        /// including automatic instrumentation
        /// </summary>
        /// <param name="settings"> A <see cref="TracerSettingsInternal"/> instance with the desired settings,
        /// or null to use the default configuration sources. This is used to configure global settings</param>
        [PublicApi]
        public static void Configure(TracerSettingsInternal settings)
        {
            TelemetryFactory.Metrics.Record(PublicApiUsage.Tracer_Configure);
            ConfigureInternal(settings is null ? null : new ImmutableTracerSettingsInternal(settings, true));
        }

        internal static void ConfigureInternal(ImmutableTracerSettingsInternal settings)
        {
            TracerManager.ReplaceGlobalManager(settings, TracerManagerFactory.Instance);
            TracerInternal.Instance.TracerManager.Start();
        }
        ...
        internal static void UnsafeSetTracerInstance(TracerInternal instance)
        {
            lock (GlobalInstanceLock)
            {
                _instance = instance;
        ...
        internal SpanContextInternal CreateSpanContext(ISpanContext parent = null, string serviceName = null, TraceId traceId = default, ulong spanId = 0, string rawTraceId = null, string rawSpanId = null)
        {
            ...
            if (parent is SpanContextInternal parentSpanContext)
            {
            ...
            var context = new SpanContextInternal(parent, traceContext, finalServiceName, traceId: traceId, spanId: spanId, rawTraceId: rawTraceId, rawSpanId: rawSpanId);
