...
namespace Datadog.Trace
{
    ...
    internal readonly struct SpanEventArgs
    {
        ...
        {
            TraceId = span?.TraceId ?? 0;
            SpanId = span?.SpanId ?? 0;
        }
        ...
        /// Creates a new <see cref="SpanEventArgs"/> using <paramref name="span"/>
        /// </summary>
        /// <param name="span">The <see cref="SpanContext"/> used to initialize the <see cref="SpanEventArgs"/> object.</param>
        public SpanEventArgs(SpanContext span)
        {
            TraceId = span?.TraceId ?? 0;
            SpanId = span?.SpanId ?? 0;
        }
