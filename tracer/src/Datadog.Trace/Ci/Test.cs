...
public sealed class Test
{
    ...
    /// <summary>
    /// Duck-typed API invoked by Datadog.Trace.Manual
    /// </summary>
    internal void SetParameters(Dictionary<string, object>? metadata, Dictionary<string, object>? arguments)
        => SetParameters(new TestParameters() { Metadata = metadata, Arguments = arguments });

    /// <summary>
    /// Duck-typed API invoked by Datadog.Trace.Manual
    /// </summary>
    internal void AddBenchmarkData(BenchmarkMeasureType measureType, string info, int n, double max, double min, double mean, double median, double standardDeviation, double standardError, double kurtosis, double skewness, double p99, double p95, double p90)
        => AddBenchmarkData(
            measureType,
            info,
            new(
    ...
    internal void SetBenchmarkMetadata(Dictionary<string, object?> values)
    {
        var hostInfo = new BenchmarkHostInfo
        {
            ProcessorName = TryGetValue<string?>(values, nameof(BenchmarkHostInfo.ProcessorName)),
            ProcessorCount = TryGetValue<int?>(values, nameof(BenchmarkHostInfo.ProcessorCount)),
            PhysicalCoreCount = TryGetValue<int?>(values, nameof(BenchmarkHostInfo.PhysicalCoreCount)),
            LogicalCoreCount = TryGetValue<int?>(values, nameof(BenchmarkHostInfo.LogicalCoreCount)),
            ProcessorMaxFrequencyHertz = TryGetValue<double?>(values, nameof(BenchmarkHostInfo.ProcessorMaxFrequencyHertz)),
            OsVersion = TryGetValue<string?>(values, nameof(BenchmarkHostInfo.OsVersion)),
            RuntimeVersion = TryGetValue<string?>(values, nameof(BenchmarkHostInfo.RuntimeVersion)),
            ChronometerFrequencyHertz = TryGetValue<double?>(values, nameof(BenchmarkHostInfo.ChronometerFrequencyHertz)),
            ChronometerResolution = TryGetValue<double>(values, nameof(BenchmarkHostInfo.ChronometerResolution)),
        };

        var jobInfo = new BenchmarkJobInfo()
        {
            Description = TryGetValue<string?>(values, nameof(BenchmarkJobInfo.Description)),
            Platform = TryGetValue<string?>(values, nameof(BenchmarkJobInfo.Platform)),
            RuntimeName = TryGetValue<string?>(values, nameof(BenchmarkJobInfo.RuntimeName)),
            RuntimeMoniker = TryGetValue<string?>(values, nameof(BenchmarkJobInfo.RuntimeMoniker)),
        };

        SetBenchmarkMetadata(in hostInfo, in jobInfo);
        return;

        static T? TryGetValue<T>(Dictionary<string, object?> values, string key)
        {
            if (values.TryGetValue(key, out var value) && value is T t)
            {
                return t;
            }
