...
internal sealed class CoverageBackfillData
{
    ...
    private CoverageBackfillData(bool isPresent, bool isValid, string? error, Dictionary<string, byte[]>? executedLinesByRelativePath)
    {
        ...
        IsValid = isValid;
        Error = error;
        ExecutedLinesByRelativePath = executedLinesByRelativePath ?? new Dictionary<string, byte[]>(StringComparer.Ordinal);
    }
    ...
    public int TotalBitmapBytes
    {
        get
        {
            ...
            {
                totalBitmapBytes += bitmap?.Length ?? 0;
            }
    ...
    public static string NormalizePath(string path)
    {
        ...
        {
            ThrowHelper.ThrowArgumentException("Coverage path cannot be empty.", nameof(path));
        }

        var normalizedPath = path.Replace('\\', '/').TrimStart('/');

        if (StringUtil.IsNullOrWhiteSpace(normalizedPath))
        {
            ThrowHelper.ThrowArgumentException("Coverage path cannot point to the repository root.", nameof(path));
        }
