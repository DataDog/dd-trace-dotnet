...
internal sealed class FlagEvaluationAggregator
{
    ...
    private const byte TagByte = (byte)'B';
    private const byte TagSByte = (byte)'y';
    private const byte TagShort = (byte)'h';
    private const byte TagUShort = (byte)'H';
    private const byte TagInt = (byte)'i';
    private const byte TagUInt = (byte)'I';
    private const byte TagLong = (byte)'l';
    private const byte TagULong = (byte)'L';
    private const byte TagFloat = (byte)'d';
    private const byte TagDouble = (byte)'f';
    private const byte TagDecimal = (byte)'m';
    private const byte TagDateTime = (byte)'t';
    private const byte TagDateTimeOffset = (byte)'T';
    private const byte TagNull = (byte)'n';
...
#pragma warning disable SA1204 // static helpers follow instance methods for readability
    ...
    public void Add(FlagEvalEvent ev)
    {
        ...
        Dictionary<string, object?>? contextAttrs = ev.ObserveFullEvaluationData && ev.ContextAttrs is { Count: > 0 } ? ev.ContextAttrs : null;
        ...
        lock (_lock)
        {
            ...
            {
                existing.Observe(evalTimeMs, ev.ObserveFullEvaluationData);
                return;
            }
            ...
            {
                AddToDegraded(ev.FlagKey, variant, ev.AllocationKey, ev.ErrorMessage, evalTimeMs, runtimeDefault, ev.ObserveFullEvaluationData);
                return;
            }
            ...
            // New full-tier entry.
            _full[fullKey] = new EvaluationEntry(evalTimeMs, runtimeDefault, ev.ObserveFullEvaluationData, contextAttrs);
            _globalCount++;
        }
    }
    ...
    private void AddToDegraded(string flagKey, string variant, string allocationKey, string errorMessage, long evalTimeMs, bool runtimeDefault, bool observeFullEvaluationData)
    {
        var degKey = new DegradedKey(flagKey, variant, allocationKey, errorMessage, runtimeDefault, observeFullEvaluationData);
        if (_degraded.TryGetValue(degKey, out EvaluationEntry? existing))
        {
            existing.Observe(evalTimeMs, observeFullEvaluationData);
            return;
        }
        ...
        _degraded[degKey] = new EvaluationEntry(evalTimeMs, runtimeDefault, observeFullEvaluationData, contextAttrs: null);
    }
    ...
    private static void AppendContextValue(List<byte> buf, object? v)
    {
        ...
        switch (v)
        {
            ...
            case byte b:
                tag = TagByte;
                payload = Encoding.UTF8.GetBytes(b.ToString(CultureInfo.InvariantCulture));
                break;
            case sbyte sb:
                tag = TagSByte;
                payload = Encoding.UTF8.GetBytes(sb.ToString(CultureInfo.InvariantCulture));
                break;
            case short s:
                tag = TagShort;
                payload = Encoding.UTF8.GetBytes(s.ToString(CultureInfo.InvariantCulture));
                break;
            case ushort us:
                tag = TagUShort;
                payload = Encoding.UTF8.GetBytes(us.ToString(CultureInfo.InvariantCulture));
                break;
            case int i:
                tag = TagInt;
            ...
            case uint ui:
                tag = TagUInt;
                payload = Encoding.UTF8.GetBytes(ui.ToString(CultureInfo.InvariantCulture));
                break;
            case long l:
                tag = TagLong;
                payload = Encoding.UTF8.GetBytes(l.ToString(CultureInfo.InvariantCulture));
                break;
            case ulong ul:
                tag = TagULong;
                payload = Encoding.UTF8.GetBytes(ul.ToString(CultureInfo.InvariantCulture));
                break;
            case float f:
                tag = TagFloat;
                payload = Encoding.UTF8.GetBytes(f.ToString("R", CultureInfo.InvariantCulture));
                break;
            case double d:
                tag = TagDouble;
                payload = Encoding.UTF8.GetBytes(d.ToString("R", CultureInfo.InvariantCulture));
                break;
            case decimal m:
                tag = TagDecimal;
                payload = Encoding.UTF8.GetBytes(m.ToString(CultureInfo.InvariantCulture));
                break;
            case DateTime dateTime:
                tag = TagDateTime;
                payload = Encoding.UTF8.GetBytes(dateTime.ToString("O", CultureInfo.InvariantCulture));
                break;
            case DateTimeOffset dateTimeOffset:
                tag = TagDateTimeOffset;
                payload = Encoding.UTF8.GetBytes(dateTimeOffset.ToString("O", CultureInfo.InvariantCulture));
                break;
            case null:
                tag = TagNull;
                payload = [];
            ...
            default:
                tag = TagOther;
                var typeName = v.GetType().FullName ?? v.GetType().Name;
                var value = v is IFormattable formattable
                                ? formattable.ToString(format: null, CultureInfo.InvariantCulture)
                                : v.ToString();
                payload = Encoding.UTF8.GetBytes(typeName + "\0" + value);
                break;
        }
