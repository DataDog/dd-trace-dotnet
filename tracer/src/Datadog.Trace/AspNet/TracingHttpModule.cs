                    ...
                    app.Context.Items[_httpContextScopeKey] is Scope scope)
                {
                    try
                    {
                        ...
                        if (security.Settings.Enabled)
                        {
                            ...
                            security.InstrumentationGateway.RaisePathParamsAvailable(httpContext, scope.Span, HttpRequestUtils.ConvertRouteValueDictionary(httpContext.Request.RequestContext.RouteData.Values), eraseExistingAddress: false);
