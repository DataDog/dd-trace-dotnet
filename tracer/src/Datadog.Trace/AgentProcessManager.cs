...
namespace Datadog.Trace
{
    ...
    {
        private static readonly ConfigurationBuilder ConfigurationBuilder = ConfigurationBuilder.FromEnvironmentSourceOnly(NullConfigurationTelemetry.Instance);
        ...
        internal static readonly ProcessMetadata TraceAgentMetadata = new()
        {
            Name = "datadog-trace-agent",
            PipeName = ConfigurationBuilder.WithKeys(ConfigurationKeys.TracesPipeName).AsString(),
            ProcessPath = ConfigurationBuilder.WithKeys(ConfigurationKeys.TraceAgentPath).AsString(),
            ProcessArguments = ConfigurationBuilder.WithKeys(ConfigurationKeys.TraceAgentArgs).AsString()
        };

        internal static readonly ProcessMetadata DogStatsDMetadata = new()
        {
            Name = "dogstatsd",
            PipeName = ConfigurationBuilder.WithKeys(ConfigurationKeys.MetricsPipeName).AsString(),
            ProcessPath = ConfigurationBuilder.WithKeys(ConfigurationKeys.DogStatsDPath).AsString(),
            ProcessArguments = ConfigurationBuilder.WithKeys(ConfigurationKeys.DogStatsDArgs).AsString(),
        };

        private static readonly List<ProcessMetadata> Processes = new(2);
        ...
        private static void StartProcesses(ImmutableAzureAppServiceSettings azureAppServiceSettings)
        {
            ...
            {
                var ddLogLevelKey = ConfigurationBuilder.WithKeys(ConfigurationKeys.LogLevel).AsStringResult();
                if (!ddLogLevelKey.ConfigurationResult.IsPresent)
                {
                    // This ensures that a single setting from applicationConfig can enable debug logs for every aspect of the extension
                    EnvironmentHelpers.SetEnvironmentVariable(ConfigurationKeys.LogLevel, "debug");
                }
            }
