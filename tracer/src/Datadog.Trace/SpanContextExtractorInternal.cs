// <copyright file="SpanContextExtractorInternal.cs" company="Datadog">
...
{
    /// <summary>
    /// The <see cref="SpanContextExtractorInternal"/> is responsible for extracting <see cref="ISpanContext"/> in the rare cases
    ...
    /// </summary>
    public class SpanContextExtractorInternal
    {
        private static readonly IDatadogLogger Log = DatadogLogging.GetLoggerFor<SpanContextExtractorInternal>();

        /// <summary>
        /// Initializes a new instance of the <see cref="SpanContextExtractorInternal"/> class
        /// </summary>
        [PublicApi]
        public SpanContextExtractorInternal()
        {
            TelemetryFactory.Metrics.Record(PublicApiUsage.SpanContextExtractor_Ctor);
        }

        internal SpanContextExtractorInternal(bool unusedParamNotToUsePublicApi)
        {
        ...
        internal static SpanContextInternal? ExtractInternal<TCarrier>(TCarrier carrier, Func<TCarrier, string, IEnumerable<string?>> getter, string? messageType = null, string? source = null)
        {
            ...
            if (spanContext is not null
             && TracerInternal.Instance.TracerManager.DataStreamsManager is { IsEnabled: true } dsm)
            {
