    ...
    internal sealed class AspNetCoreDiagnosticObserver : DiagnosticObserver
    {
            ...
            HttpRequest request)
        {
                ...
                RecordedSpanContext spanContext)
            {
                ...
                if (!isUsingEndpointRouting && isFirstExecution)
                {
                    ...
                    rootSpan.SetResourceName(span.ResourceName); // TODO: This one is technically a problem, as it changes sampling, but this is a problem today I believe
                }
        ...
        private void OnRoutingEndpointMatched(object arg)
        {
            ...
            if (arg.TryDuckCast<HttpRequestInEndpointMatchedStruct>(out var typedArg)
             ...
             && httpContext.Items[AspNetCoreHttpRequestHandler.HttpContextTrackingKey] is AspNetCoreHttpRequestHandler.RequestTrackingFeature { RootScope.Span: Span { } rootSpan } trackingFeature)
            {
        ...
        private void OnMvcBeforeAction(object arg)
        {
             ...
             && httpContext.Items[AspNetCoreHttpRequestHandler.HttpContextTrackingKey] is AspNetCoreHttpRequestHandler.RequestTrackingFeature { RootScope.Span: Span { } rootSpan } trackingFeature)
            {
                ...
                if (integrationEnabled)
                {
                    ...
                    {
                        spanBase = StartMvcCoreSpan(_tracer, trackingFeature, typedArg, httpContext, request);
                    }
                }
