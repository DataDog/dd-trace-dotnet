...
if [ "$1" = "linux" ]; then
    ...
    rm -f appsec_iast_baseline.json
    ...
    crank --config Security.Samples.AspNetCoreSimpleController.yml --scenario appsec_iast_baseline --profile linux --json appsec_iast_baseline.json $repository $commit  --property name=AspNetCoreSimpleController --property scenario=appsec_iast_baseline --property profile=linux --property arch=x64 --variable commit_hash=$commit_sha
    dd-trace --crank-import="appsec_iast_baseline.json"
