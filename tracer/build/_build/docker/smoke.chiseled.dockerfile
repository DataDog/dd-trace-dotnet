...
ENTRYPOINT ["/opt/datadog/linux-x64/dd-dotnet", "run", "--", "dotnet", "/app/AspNetCoreSmokeTest.dll"]

###########################################################
FROM installer-base as dd-dotnet-final-linux-arm64
ENTRYPOINT ["/opt/datadog/linux-arm64/dd-dotnet", "run", "--", "dotnet", "/app/AspNetCoreSmokeTest.dll"]
