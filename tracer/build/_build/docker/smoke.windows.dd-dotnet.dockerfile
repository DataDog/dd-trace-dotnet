...
FROM mcr.microsoft.com/dotnet/sdk:$DOTNETSDK_VERSION-windowsservercore-ltsc2025 as builder
...
RUN if($env:CHANNEL_32_BIT){ \
    ...
    curl 'https://github.com/dotnet/install-scripts/raw/2bdc7f2c6e00d60be57f552b8a8aab71512dbcb2/src/dotnet-install.ps1' -o dotnet-install.ps1; \
...
ENTRYPOINT ["c:\\Program Files\\Datadog\\.NET Tracer\\dd-dotnet.cmd", "run","--set-env", "DD_APPSEC_ENABLED=1","--set-env", "DD_TRACE_DEBUG=1", "--", "dotnet", "AspNetCoreSmokeTest.dll"]
