...
# Retry connection resets as well as HTTP failures, bounding each download to five minutes.
RUN if [ "$(uname -m)" = "x86_64" ]; \
    then attempt=1; \
        until curl -fsSL --connect-timeout 30 --max-time 300 "https://github.com/Azure/azure-functions-core-tools/releases/download/${AZURE_FUNCTIONS_CORE_TOOLS_VERSION}/Azure.Functions.Cli.linux-x64.${AZURE_FUNCTIONS_CORE_TOOLS_VERSION}.zip" --output azure-functions-core-tools.zip; do \
            rm -f azure-functions-core-tools.zip; \
            if [ "$attempt" -ge 5 ]; then \
                echo "Azure Functions Core Tools download failed after $attempt attempts" >&2; \
                exit 1; \
            fi; \
            echo "Azure Functions Core Tools download attempt $attempt failed; retrying" >&2; \
            sleep $((attempt * 5)); \
            attempt=$((attempt + 1)); \
        done \
