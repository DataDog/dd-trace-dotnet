...
partial class Build
{
    ...
    Target CreatePlatformlessSymlinks => _ => _
        ...
        .Executes(() =>
        {
            ...
            directories.ForEach(existingDir =>
            {
                ...
                {
                    Logger.Information($"Skipping '{newDir}' as already exists");
                }
                else
                {
    ...
    private void CheckLogsForErrors(List<Regex> knownPatterns, bool allFilesMustExist, LogLevel minLogLevel)
    {
        ...
        {
            Logger.Information("No errors found in managed or native logs");
            return;
        }

        Logger.Warning("Found the following errors in log files:");
