    ...
    public class ScopeFactoryTests
    {
        ...
        public async Task CreateOutboundHttpScope_AlwaysCreatesOneAutomaticInstrumentationScope(int integration1, int integration2)
        {
            ...
            using (var manualScope = tracer.StartActive("http.request"))
            {
                ...
                // manualScope.Span.ResourceName = $"{method} {url}"; // non-recording-spans experiment: setter removed
                ((Span)manualScope.Span).SetService($"{tracer.DefaultServiceName}-http-client", null);
