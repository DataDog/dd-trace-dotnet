...
Console.WriteLine($"MassTransit 7 Sample - MASSTRANSIT_TRANSPORT={transport}");
...
async Task RunAmazonSqs() =>
    await RunWithTransport<GettingStartedWithSqs>(
        ...
        cfg =>
        {
            ...
            {
                // LocalStack endpoint override (default http://localhost:4566)
...
async Task RunExceptionTest()
{
    ...
    var serviceProvider = services.BuildServiceProvider();
    var busControl = serviceProvider.GetRequiredService<IBusControl>();
    var consumeSignalObserver = new ConsumeSignalObserver(new Dictionary<Type, string>
    {
        [typeof(FailingMessage)] = "fault:consumer-exception"
    });
    ...
    try
    {
        ...
        await busControl.StartAsync();
        var consumeObserverHandle = busControl.ConnectConsumeObserver(consumeSignalObserver);
        ...
        consumeObserverHandle.Disconnect();
    }
    finally
    {
...
async Task RunHandlerExceptionTest()
{
    ...
    var consumeSignalObserver = new ConsumeSignalObserver(new Dictionary<Type, string>
    {
        [typeof(HandlerFailingMessage)] = "fault:handler-exception"
    });
...
async Task RunSagaExceptionTest()
{
    ...
    var consumeSignalObserver = new ConsumeSignalObserver(new Dictionary<Type, string>
    {
        [typeof(OrderFailed)] = "fault:saga-exception"
    });
