    ...
    internal struct DdwafObjectStruct
    {
        ...
        internal Dictionary<string, object?> DecodeMap()
        {
            ...
            if (nbEntriesStart > 0)
            {
                ...
                {
                    Console.WriteLine($"Expecting type {nameof(DDWAF_OBJ_TYPE.DDWAF_OBJ_MAP)} to decode waf errors and instead got a {Type} ");
                }
                else
                {
        ...
        private List<T> DecodeArray<T>()
        {
            ...
            if (nbEntriesStart > 0)
            {
                ...
                {
                    Console.WriteLine($"Expecting type {nameof(DDWAF_OBJ_TYPE.DDWAF_OBJ_ARRAY)} to decode waf errors and instead got a {Type} ");
                }
                else
                {
                    var structSize = Marshal.SizeOf(typeof(DdwafObjectStruct));
