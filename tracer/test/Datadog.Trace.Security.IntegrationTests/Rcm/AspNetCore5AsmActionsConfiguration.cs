...
public class AspNetCore5AsmActionsConfiguration : RcmBase
{
    ...
    public async Task TestBlockingAction(string type, int statusCode, string argument, string actionName)
    {
        ...
        await agent.SetupRcmAndWait(Output, new[] { ((object)new Payload { Actions = new[] { new Action { Id = actionName, Type = BlockingAction.BlockRequestType, Parameters = new Parameter { StatusCode = 403, Type = "auto" } } } }, AsmProduct, nameof(TestBlockingAction)) });
