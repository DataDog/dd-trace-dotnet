...
namespace Datadog.Trace.Security.IntegrationTests.Rcm
{
    public class AspNetCore5AsmRulesToggle : RcmBase
    {
        ...
        public async Task TestRulesToggling()
        {
            ...
            var request1 = await agent.SetupRcmAndWait(Output, new[] { ((object)new Payload() { RuleOverride = new[] { new RuleOverride { Id = ruleId, Enabled = false } } }, acknowledgedId) }, ASMProduct, appliedServiceNames: new[] { acknowledgedId });
            ...
            var request2 = await agent.SetupRcmAndWait(Output, new[] { ((object)new Payload() { RuleOverride = new[] { new RuleOverride { Id = ruleId, Enabled = true } } }, acknowledgedId2) }, ASMProduct, appliedServiceNames: new[] { acknowledgedId2 });
