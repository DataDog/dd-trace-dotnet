...
namespace Datadog.Trace.TestHelpers
{
    public sealed class AspNetCoreTestFixture : IDisposable
    {
        ...
        public async Task TryStartApp(TestHelper helper, bool sendHealthCheck = true, string packageVersion = "")
        {
            ...
            if (Process is null)
            {
                WriteToOutput("Starting aspnetcore sample (no agent — NullAgentWriter active)");
                Process = await helper.StartSample(null, arguments: null, packageVersion: packageVersion, aspNetCorePort: 0);
        ...
        public async Task SendHttpRequestAndWait(string path, HttpMethod method = null, Dictionary<string, string> headers = null)
        {
