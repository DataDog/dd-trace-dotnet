...
namespace Datadog.Trace.Tests.Agent
{
    public class AgentWriterTests
    {
        ...
        public async Task SpanSampling_CanComputeStats_ShouldNotSend_WhenSpanSamplingDoesNotMatch()
        {
            ...
            var tracer = new TracerInternal(settings, agent, sampler: null, scopeManager: null, statsd: null);

            var traceContext = new TraceContext(tracer);
            var spanContext = new SpanContextInternal(null, traceContext, "service");
        ...
        public async Task SpanSampling_ShouldSend_MultipleMatchedSpans_WhenStatsDrops()
        {
            ...
            var traceContext = new TraceContext(tracer);
            traceContext.SetSamplingPriority(SamplingPriorityValues.UserReject, SamplingMechanism.Manual);
            var rootSpanContext = new SpanContextInternal(null, traceContext, "service");
            var rootSpan = new Span(rootSpanContext, DateTimeOffset.UtcNow) { OperationName = "operation" };
            var keptChildSpan = new Span(new SpanContextInternal(rootSpanContext, traceContext, "service"), DateTimeOffset.UtcNow) { OperationName = "operation" };
        ...
        public async Task SpanSampling_ShouldSend_MultipleMatchedSpans_WhenStatsDropsOne()
        {
            ...
            var rootSpanContext = new SpanContextInternal(null, traceContext, "service");
            var rootSpan = new Span(rootSpanContext, DateTimeOffset.UtcNow) { OperationName = "operation" };
            var droppedChildSpan = new Span(new SpanContextInternal(rootSpanContext, traceContext, "service"), DateTimeOffset.UtcNow) { OperationName = "drop_me" };
            var droppedChildSpan2 = new Span(new SpanContextInternal(rootSpanContext, traceContext, "service"), DateTimeOffset.UtcNow) { OperationName = "drop_me_also" };
            var keptChildSpan = new Span(new SpanContextInternal(rootSpanContext, traceContext, "service"), DateTimeOffset.UtcNow) { OperationName = "operation" };
            traceContext.AddSpan(rootSpan); // IS single span sampled
        ...
        public async Task AddsTraceKeepRateMetricToRootSpan()
        {
            ...
            var rootSpanContext = new SpanContextInternal(null, traceContext, null);
            var rootSpan = new Span(rootSpanContext, DateTimeOffset.UtcNow);
            var childSpan = new Span(new SpanContextInternal(rootSpanContext, traceContext, null), DateTimeOffset.UtcNow);
        ...
        private static ArraySegment<Span> CreateTraceChunk(int spanCount, ulong startingId = 1)
        {
            ...
            {
                var spanContext = new SpanContextInternal(startingId + i, startingId + i);
        ...
        private static TracerSettingsInternal SpanSamplingRule(string serviceName, string operationName, float sampleRate = 1.0f)
        {
            ...
            return TracerSettingsInternal.Create(new() { { ConfigurationKeys.SpanSamplingRules, JsonConvert.SerializeObject(rules) } });
        }
    }
}
