...
public class SpanMessagePackFormatterTests
{
    ...
    public void SerializeSpans()
    {
        ...
        var parentContext = new SpanContextInternal(new TraceId(0, 1), 2, (int)SamplingPriority.UserKeep, "ServiceName1", "origin1");

        var spans = new[]
        {
            new Span(parentContext, DateTimeOffset.UtcNow),
            new Span(new SpanContextInternal(parentContext, new TraceContext(Mock.Of<IDatadogTracer>()), "ServiceName1"), DateTimeOffset.UtcNow),
            new Span(new SpanContextInternal(new TraceId(0, 5), 6, (int)SamplingPriority.UserKeep, "ServiceName3", "origin3"), DateTimeOffset.UtcNow),
        };
    ...
    public async Task TraceId128_PropagatedTag(bool generate128BitTraceId)
    {
        ...
        var settings = TracerSettingsInternal.Create(new() { { ConfigurationKeys.FeatureFlags.TraceId128BitGenerationEnabled, generate128BitTraceId } });
        var agentWriter = new AgentWriter(mockApi, statsAggregator: null, statsd: null, automaticFlush: false);
        var tracer = new TracerInternal(settings, agentWriter, sampler: null, scopeManager: null, statsd: null, NullTelemetryController.Instance, NullDiscoveryService.Instance);
