...
public class FlagEvalEVPHookTests
{
    ...
    {
        Datadog.Trace.FeatureFlags.Evp.FeatureFlagsEvpTransport.FlagEvaluationIntakePath
            .Should().Be("api/v2/flagevaluation");
    }

    [Fact]
    public void Dispose_RemovesFlagEvaluationApiFromModule()
    {
        var module = new FeatureFlagsModule(CreateSettings(killswitch: null), new MockRcmSubscriptionManager());
        ...
        module.GetFlagEvalEVPApi().Should().BeNull("a stale CallTarget bridge must fail closed after module disposal");
    }
