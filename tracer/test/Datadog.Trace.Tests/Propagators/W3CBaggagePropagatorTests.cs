...
public class W3CBaggagePropagatorTests
{
    ...
    public void CreateHeader(string expectedHeader, (string Key, string Value)[] inputPairs)
    {
        ...
        {
            baggage.Set(pair.Key, pair.Value);
        }

        W3CBaggagePropagator.CreateHeader(baggage, W3CBaggagePropagator.DefaultMaximumBaggageItems, W3CBaggagePropagator.DefaultMaximumBaggageItems)
                            .Should()
                            .Be(expectedHeader);
    }
    ...
    public void CreateHeader_MaxItems(int maxBaggageItems, string expected)
    {
        ...
        W3CBaggagePropagator.CreateHeader(baggage, maxBaggageItems, W3CBaggagePropagator.DefaultMaximumBaggageItems)
                            .Should()
                            .Be(expected);
    }
    ...
    public void ExtractHeader(string inputHeader, (string Key, string Value)[] expectedPairs)
    {
        ...
        expectedPairs.Length.Should().Be(baggage!.Items?.Count);

        foreach (var pair in expectedPairs)
        {
            baggage.Get(pair.Key).Should().Be(pair.Value);
        }
    }
    ...
    public void Extract_IHeadersCollection()
    {
        ...
        baggage.Get("key1").Should().Be("value1");
        baggage.Get("key2").Should().Be("value2");
