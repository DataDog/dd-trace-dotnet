...
public class LiveDebuggerTests
{
    ...
    public async Task DebuggerEnabled_ServicesCalled()
    {
        ...
        await Task.WhenAny(debugger.InitializeAsync(), Task.Delay(TimeSpan.FromSeconds(1)));

        probeStatusPoller.Called.Should().BeTrue();
        debuggerSink.Called.Should().BeTrue();
        managerMock.Products.ContainsKey(LiveDebugger.Product.Name).Should().BeTrue();
    }
    ...
    public async Task DebuggerDisabled_ServicesNotCalled()
    {
        ...
        managerMock.Products.ContainsKey(LiveDebugger.Product.Name).Should().BeFalse();
    }
