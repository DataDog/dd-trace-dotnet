...
public class DataStreamsTransactionContainerTest
{
    ...
    [Fact]
    public void ShouldFlush_ReturnsFalse_WhenBelowThreshold()
    {
        var container = new DataStreamsTransactionContainer(1024);
        container.Add(new DataStreamsTransactionInfo("id", 1L, "cp"));
        container.ShouldFlush.Should().BeFalse();
    }

    [Fact]
    public void ShouldFlush_ReturnsTrue_WhenThresholdExceeded()
    {
        ...
        {
            container.Add(new DataStreamsTransactionInfo(id, (long)i, "cp"));
        }

        container.ShouldFlush.Should().BeTrue();
    }

    [Fact]
    public void Add_DoesNotDropData_WhenAboveFlushThreshold()
    {
        var id = new string('x', 512);
        var byteCount = new DataStreamsTransactionInfo(id, 0L, "cp").GetByteCount();
        var count = (512 * 1024 / byteCount) + 10;
        ...
        container.Size().Should().Be(count * byteCount);
    }
}
