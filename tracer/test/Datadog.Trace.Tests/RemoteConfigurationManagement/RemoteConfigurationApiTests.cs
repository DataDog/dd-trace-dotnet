...
public class RemoteConfigurationApiTests
{
    ...
    [Theory]
    [InlineData(MimeTypes.MultipartFormData)]
    [InlineData(MimeTypes.MsgPack)]
    [InlineData(MimeTypes.PlainText)]
    public async Task WhenEndpointReturnsWrongContentType_Throws(string contentType)
    {
        var responseContent = RcmResponses.HugeConfig();
        var requestFactory = new TestRequestFactory(
            url => new TestApiRequest(url, responseContent: responseContent, responseContentType: contentType));
        var discoveryService = new DiscoveryServiceMock();
        var api = RemoteConfigurationApi.Create(requestFactory, discoveryService);
        discoveryService.TriggerChange();
        ...
        await api.Invoking(x => x.GetConfigs(GetRequest()))
                 ...
                 .WithInnerExceptionExactly<RemoteConfigurationDeserializationException, Exception>()
                 .WithMessage($"Unexpected content type {contentType}*");
    }
    ...
    public async Task WhenEndpointReturnsInvalidContent_InvalidFirstCharacter_Throws(string responseContent)
    {
        ...
        await api.Invoking(x => x.GetConfigs(GetRequest()))
                 .Should()
                 .ThrowAsync<RemoteConfigurationDeserializationException>()
                 .WithInnerExceptionExactly<RemoteConfigurationDeserializationException, JsonReaderException>()
                 .WithMessage($"Unexpected character encountered while parsing value: {responseContent[0]}. Path '', line 0, position 0.");
    }
