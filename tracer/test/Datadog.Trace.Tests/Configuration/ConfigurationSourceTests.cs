...
namespace Datadog.Trace.Tests.Configuration
{
    ...
    public class ConfigurationSourceTests : IDisposable
    {
        ...
        public static IEnumerable<(Func<InternalGlobalSettings, object> Getter, object Expected)> GetGlobalDefaultTestData()
        {
            yield return (s => s.DebugEnabled, false);
            yield return (s => s.DiagnosticSourceEnabled, true);
        }

        public static IEnumerable<(string Key, string Value, Func<InternalGlobalSettings, object> Getter, object Expected)> GetGlobalTestData()
        {
        ...
        public static IEnumerable<(Func<InternalTracerSettings, object> SettingGetter, object ExpectedValue)> GetDefaultTestData()
        {
...
#pragma warning restore 618
        ...
        public static IEnumerable<(string Key, string Value, Func<InternalTracerSettings, object> Getter, object Expected)> GetTestData()
        {
...
#pragma warning restore 618
        ...
        public static IEnumerable<(string Value, Func<InternalTracerSettings, object> Getter, object Expected)> GetJsonTestData()
        {
        ...
        public static IEnumerable<(string Value, Func<InternalTracerSettings, object> Getter, object Expected)> GetBadJsonTestData3()
        {
        ...
        public void EnvironmentConfigurationSource()
        {
            foreach (var (key, value, settingGetter, expectedValue) in GetTestData())
            {
                InternalTracerSettings settings;
            ...
            static InternalTracerSettings GetTracerSettings(string key, string value)
            {
                Environment.SetEnvironmentVariable(key, value, EnvironmentVariableTarget.Process);
                IConfigurationSource source = new EnvironmentConfigurationSource();
                return new InternalTracerSettings(source);
            }
        }
        ...
        public void GlobalDefaultSetting()
        {
            ...
            {
                var settings = new InternalGlobalSettings(NullConfigurationSource.Instance, NullConfigurationTelemetry.Instance);
                object actualValue = settingGetter(settings);
                Assert.Equal(expectedValue, actualValue);
            }
        }
