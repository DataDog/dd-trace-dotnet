    ...
    public class ConfigurationSourceTests : IDisposable
    {
        ...
        public ConfigurationSourceTests()
        {
            _envVars = GetTestData()
                      ...
                      .Concat([ConfigurationKeys.BreakingChangesEnabled])
                      .Distinct()
...
#pragma warning restore 618
        ...
        [Fact]
        public void BreakingChanges_NameValueConfigurationSource() => AssertNameValueConfigurationSource(GetBreakingChangeTestData(), setBreakingChangesEnabled: true);

        [Fact]
        public void EnvironmentConfigurationSource() => AssertEnvironmentConfigurationSource(GetTestData());

        [Fact]
        public void BreakingChanges_EnvironmentConfigurationSource() => AssertEnvironmentConfigurationSource(GetBreakingChangeTestData(), setBreakingChangesEnabled: true);

        [Fact]
        public void JsonConfigurationSource() => AssertJsonConfigurationSource(GetTestData());

        [Fact]
        public void BreakingChanges_JsonConfigurationSource() => AssertJsonConfigurationSource(GetBreakingChangeTestData(), setBreakingChangesEnabled: true);
        ...
        private void AssertNameValueConfigurationSource(IEnumerable<(string Key, string Value, Func<TracerSettings, object> Getter, object Expected)> testData, bool setBreakingChangesEnabled = false)
        {
            foreach (var (key, value, settingGetter, expectedValue) in testData)
            {
                var collection = new NameValueCollection { { key, value } };
                collection.Add(ConfigurationKeys.BreakingChangesEnabled, setBreakingChangesEnabled.ToString());
                IConfigurationSource source = new NameValueConfigurationSource(collection);
                var settings = new TracerSettings(source);
                object actualValue = settingGetter(settings);
                // Assert.Equal(expectedValue, actualValue);
        ...
        private void AssertEnvironmentConfigurationSource(IEnumerable<(string Key, string Value, Func<TracerSettings, object> Getter, object Expected)> testData, bool setBreakingChangesEnabled = false)
        {
            foreach (var (key, value, settingGetter, expectedValue) in testData)
            {
                ...
                if (key == "DD_SERVICE_NAME")
                {
                    ...
                    settings = GetTracerSettings(key, value, setBreakingChangesEnabled);
                }
            ...
            static TracerSettings GetTracerSettings(string key, string value, bool setBreakingChangesEnabled)
            {
                Environment.SetEnvironmentVariable(key, value, EnvironmentVariableTarget.Process);
                Environment.SetEnvironmentVariable(ConfigurationKeys.BreakingChangesEnabled, setBreakingChangesEnabled.ToString(), EnvironmentVariableTarget.Process);
                IConfigurationSource source = new EnvironmentConfigurationSource();
                return new TracerSettings(source);
            }
        }

        private void AssertJsonConfigurationSource(IEnumerable<(string Key, string Value, Func<TracerSettings, object> Getter, object Expected)> testData, bool setBreakingChangesEnabled = false)
        {
            foreach (var (key, value, settingGetter, expectedValue) in testData)
            {
                var config = new Dictionary<string, string> { [key] = value };
                config.Add(ConfigurationKeys.BreakingChangesEnabled, setBreakingChangesEnabled.ToString());
