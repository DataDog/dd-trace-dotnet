...
namespace Datadog.Trace.Tests.Sampling
{
    ...
    public class CustomSamplingRuleRegexTests
    {
        ...
        public void Constructs_With_ResourceName()
        {
            ...
            var matchingSpan = new Span(new InternalSpanContext(1, 1, serviceName: "foo"), DateTimeOffset.Now) { ResourceName = "/api/v1/user/123" };
            var nonMatchingSpan = new Span(new InternalSpanContext(1, 1, serviceName: "foo"), DateTimeOffset.Now) { ResourceName = "/api/v2/user/123" };
        ...
        public void Constructs_With_Tags()
        {
            ...
            var matchingSpan = new Span(new InternalSpanContext(1, 1, serviceName: "foo"), DateTimeOffset.Now);
            matchingSpan.SetTag("http.method", "GET");
            matchingSpan.SetMetric("http.status_code", 200);

            var nonMatchingSpan1 = new Span(new InternalSpanContext(1, 1, serviceName: "foo"), DateTimeOffset.Now);
            nonMatchingSpan1.SetTag("http.method", "GET");

            var nonMatchingSpan2 = new Span(new InternalSpanContext(1, 1, serviceName: "foo"), DateTimeOffset.Now);
