...
namespace Datadog.Trace.ClrProfiler.IntegrationTests.CI.Agent
{
    public class ApmAgentWriterTests
    {
        ...
        {
            var spans1 = new ArraySegment<Span>(new[] { Span.CreateSpan(Span.CreateSpanContext(1, 1), DateTimeOffset.UtcNow) });
            ...
            var spans2 = new ArraySegment<Span>(new[] { Span.CreateSpan(Span.CreateSpanContext(2, 2), DateTimeOffset.UtcNow) });
