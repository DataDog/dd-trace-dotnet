...
namespace Datadog.Trace.ClrProfiler.IntegrationTests.CI.Agent
{
    public class ApmAgentWriterTests
    {
        ...
        {
            var spans1 = new ArraySegment<Span>(new[] { new Span(new InternalSpanContext(1, 1), DateTimeOffset.UtcNow) });
            ...
            var spans2 = new ArraySegment<Span>(new[] { new Span(new InternalSpanContext(2, 2), DateTimeOffset.UtcNow) });
