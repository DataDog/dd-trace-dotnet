...
namespace Datadog.Trace.ClrProfiler.IntegrationTests.AdoNet
{
    ...
    public class MicrosoftDataSqlClientTests : TracingIntegrationTest
    {
        ...
        public async Task SubmitsTraces(string packageVersion, string metadataSchemaVersion, bool dbmEnabled, string propagation)
        {
            ...
            if (dbmEnabled && propagation == "full")
            {
                var instrumentationSpans = agent.WaitForSpans(expectedSpanCount, operationName: "set context_info");
                Assert.Equal(expectedSpanCount, instrumentationSpans.Count);
                ValidateIntegrationSpans(instrumentationSpans, metadataSchemaVersion, clientSpanServiceName, isExternalSpan);
            }
