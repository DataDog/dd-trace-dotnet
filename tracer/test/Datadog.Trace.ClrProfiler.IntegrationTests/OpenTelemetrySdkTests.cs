...
namespace Datadog.Trace.ClrProfiler.IntegrationTests
{
    ...
    public class OpenTelemetrySdkTests : TracingIntegrationTest
    {
        ...
        public async Task SubmitsOtlpTraces(string packageVersion, string datadogTracesEnabled, string otelTracesEnabled, string protocol)
        {
            ...
            snapshotName = otelTracesEnabled.Equals("true") ? $"{snapshotName}_OTEL" : $"{snapshotName}_DD";
            ...
            using (await RunSampleAndWaitForExit(agent, packageVersion: packageVersion ?? "1.13.1"))
            {
                ...
                {
                    attribute["value"]!["string_value"] = "normalized-git-commit-sha";
                }
                ...
                foreach (var span in tracesRequests.SelectTokens("$..spans[*]"))
                {
                    ...
                    span["trace_id"] = "normalized-trace-id";
                    span["span_id"] = "normalized-span-id";
                    if (span["parent_span_id"] != null)
                    {
                        span["parent_span_id"] = "normalized-parent-span-id";
                    }
                }

                foreach (var attribute in tracesRequests.SelectTokens("$..spans[*].attributes[?(@.key == 'otel.trace_id')]"))
                {
                    attribute["value"]!["string_value"] = "normalized-otel-trace-id";
                }

                foreach (var link in tracesRequests.SelectTokens("$..links[*]"))
                {
                    link["trace_id"] = "normalized-trace-id";
                    link["span_id"] = "normalized-span-id";
                }

                foreach (var @event in tracesRequests.SelectTokens("$..events[*]"))
                {
                    if (@event["time_unix_nano"] != null)
                    {
                ...
                if (datadogTracesEnabled.Equals("true"))
                {
                    ...
                    {
                        tracesRequest["resource_spans"].Should().HaveCount(1);
                        var resourceAttributes = tracesRequest["resource_spans"][0]["resource"]["attributes"];
                    ...
                    foreach (var tracesRequest in tracesRequests)
                    {
                        tracesRequest["resource_spans"][0]["scope_spans"].Should().HaveCount(1);
                        var spans = tracesRequest["resource_spans"][0]["scope_spans"][0]["spans"] as JArray;
                    ...
                    tracesRequests[0]["resource_spans"][0]["scope_spans"][0]["spans"] = sortedSpans;
                    finalJson = tracesRequests[0].ToString(Formatting.Indented);
                }
                else
                {
                    // Sort the spans by name to stabilize
                    foreach (var scopeSpan in tracesRequests.SelectTokens("$..scope_spans[*]"))
                    {
