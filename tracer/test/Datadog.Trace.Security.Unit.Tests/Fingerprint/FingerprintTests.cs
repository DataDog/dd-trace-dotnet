...
public class FingerprintTests : WafLibraryRequiredTest
{
    ...
    public void GivenAFingerprintRequest_WhenRunWAF_FingerprintIsGenerated()
    {
        ...
        args.Add(AddressesConstants.WafContextProcessor, new Dictionary<string, object> { { "fingerprint", true } });
    ...
    public void GivenAFingerprintRequest_WhenRunWAFWithEphemeral_FingerprintIsGenerated()
    {
        ...
        var args = CreateRequestArgs();
        var result = context.Run(args, TimeoutMicroSeconds);
        result.FingerprintDerivatives.Should().BeNullOrEmpty();
        ...
        resultEph.FingerprintDerivatives.Should().HaveCount(4);
    }

    [Fact]
    public void GivenAFingerprintRequest_WhenRunWAFWithEphemeral_FingerprintIsGenerated2()
    {
        var ruleFile = "rasp-rule-set.json";
        var context = InitWaf(true, ruleFile, new Dictionary<string, object>(), out var waf);
        var args = CreateRequestArgs2();
        var result = context.Run(args, TimeoutMicroSeconds);
        result.FingerprintDerivatives.Should().BeNullOrEmpty();
        context.RunWithEphemeral(new Dictionary<string, object>() { { "server.io.fs.file", "/nonexisting.txt" } }, TimeoutMicroSeconds, true);
        args = new Dictionary<string, object>
        {
            { AddressesConstants.WafContextProcessor, new Dictionary<string, object> { { "fingerprint", true } } },
        };
        ...
        var resultEph = context.RunWithEphemeral(args, TimeoutMicroSeconds, true);
        resultEph.FingerprintDerivatives.Should().HaveCount(3);
    }

    private static Dictionary<string, object> CreateRequestArgs()
    {
    ...
    private static Dictionary<string, object> CreateRequestArgs2()
    {
        return new Dictionary<string, object>
        {
