...
@"
      <TargetFramework>{0}</TargetFramework>";
        ...
        private const string RunOnWindowsMetadata =
@"
      <RunOnWindows>true</RunOnWindows>";

        private const string EntryFormat =
@"    <PackageVersionSample Include=""test\test-applications\integrations\{0}\{0}.csproj"">
      <Properties>ApiVersion={1};RestoreRecursive=false;BuildProjectReferences=false</Properties>{2}{3}
        ...
        public override void Write(PackageVersionEntry packageVersionEntry, IEnumerable<string> netFrameworkPackageVersions, IEnumerable<string> netCorePackageVersions)
        {
            ...
            foreach (string packageVersion in netFrameworkPackageVersions)
            {
                ...
                string runOnWindowsMetadataString = packageVersionEntry.RunOnWindows ? RunOnWindowsMetadata : string.Empty;
                FileStringBuilder.AppendLine(string.Format(EntryFormat, packageVersionEntry.SampleProjectName, packageVersion, targetFrameworkProperty, runOnWindowsMetadataString));
            }
